...
| ~~**A3**~~ | ~~88 elementos a 9px en teléfono~~ — **RESUELTO PARCIAL, por decisión del usuario.** Se mostró el mockup de subir un escalón la escala completa y se veía grande, así que suben **solo `--text-micro` (9→10) y `--text-caption` (10→11)** bajo `(pointer: coarse)`. Son el ~70% del texto en móvil, o sea casi toda la ganancia de legibilidad, y al no tocar body/title las tablas densas siguen entrando sin scroll extra. Medido en iPhone 13 real: 9px pasa de **88 a 25** elementos; escritorio **idéntico al píxel**. | `index.css` | ✓ |
