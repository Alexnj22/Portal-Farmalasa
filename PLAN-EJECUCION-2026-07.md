...
|---|---|---|---|
| 6.A | Capa de datos | 390 `supabase.from()` en 58 archivos, sin capa, sin caché; el límite 1000-filas se resuelve a mano vista por vista | 🟡 **En progreso — 3 PRs aplicados. Meta explícita del usuario: migrar los 385 sitios restantes, sin excepción (no solo las tablas de mayor tráfico).** PR1 (2026-07-15, v2.17.9): `src/data/inventory.js` (4 funciones), `WidgetInventorySearch.jsx`. **Hallazgo real de paso**: 2 de las 3 queries del widget a `inventory`/`products` NO usaban `fetchAllRows` — un término de búsqueda amplio podía truncar resultados en silencio. PR2 (2026-07-15, v2.17.10): `src/data/pedidos.js` (22 funciones), las 45 llamadas de `TabPedidos.jsx`. Archivo de mayor riesgo del inventario — flujos secuenciales/ramificados con queries entrelazadas: sustitución 1:1, sin tocar control de flujo. Deduplicación real donde el query era idéntico entre sitios (`updatePedidoSucursalStatus`/`fetchPedidoSucursalStatus` genéricos, ~15 sitios; `fetchBodegaBranchId`, 4 sitios). **Bug de paginación corregido de paso**: dos while-loops manuales duplicando `fetchAllRows` → reemplazados. PR3 (2026-07-15, v2.17.11): `src/data/productos.js` (15 funciones), las 29 llamadas de `TabCatalogo.jsx`. Dos pares de duplicados literales consolidados (`updateProductFoto` cubre 2 sitios; `fetchProductDetail` cubre las 5 queries idénticas de `prefetchRow`+`toggleRow`, 10 sitios). El query dinámico de la lista principal (`loadProducts`, con `.or/.eq/.order` condicionales) se consolidó en `fetchProductsList(params)`; el control de flujo (early-return si `effectiveBids` queda vacío) se quedó en el componente. PR3 verificado en vivo con Playwright contra datos reales de producción (contadores 4,354 activos/818 inactivos/11 nuevos/6 modificados/4 con pérdida/27 margen bajo, búsqueda "electrolit" y fila expandida con precios reales). PR4 (2026-07-15, v2.17.12): `src/data/system.js` (24 funciones), 34 llamadas de `systemSlice.js` — el conteo real era 34, no 31 (el grep inicial de línea única no detectaba las `supabase\n.from(...)` partidas en dos líneas, comunes en este archivo). **Deliberadamente NO se tocó `fetchBoot`** (11 sitios) ni `bootStatus`/`isBootSyncing`/`bootPromise` — es la función de mayor tráfico y riesgo del proyecto (corre en cada login), su refactor es 6.B con prueba en staging primero; `fetchKioskBoot` sí se migró (función independiente, no toca la máquina de boot). 4 pares de duplicados literales consolidados (`updateEmployeeFields`, `insertEmployeeDocument`, `updateEmployeeEventMetadata`, `setShiftActive`). Verificado en vivo: login (fetchBoot intacto), Centro de Comunicaciones, Horarios y Turnos, Jerarquía Institucional, todos con datos reales. Los paths de escritura (eventos RRHH, roles, avisos, turnos, asuetos, rosters) no se ejercitaron en vivo por no escribir datos de RRHH reales sin permiso explícito — verificados por sustitución 1:1 exacta de cada query. Los 4 PRs con build+lint+15 tests verdes. Quedan 277 sitios en 58 archivos (ver tasklist de la sesión: `TabMinMax.jsx` 20 restantes, `FacturacionView.jsx` 18, y el resto repartido en ~56 archivos más) |
