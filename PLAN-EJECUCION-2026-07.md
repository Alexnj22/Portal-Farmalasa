...
| 6.C | Dividir `TabMinMax.jsx`/`TabPedidos.jsx` (~3,900 líneas c/u) | Mini-apps en un archivo | 🟡 **En progreso — 4 PRs aplicados 2026-07-15 (v2.17.2→v2.17.5).** `TabMinMax.jsx`: 3947→2584 líneas (-1363, -34%). PR1: 6 componentes 100% presentacionales de menor acoplamiento + helpers. PR2: `AbcXyzMatrix` y `RowActions`. PR3: `ExpandedPanel` (el sub-componente más grande y con más acoplamiento real — fetches RPC propios en 2 waves, consume `StockBar`/`AbcXyzBadge` ya extraídos). PR4: `ConfigPanel` (escribe en `stock_config`) y `LabsPanel` (visibilidad de laboratorios, con auditoría). 10 sub-componentes + todos los helpers/constantes compartidos ya viven en `src/views/productos/tabminmax/`. Solo queda en `TabMinMax.jsx` el componente principal (~1,955 líneas: fetch/estado de la tabla, filtros, `DataTable`, modales de historial/publicación) — extraerlo requeriría partir el propio hook de estado, no es "otro sub-componente más", así que queda para una sesión aparte con más análisis. Extracción mecánica en los 4 PRs, sin cambio de comportamiento — verificado en vivo con Playwright contra datos reales en cada uno (incl. `ConfigPanel`/`LabsPanel` abriendo con valores/listas reales). Después: mismo tratamiento completo para `TabPedidos.jsx` (`MotorcycleAnim`, `PauseModal`, `AnularModal`, `ApoioScanModal`, `ItemSection`, `LifecycleTimeline`, `DifSection`, etc. ya identificados) |
