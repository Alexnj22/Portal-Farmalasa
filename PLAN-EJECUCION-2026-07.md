...
| 6.C | Dividir `TabMinMax.jsx`/`TabPedidos.jsx` (~3,900 líneas c/u) | Mini-apps en un archivo | 🟡 **En progreso — 2 PRs aplicados 2026-07-15 (v2.17.2→v2.17.3).** `TabMinMax.jsx`: 3947→3507 líneas (-440). PR1: 6 componentes 100% presentacionales de menor acoplamiento (`CoverageBar`, `StockBar`, `AbcXyzBadge`, `CardSkeletons`, `CostCards`, `DraftCostCard`) + helpers compartidos (`fmtMoney`, `normXyz` → `tabminmax/helpers.js`; `ABC_CFG`, `XYZ_CFG` → `tabminmax/constants.js`). PR2: `AbcXyzMatrix` (grilla clicable) y `RowActions` (botones de fila + dropdown "Más") — mismo nivel de acoplamiento bajo, props+callbacks sin closures sobre el estado de `TabMinMax`. Todo en `src/views/productos/tabminmax/`. Extracción mecánica, sin cambio de comportamiento — verificado en vivo con Playwright contra datos reales en cada PR. Ambos archivos ya tenían buena separación interna de sub-componentes (no era "todo junto sin límites", era "todo junto en un solo archivo"), así que el resto es mecánico: queda por extraer de `TabMinMax.jsx` → `ExpandedPanel` (~547 líneas, el más grande y con más acoplamiento real — hace sus propios fetches RPC), `ConfigPanel`, `LabsPanel`; después el mismo tratamiento completo para `TabPedidos.jsx` (`MotorcycleAnim`, `PauseModal`, `AnularModal`, `ApoioScanModal`, `ItemSection`, `LifecycleTimeline`, `DifSection`, etc. ya identificados) |
