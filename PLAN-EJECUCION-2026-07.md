...
| 6.C | Dividir `TabMinMax.jsx`/`TabPedidos.jsx` (~3,900 líneas c/u) | Mini-apps en un archivo | 🟡 **En progreso — 5 PRs aplicados 2026-07-15 (v2.17.2→v2.17.6).** `TabMinMax.jsx`: 3947→2584 líneas (-34%), 10 sub-componentes extraídos en 4 PRs — ver detalle en commits. Solo queda el componente principal (~1,955 líneas), que requeriría partir el propio hook de estado — no es mecánico, queda para sesión aparte. **`TabPedidos.jsx` arrancado (PR5): 3943→3840 líneas.** Primer lote: animaciones de stage (`StageAnim` + 6 sub-animaciones), `EmpChip`, `StagePill`, `SucPill` → `src/views/pedidos/tabpedidos/`. **Hallazgo de paso, no corregido:** `StageAnim`/`EmpChip`/`StagePill` no tienen ningún caller en el archivo — código muerto real, extraído igual (mecánico), decisión de si se borra queda aparte. `SucPill` sí tiene caller real, verificado en vivo con Playwright en la pestaña PEDIDOS real (pills de color por sucursal correctos, junto al `LifecycleTimeline` sin extraer aún funcionando igual). Quedan en `TabPedidos.jsx`: `PauseModal`, `AnularModal`, `ApoioScanModal`, `ItemSection`, `PauseBadge`, `LifecycleTimeline`, `ItemSections`, `DifSection`, `PostCompletionSection`, `ReceptionActions`, `FilterPill`, y el componente principal |
