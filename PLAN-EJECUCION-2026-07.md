...
| 6.C | Dividir `TabMinMax.jsx`/`TabPedidos.jsx` (~3,900 líneas c/u) | Mini-apps en un archivo | 🟡 **En progreso — 3 PRs aplicados 2026-07-15 (v2.17.2→v2.17.4).** `TabMinMax.jsx`: 3947→2913 líneas (-1034, -26%). PR1: 6 componentes 100% presentacionales de menor acoplamiento (`CoverageBar`, `StockBar`, `AbcXyzBadge`, `CardSkeletons`, `CostCards`, `DraftCostCard`) + helpers compartidos. PR2: `AbcXyzMatrix` (grilla clicable) y `RowActions` (botones de fila + dropdown "Más"). PR3: `ExpandedPanel` (~547 líneas, el sub-componente más grande y con más acoplamiento real — hace sus propios fetches RPC en 2 waves, usa `useAuth`/`useNowTick`, y consume `StockBar`/`AbcXyzBadge` ya extraídos — primera extracción que consume a otra). De paso quedaron compartidos sin duplicar: `ERP_NAMES`/`ERP_ORDER`/`ALERT` (usados también en ~25 sitios del cuerpo principal) y `sortedPres`/`smallestPres`/`formatUnits`/`formatDominant`. Todo en `src/views/productos/tabminmax/`. Extracción mecánica, sin cambio de comportamiento — verificado en vivo con Playwright contra datos reales en cada PR (el de `ExpandedPanel` expandiendo una fila real: grilla de 7 sucursales, compras/ventas, proyección de stock, historial de cálculos, todo idéntico). Ambos archivos ya tenían buena separación interna de sub-componentes (no era "todo junto sin límites", era "todo junto en un solo archivo"), así que el resto es mecánico: queda por extraer de `TabMinMax.jsx` → `ConfigPanel`, `LabsPanel`, y el propio componente `TabMinMax` (~1,220 líneas restantes tras los sub-componentes); después el mismo tratamiento completo para `TabPedidos.jsx` (`MotorcycleAnim`, `PauseModal`, `AnularModal`, `ApoioScanModal`, `ItemSection`, `LifecycleTimeline`, `DifSection`, etc. ya identificados) |
