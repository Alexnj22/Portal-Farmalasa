...
| 4.4 | `refresh_product_sales_monthly_agg()` cada hora | Fase 0 | ✅ **Cerrado sin cambios 2026-07-15, variación diagnosticada a fondo 2026-07-16.** Análisis con `cron.job_run_details` (jobid 138, ~116 corridas reales, no el promedio acumulado de `pg_stat_statements`): la duración normal por hora del día oscila **6.6s–9.6s de promedio** (rango real 0.8s–15.5s), NO por contención con otros crons — verificado directamente cruzando el minuto exacto de las corridas más lentas contra `cron.job_run_details` de los demás jobs (`dte-*-min`, `sync-inv-suc*-1min`): en el minuto de una corrida de 18.3s, esos otros jobs terminaron en 10-200ms, sin bloqueo ni espera. **Causa real, confirmada leyendo la función**: no es incremental — cada corrida vuelve a escanear y reagrupar el CTE `fresh` sobre **3 meses completos** de `sales_invoice_items JOIN sales_invoices` desde cero (el `DELETE`+`INSERT ON CONFLICT` que sigue solo evita escrituras redundantes, pero no evita el recálculo). Dos patrones explican la variación: (1) **hora 12 UTC es consistentemente la más lenta del día** (avg 7.77s, incluye los picos de 14-15.5s) — es la primera corrida tras la ventana nocturna sin crons (06:00–11:59 UTC), con buffer cache frío para esas tablas; (2) horas 12-23 UTC (horario de atención) promedian ~30-40% más que 0-5 UTC (6.6-7.1s) por el volumen creciente de facturas del mes en curso dentro de la ventana de 3 meses. Los 2 outliers extremos del historial (32.8s y 15.6s, ambos 2026-07-08 15:48-16:15 UTC) coinciden exactamente con el outage documentado de RLS en tablas calientes — no son parte del patrón normal. **Fix real si se quiere quitar la variación** (no aplicado, no urgente — el job nunca falla ni se solapa con el siguiente por el margen de 1h): volverla incremental con watermark (procesar solo facturas nuevas/modificadas desde la última corrida exitosa) en vez de re-escanear 3 meses siempre; mismo patrón que ya se usa en los syncs (`IS DISTINCT FROM` + upsert condicional) |
