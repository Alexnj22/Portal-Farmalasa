...
|---|---|---|---|
| 6.A | Capa de datos | 390 `supabase.from()` en 58 archivos, sin capa, sin caché; el límite 1000-filas se resuelve a mano vista por vista | 🟡 **En progreso — 2 PRs aplicados. Meta explícita del usuario: migrar los 385 sitios restantes, sin excepción (no solo las tablas de mayor tráfico).** PR1 (2026-07-15, v2.17.9): `src/data/inventory.js`, 4 funciones (`fetchProductPhotoMap`, `fetchProductsByPrincipioActivo`, `searchInventory`, `fetchInventoryByProductIds`), todas paginadas con `fetchAllRows`. Empezado por `WidgetInventorySearch.jsx`. **Hallazgo real de paso**: 2 de las 3 queries del widget a `inventory`/`products` NO usaban `fetchAllRows` — un término de búsqueda amplio podía truncar resultados en silencio. Verificado en vivo con Playwright: búsqueda real "ensure" → resultados idénticos a antes. **Nota de diseño**: funciones async simples, no hooks — este consumidor dispara la búsqueda de forma imperativa con debounce propio; se evalúa hook vs. función caso por caso según cada consumidor. PR2 (2026-07-15, v2.17.10): `src/data/pedidos.js` (22 funciones), las 45 llamadas de `TabPedidos.jsx` (componente principal, post-6.C) migradas. Archivo de mayor riesgo del inventario 6.A — flujos secuenciales/ramificados (confirmación de llegada, reenvío de cajas, finalizar con cajas) con queries entrelazadas en el mismo handler: se hizo sustitución 1:1 de cada query por su función, sin tocar el control de flujo. Deduplicación real donde el query era 100% idéntico entre sitios (`updatePedidoSucursalStatus`/`fetchPedidoSucursalStatus` genéricos cubren ~15 sitios; `fetchBodegaBranchId` cubre 4 lookups idénticos). **Bug de paginación corregido de paso**: `fetchItems` tenía dos while-loops manuales de paginación 1000-en-1000 (`pedido_items`, `pedido_item_eventos`) duplicando lo que ya hace `fetchAllRows` — reemplazados. Verificado en vivo con Playwright contra datos reales: tab PEDIDOS, 6 sucursales activas, expandir pedido 04-140726-1-S3 (474 solicitados/151 enviados/308 sin stock/13 por regla) → drill-down a "Productos enviados" con datos reales intactos. Build+lint+15 tests verdes en ambos PRs. Quedan 340 sitios en 60 archivos (ver tasklist de la sesión: `TabCatalogo.jsx` 29, `systemSlice.js` 31, `TabMinMax.jsx` 20 restantes, `FacturacionView.jsx` 18, y el resto repartido en ~55 archivos más) |
