...
|---|---|---|---|
| 6.A | Capa de datos | 390 `supabase.from()` en 58 archivos, sin capa, sin caché; el límite 1000-filas se resuelve a mano vista por vista | 🟡 **En progreso — 6 PRs aplicados (2026-07-15, v2.17.9→v2.17.14). Meta explícita del usuario: migrar los 385 sitios restantes, sin excepción.** Patrón general (todos los PRs): `src/data/<entidad>.js` nuevo/extendido, sustitución 1:1 de cada query sin tocar control de flujo de negocio, dedup solo si el query es 100% idéntico entre sitios, build+lint+15 tests + verificación en vivo con Playwright contra datos reales antes de cada commit. PR1 `inventory.js` (4 fn) `WidgetInventorySearch.jsx` — hallazgo: 2/3 queries no paginaban. PR2 `pedidos.js` (22 fn) 45 sitios `TabPedidos.jsx` (mayor riesgo: flujos secuenciales/ramificados); 2 while-loops manuales reemplazados por `fetchAllRows`. PR3 `productos.js` (15 fn) 29 sitios `TabCatalogo.jsx`. PR4 `system.js` (24 fn) 34 sitios `systemSlice.js` (conteo real, no 31 — grep de línea única no veía `supabase\n.from()` partido en 2 líneas, patrón repetido en varios PRs siguientes) — **deliberadamente NO se tocó `fetchBoot`** (11 sitios, `bootStatus`: mayor tráfico/riesgo del proyecto, refactor es 6.B con staging primero). PR5 `stockParams.js` (10 fn) 23 sitios `TabMinMax.jsx` — mayoría upsert/update a `product_stock_params` con clave compuesta, consolidados en 2 funciones genéricas. PR6 `facturacion.js` (19 fn) 23 sitios `FacturacionView.jsx` (conteo real, no 18) — **hallazgo real**: la query de pagos no-efectivo filtraba `sales_invoices` (tabla flagged) SIN `fetchAllRows`, corregido. Verificado en vivo: SALTOS con gaps reales, NO EFECTIVO con 470 pendientes reales ($11,544.66) y nombres de clientes reales. PR7 `employees.js` (10 fn) + `branches.js` (13 fn), 35 sitios en `employeeSlice.js`+`branchSlice.js` (conteo real, no 29) — reutilización real: `employee_branches`/`employee_rosters` ya tenían función en `data/system.js` (PR4), `employeeSlice.js` las importa en vez de duplicar. Verificado en vivo: Monitor en Tiempo Real (49 empleados reales), Sucursales (7 reales), Gestión de Personal (48 reales). **Nota transversal**: en PR4/PR5/PR7 los paths de escritura (RRHH, roles, avisos, turnos, MIN/MAX de compras, sucursales, asistencia) no se ejercitaron en vivo por no tocar datos reales sin permiso explícito — verificados por sustitución 1:1 exacta. Quedan 196 sitios en 54 archivos (ver tasklist de la sesión) |
