...
|---|---|---|---|
| 6.A | Capa de datos | 390 `supabase.from()` en 58 archivos, sin capa, sin caché; el límite 1000-filas se resuelve a mano vista por vista | 🟡 **En progreso — 1er PR aplicado 2026-07-15 (v2.17.9).** `src/data/inventory.js` (antes `src/data/` solo tenía catálogos estáticos): 4 funciones (`fetchProductPhotoMap`, `fetchProductsByPrincipioActivo`, `searchInventory`, `fetchInventoryByProductIds`), todas paginadas con `fetchAllRows`. Empezado por `WidgetInventorySearch.jsx` como sugería el plan. **Hallazgo real de paso**: 2 de las 3 queries del widget a `inventory`/`products` NO usaban `fetchAllRows` — `inventory` es una de las tablas que CLAUDE.md marca como obligatoriamente paginada; un término de búsqueda amplio podía truncar resultados en silencio. Cerrado. Verificado en vivo con Playwright: búsqueda real "ensure" → resultados idénticos a antes. **Nota de diseño**: no se implementó como "hook por entidad" (como decía la redacción original) sino como funciones async simples — este consumidor dispara la búsqueda de forma imperativa con debounce propio, un hook con auto-fetch no encaja; se evaluará hook vs. función caso por caso según cada consumidor en la migración oportunista. Migración oportunista, quedan 389 sitios |
