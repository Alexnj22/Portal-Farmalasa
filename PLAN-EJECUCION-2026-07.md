...
| 6.C | Dividir `TabMinMax.jsx`/`TabPedidos.jsx` (~3,900 líneas c/u) | Mini-apps en un archivo | 🟡 **En progreso — 6 PRs aplicados 2026-07-15 (v2.17.2→v2.17.7).** `TabMinMax.jsx`: 3947→2584 líneas (-34%), 10 sub-componentes en 4 PRs. Solo queda el componente principal (~1,955 líneas, requiere partir el propio hook de estado — sesión aparte). **`TabPedidos.jsx`: 3943→3457 líneas (2 PRs).** PR5: animaciones de stage + `EmpChip`/`StagePill`/`SucPill` (hallazgo de paso: `StageAnim`/`EmpChip`/`StagePill` sin ningún caller, código muerto real, extraído igual sin tocar — decisión de borrarlo queda aparte). PR6: `PauseModal`, `AnularModal`, `ApoioScanModal` (escáner de carné con lookup+upsert+auditoría propios) → `src/views/pedidos/tabpedidos/`. Verificación con límite honesto en PR6: los 3 modales requieren un pedido en estado específico de ciclo de vida que ningún pedido real tenía al verificar — no se forzó ese estado con un write real solo para probar; sí confirmado cero errores de consola cargando /pedidos y expandiendo una card real. Quedan en `TabPedidos.jsx`: `ItemSection`, `PauseBadge`, `LifecycleTimeline`, `ItemSections`, `DifSection`, `PostCompletionSection`, `ReceptionActions`, `FilterPill`, y el componente principal |
