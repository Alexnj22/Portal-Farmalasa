...
|---|---|---|---|
| 6.A | Capa de datos | 390 `supabase.from()` en 58 archivos, sin capa, sin caché; el límite 1000-filas se resuelve a mano vista por vista | 🟡 **En progreso — 10 PRs aplicados (2026-07-15, v2.17.9→v2.17.18), 294 sitios migrados de 385 (76%). Meta explícita del usuario: migrar TODOS, sin excepción.** Patrón fijo en cada PR: `src/data/<entidad>.js` nuevo/extendido, sustitución 1:1 de cada query sin tocar control de flujo, dedup solo si el query es idéntico entre sitios (incluida reutilización entre PRs del mismo bloque, ej. PR10 reutiliza `updatePedidoSucursalStatus` de PR2), build+lint+15 tests + Playwright en vivo contra datos reales antes de cada commit. Resumen por PR (detalle completo en el historial de commits `refactor(bloque6.A)`): PR1 `inventory.js` (4fn/WidgetInventorySearch, hallazgo: búsqueda sin paginar). PR2 `pedidos.js` (22fn/45 sitios TabPedidos.jsx). PR3 `productos.js` (15fn/29 sitios TabCatalogo.jsx). PR4 `system.js` (24fn/34 sitios systemSlice.js — `fetchBoot` deliberadamente intacto, es 6.B). PR5 `stockParams.js` (10fn/23 sitios TabMinMax.jsx). PR6 `facturacion.js` (19fn/23 sitios FacturacionView.jsx, hallazgo: pago no-efectivo sin paginar). PR7 `employees.js`+`branches.js` (23fn/35 sitios employeeSlice+branchSlice). PR8 `requests.js`+4 módulos chicos (50fn/63 sitios requestsSlice+practicantes+notifications+conteoInventario+payroll). PR9 `ventas.js`+`encuestas.js` (23fn/24 sitios VentasView+EncuestaAdminView.jsx). PR10 `recepcion.js`+`dashboard.js` (15fn/18 sitios RecepcionModal+DashboardView.jsx) — verificado en vivo con el Dashboard completo (49 empleados, 4 solicitudes pendientes, widgets de ventas reales, facturación del día real). **Hallazgos reales de paginación corregidos**: WidgetInventorySearch (PR1), pagos no-efectivo en Facturación (PR6), lote de product_precios en RecepcionModal (PR10). **Nota transversal**: desde PR4, los paths de escritura de datos sensibles no se ejercitan en vivo sin permiso explícito — se verifican por sustitución 1:1 exacta de cada query. Quedan 91 sitios en 43 archivos (ver tasklist de la sesión) |
