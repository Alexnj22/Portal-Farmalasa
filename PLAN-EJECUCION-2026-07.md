...
|---|---|---|---|
| 6.A | Capa de datos | 390 `supabase.from()` en 58 archivos, sin capa, sin caché; el límite 1000-filas se resuelve a mano vista por vista | 🟡 **En progreso — 5 PRs aplicados (2026-07-15, v2.17.9→v2.17.13). Meta explícita del usuario: migrar los 385 sitios restantes, sin excepción.** Patrón general: cada PR crea/extiende `src/data/<entidad>.js`, sustitución 1:1 de cada query (sin tocar control de flujo de negocio), dedup solo cuando el query es 100% idéntico entre sitios, build+lint+15 tests verdes + verificación en vivo con Playwright contra datos reales antes de cada commit. PR1 `src/data/inventory.js` (4 fn), `WidgetInventorySearch.jsx` — **hallazgo real**: 2/3 queries del widget no usaban `fetchAllRows`, búsqueda amplia podía truncar en silencio. PR2 `src/data/pedidos.js` (22 fn), 45 sitios de `TabPedidos.jsx` — archivo de mayor riesgo (flujos de negocio secuenciales/ramificados); 2 while-loops manuales de paginación reemplazados por `fetchAllRows`. PR3 `src/data/productos.js` (15 fn), 29 sitios de `TabCatalogo.jsx` — query dinámico de lista principal consolidado en `fetchProductsList(params)` preservando el control de flujo en el componente. PR4 `src/data/system.js` (24 fn), 34 sitios de `systemSlice.js` (conteo real, no 31 — grep de línea única no detectaba `supabase\n.from()` partidos en 2 líneas) — **deliberadamente NO se tocó `fetchBoot`** (11 sitios, `bootStatus`/`isBootSyncing`/`bootPromise`: función de mayor tráfico/riesgo del proyecto, corre en cada login, refactor es 6.B con staging primero); `fetchKioskBoot` sí migrado (independiente del boot). PR5 `src/data/stockParams.js` (10 fn), 23 sitios de `TabMinMax.jsx` (conteo real, no 20, mismo motivo de grep) — la mayoría upsert/update a `product_stock_params` con clave compuesta (erp_product_id, erp_sucursal_id), consolidados en `upsertStockParams`/`updateStockParams` genéricos (~15 sitios). Verificado en vivo: cost cards reales, matriz ABC×XYZ, tabla MIN·MAX, modal de historial con auditoría real. **Nota transversal**: en PR4 y PR5 los paths de escritura (RRHH, roles, avisos, turnos, MIN/MAX de compras) no se ejercitaron en vivo por no tocar datos reales de producción sin permiso explícito — verificados por sustitución 1:1 exacta de cada query, no por corrida en vivo. Quedan 254 sitios en 57 archivos (ver tasklist de la sesión: `FacturacionView.jsx` 18, y el resto repartido en ~56 archivos más) |
