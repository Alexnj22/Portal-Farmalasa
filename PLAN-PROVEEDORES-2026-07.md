...
- [ ] 4.1 `src/views/purchases/ProveedoresView.jsx` — estándares completos:
      GlassViewLayout + ViewTabBar (búsqueda header, `normSearch`/`tokenMatch`),
      filter pill EN EL BODY (§17 DESIGN.md — no repetir el error de
      ComprasView), DataTable estándar (numéricos centrados, sort client-side),
      LiquidSelect, empty state glassmorphism, sin border-l, contraste mínimo.
      Columnas: Proveedor (nombre + nombre comercial), NIT/NRC, Giro, Categoría
      (LiquidSelect inline si `can_edit`), Match ERP (badge o botón "Emparejar"
      — reusar el patrón `SupplierMatchCell` de FacturasCompraView, con botón
      cancelar), Docs (count), Última compra. Filtros: categoría /
      "(sin categoría)" / "(sin match ERP)" / clase / activo.
- [ ] 4.2 Modal detalle (UnifiedModal — TODO botón interno con `type="button"`,
      regla aprendida): datos fiscales completos, campos manuales editables
      (contacto, teléfono 2, nombre cheques, notas, activo, percibe_1 override),
      lista de sus últimos documentos (link a Facturas de Compra filtrado).
- [ ] 4.3 Checklist módulo nuevo: ruta en `App.jsx`, menú (grupo Inventario),
      `PermissionsView.jsx`, BD (Fase 1). `appendAuditLog` en TODA acción.
- [ ] 4.4 En `FacturasCompraView`: la columna proveedor pasa a preferir
      `proveedor_id` (maestro) mostrando el match ERP como dato secundario.
      (Cambio mínimo — no rehacer la vista.)
- [ ] 4.5 Migraciones de prod ANTES del push (Vercel auto-deploya). Bump
      `APP_VERSION` + changelog. Commit + push.
- [ ] 4.6 Verificación visual obligatoria (Playwright: lista real, asignar
      categoría, match ERP, guard de permisos con rol sin acceso).

### Fase 5 — Cierre

- [ ] 5.1 Advisor 0 ERRORES final, repasar criterios §6.
