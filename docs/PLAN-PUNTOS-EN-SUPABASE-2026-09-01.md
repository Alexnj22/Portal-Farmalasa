...
El bloque de pruebas completo está al final de este documento (§8) para volver a
...
| `puntos_canjear(customer_id, puntos, invoice_id, sucursal, motivo)` | FIFO sobre los lotes vivos; falla si el saldo no alcanza o si no llega al mínimo |
...
3. **El mínimo de canje se lee de la configuración, no de una constante.** Hoy
   son 100 puntos y el dato que reencuadra el programa dice que **el cuello de
   botella es el mínimo, no el reloj** (3 de cada 4 clientes activos no llegan a
   100 en seis meses, y alargar el plazo casi no lo mejora). Ese número va a
   moverse; que viva en `security_config` o en una tabla de parámetros y no
...
`vence_el = 2027-10-01`, y no hay que averiguar el origen de un solo punto.
...
- Las **27 cuentas con el mismo ticket cobrado dos veces**. Se migra el saldo tal
  como está: corregirlo sería quitarle puntos a alguien sin avisarle.
- Las **26 ventas anuladas con los puntos ya entregados** ($1,110), por decisión
  del usuario («restemos de ahora en adelante»).

---

## 5. El orden, y dónde está el freno
...
4. Acumular en PARALELO          ── los dos circuitos escribiendo, sin canjes
5. CUADRAR                       ── saldo de MySQL contra saldo de Postgres
6. Migrar los saldos históricos
7. El canje en el mostrador      ←── EL FRENO
8. Apagar MySQL
```

**Del 1 al 6 se puede hacer sin tocar la operación de la sala.** El paso 4 es la
prueba real: los dos sistemas mirando las mismas ventas y llegando al mismo
número, todos los días, hasta que nadie dude.

**El paso 7 es el que manda sobre el calendario y no es técnico.** Hoy el canje
lo hace la aplicación de ellos contra MySQL. Mientras eso no exista en el portal,
**la base vieja no se puede apagar** aunque todo lo demás esté listo.

---

## 6. Lo que hay que decidir antes de empezar

1. **Cómo canjea la caja.** ¿El portal expone algo que el sistema de la caja
   llame, o el canje pasa a una pantalla del portal —que ya está corriendo en las
   seis salas—? Esto define el paso 7 y con él la fecha de apagado.
2. **Cuánto tiempo queda de MariaDB.** Si son semanas, el paso 4 se acorta y hay
   que aceptar menos días de cuadre.
3. **El mínimo de canje.** Está en 100 y el dato dice que ahí está el cuello de
   botella. Si va a bajar, conviene que baje **antes** de la migración: cambiarlo
   después obliga a explicarle a la gente dos reglas en dos meses.
4. **Qué pasa con las 27 cuentas de doble crédito.** La propuesta es migrarlas
   como están. Es una decisión del negocio, no del código.

⚠️ **Y una que se toma antes de construir, no después:** el día que
`/mis-puntos` permita canjear, **DUI + teléfono deja de alcanzar** como llave.
Hoy es proporcionado porque la pantalla sólo *muestra* y no hace nada que no se
pueda deshacer. Para canjear hay que pasar a código por SMS.

---

## 7. Lo que hay que hacer igual, aunque esto no arranque
...
## 8. El bloque de pruebas, para volver a correrlo
...
## 9. Cuando se confirme, el orden exacto de la sesión
