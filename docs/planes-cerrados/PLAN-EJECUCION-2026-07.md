...
**Estado al 2026-07-16: CERRADO.** 3.1/3.2/3.4/3.5/3.6/3.7 resueltos. **3.3
diferido a propósito, con el usuario informado**: ya se había intentado el
2026-07-11 y revertido por exceder el alcance aprobado (requiere ~9,800
líneas de DDL en el registro de prod); no resuelve nada roto hoy (staging
ya funciona sin eso) — el usuario confirmó dejarlo en pausa tras conocer
el tamaño real de la operación. Detalle completo de cada ítem debajo.
...
| 3.2 | Sembrar catálogos de referencia (roles/branches/shifts/holidays/presentaciones/laboratorios) — NUNCA employees/ventas | ✅ Aplicado 2026-07-16 vía `execute_sql` directo sobre staging (`ewcmerxqjvludtgskuin`), no `apply_migration` — es carga de datos, no DDL, y staging no es prod. Fetch de las 6 tablas desde prod vía `json_agg`, insert con `jsonb_to_recordset` + `OVERRIDING SYSTEM VALUE` (columnas `id` son identity `GENERATED ALWAYS`; `presentaciones`/`laboratorios` no lo son, PK simple). `roles` insertado en 2 pasadas (parent_role_id/secondary_parent_role_id nulificados en el insert, luego UPDATE) por la FK autorreferencial. **Hallazgo no anticipado en el plan original**: `branches.settings.legal.nurses` contiene UUIDs reales de empleados (enfermeras) — rompía el compromiso "cero PII" de staging. Resuelto con el usuario en el momento: `jsonb_set(settings, '{legal,nurses}', '[]'::jsonb, true)` antes de insertar, vacía `nurses` preservando el resto de `settings` (rent/location/services, sin PII). Verificado: conteos en staging = prod exacto (roles 22, branches 8, shifts 14, holidays 27, presentaciones 232, laboratorios 355); 0 branches con `nurses` no vacío; self-referencia de `roles` (id=23 → parent=22, secondary=19) correcta |
...
**Estado al 2026-07-17: BLOQUE 4 CERRADO — 7/7.** 4.1/4.2/4.5/4.6 aplicados
en sesiones anteriores. 4.7 ya no aplica (resuelto indirectamente por
otro trabajo). 4.3 aplicado y verificado 2026-07-15 (`product_stock_params`
fuera de realtime, reemplazado por polling quirúrgico). 4.4 diagnosticado a
fondo 2026-07-15/16 (la variación real era 6.6-15.5s/corrida por full-scan
de 3 meses en cada corrida, no un promedio inflado) y vuelto incremental
con watermark 2026-07-17, aplicado y verificado en prod.
...
| 4.4 | `refresh_product_sales_monthly_agg()` cada hora | Fase 0 | ✅ **Diagnosticada 2026-07-15/16, vuelta incremental y aplicada en prod 2026-07-17.** Causa real (confirmada leyendo la función): no era incremental — cada corrida re-escaneaba y reagrupaba el CTE `fresh` sobre 3 meses completos de `sales_invoice_items JOIN sales_invoices` desde cero, 6.6-15.5s por corrida. Fix: nueva tabla `job_watermarks` (bookkeeping puro, RLS sin policies ni grants a anon/authenticated — mismo patrón que `login_rate_limit`) guarda el último `sales_invoices.updated_at` procesado; la función ahora filtra `touched_invoices` por ese watermark (con margen de seguridad de 15min contra la ventana de visibilidad de READ COMMITTED), deriva las `affected_keys` (`year_month`,`branch_id`,`erp_product_id`,`presentacion`) tocadas por esas facturas, y solo recalcula/DELETE+INSERT esas keys — no todo el rango de 3 meses. Watermark seguro porque `sales_invoice_items` no tiene columna de auditoría propia pero `sales_invoices.updated_at` sí es un proxy confiable (verificado: `sync-dte-sales` siempre bumpea `updated_at` de la factura padre cuando sus items cambian; sin trigger de auto-touch; el único UPDATE directo del frontend, `recibido_mh` en `facturacion.js`, no toca ninguna columna que este agregado lea). Nuevo índice `idx_sales_invoices_updated_at` (`CREATE INDEX CONCURRENTLY`, no bloquea lecturas/escrituras). Verificado en transacciones `ROLLBACK` sobre datos reales de prod antes de aplicar: primera corrida (watermark vacío) da resultado byte-idéntico al full-scan viejo (27,648 filas, misma cantidad/neto); corrida incremental simulada tocando 5 facturas reales → 0 discrepancias en 500 keys de muestra no tocadas y 0 discrepancias en las keys sí afectadas contra un recálculo manual completo. Aplicado a prod: migración `20260717015500_bloque4_4_incremental_product_sales_monthly_agg.sql`, primera invocación real en vivo escribió 0 filas (la tabla ya estaba sincronizada por la última corrida del cron viejo — anti-churn correcto), conteos post-aplicación idénticos a antes. Advisor de seguridad: solo INFO (RLS sin policy en `job_watermarks`, mismo patrón aceptado que `login_rate_limit`), sin WARN/ERROR nuevos |
...
|---|---|---|---|
| 6.A | Capa de datos | 390 `supabase.from()` en 58 archivos, sin capa, sin caché; el límite 1000-filas se resuelve a mano vista por vista | 🟢 **CERRADO (2026-07-15, v2.17.9→v2.17.30, 22 PRs). Los 173 sitios reales del recuento corregido migrados al 100%.** Patrón fijo en cada PR: `src/data/<entidad>.js` nuevo/extendido, sustitución 1:1 de cada query sin tocar control de flujo, dedup solo si el query es idéntico entre sitios (incluida reutilización entre PRs del mismo bloque — para PR13, 11 de 29 sitios no crearon función nueva, solo importaron una ya existente de un PR anterior), build+lint+15 tests + Playwright en vivo contra datos reales antes de cada commit. Resumen por PR (detalle completo en el historial de commits `refactor(bloque6.A)`): PR1 `inventory.js` (4fn/WidgetInventorySearch, hallazgo: búsqueda sin paginar). PR2 `pedidos.js` (22fn/45 sitios TabPedidos.jsx). PR3 `productos.js` (15fn/29 sitios TabCatalogo.jsx). PR4 `system.js` (24fn/34 sitios systemSlice.js — `fetchBoot` deliberadamente intacto, es 6.B). PR5 `stockParams.js` (10fn/23 sitios TabMinMax.jsx). PR6 `facturacion.js` (19fn/23 sitios FacturacionView.jsx, hallazgo: pago no-efectivo sin paginar). PR7 `employees.js`+`branches.js` (23fn/35 sitios employeeSlice+branchSlice). PR8 `requests.js`+4 módulos chicos (50fn/63 sitios requestsSlice+practicantes+notifications+conteoInventario+payroll). PR9 `ventas.js`+`encuestas.js` (23fn/24 sitios VentasView+EncuestaAdminView.jsx). PR10 `recepcion.js`+`dashboard.js` (15fn/18 sitios RecepcionModal+DashboardView.jsx). PR11 `permissions.js`+`cotizaciones.js` (14fn/25 sitios PermissionsView+CotizacionesView.jsx). PR12 `laboratorios.js`+`dispatchRules.js`+`attendanceAudit.js` (22fn/25 sitios TabPoliticaVencimiento+TabReglas+AttendanceAuditView.jsx). PR13 `inventarioTab.js`+`employeeHome.js`+extensiones a `encuestas.js`/`productos.js` (13fn nuevas/29 sitios TabInventario+EmployeeHomeView+EncuestaView+SrsEnriquecerModal.jsx) — verificado en vivo: Inventario (13,925 productos, 525 vencidos, $313,780.13 inversión), Inicio de empleado con horario real, Clima Organizacional con score global 78/100. **Hallazgos reales de paginación corregidos**: WidgetInventorySearch (PR1), pagos no-efectivo en Facturación (PR6), lote de product_precios en RecepcionModal (PR10) y CotizacionesView (PR11), dispatch_rules en TabReglas (PR12). **Nota transversal**: desde PR4, los paths de escritura de datos sensibles no se ejercitan en vivo sin permiso explícito — se verifican por sustitución 1:1 exacta de cada query. **Recuento corregido 2026-07-15 tarde**: un escaneo completo del codebase (excluyendo `src/data/`) encontró 173 sitios reales en 54 archivos — el inventario original de 385 (y las cuentas "restantes" de cada PR) subestimaban por el mismo motivo que systemSlice/TabMinMax/FacturacionView: el grep de línea única no veía `supabase\n.from()` partido en 2 líneas, y esto afectaba también a archivos nunca inventariados en el primer barrido. Top offenders: `vacationPlanSlice.js` (14), `systemSlice.js` (11 restantes — todos dentro de `fetchBoot`, fuera de alcance por diseño), `SchedulesView.jsx` (8), `TabRutas.jsx`/`WidgetAnnulmentRequest.jsx` (7 c/u). Continuando con el mismo patrón. PR14 (v2.17.22) `vacationPlans.js` (8fn/14 sitios vacationPlanSlice.js, primer archivo del recuento corregido) — reutiliza `updateApprovalRequest` de `data/requests.js`; verificado en vivo: /vacation-plan con asignación real (Andy Mancia, 02-ene→16-ene, CONFIRMADO). PR15 (v2.17.23) `schedules.js` (5fn nuevas/15 sitios SchedulesView.jsx+TabRutas.jsx, 8+7) — SchedulesView reutiliza fetchRostersForWeekByEmployees/upsertWeeklyRoster/upsertBulkWeeklyRosters; TabRutas resultó casi puro reuso (5 de 7 sitios ya existían en data/pedidos.js, solo 2 funciones nuevas agregadas ahí mismo en vez de crear archivo aparte); verificado en vivo: /schedules (horario real La Popular), /pedidos?tab=rutas (Ruta #4 completada, conductor real). PR16 (v2.17.24) `customers.js` nuevo (1fn) + extensiones a `requests.js`(+1fn genérica reutilizada 4×)/`facturacion.js`(+2fn)/`pedidos.js`(+11fn) — 19 sitios WidgetAnnulmentRequest.jsx+TabGenerar.jsx+CrearRutaModal.jsx (7+6+6); verificado en vivo: /pedidos?tab=generar (981 productos sin stock, % abastecimiento real), /overview (widgets sales_invoices con datos reales); CrearRutaModal no se pudo ejercitar en vivo (botón gateado por permiso que la cuenta de prueba no tiene, igual que TabRutas en PR15) — verificado por sustitución 1:1. PR17 (v2.17.25) `promotions.js` nuevo (6fn) + `employeeSelfService.js` nuevo (9fn) — 18 sitios PromoModal.jsx+EmployeeRequestsView.jsx+EmployeeScheduleView.jsx (6+6+6); ambos EmployeeXView reutilizan updateApprovalRequest/insertApprovalRequest/updateVacationPlan ya existentes; verificado en vivo: /my-requests (solicitudes reales), /promociones→Nueva (sucursales reales en Paso 1); EmployeeScheduleView.jsx no tiene ruta activa en App.jsx (migrado igual, no verificable en vivo por eso). PR18 (v2.17.26) `compras.js`+`minmaxRequests.js`+`minmaxLabs.js` nuevos (5fn c/u) + `conteoInventario.js` extendido (+5fn) — 20 sitios ComprasView.jsx+WidgetMinMaxRequest.jsx+ConteoDetailView.jsx+LabsPanel.jsx (5+5+5+5); verificado en vivo: /compras (334 facturas reales), /minmax→panel laboratorios (5 ocultos, conteos reales), /overview→widget Ajuste Min/Max (búsqueda real), /conteo-inventario (conteo real de Bodega en progreso). PR19 (v2.17.27) `ventasPerdidas.js` nuevo (4fn) + extensiones a `pedidos.js`(+1fn)/`system.js`(+2fn) — 16 sitios RutaMapModal.jsx+RutaEnCursoCard.jsx+VentasPperdidasView.jsx+ShiftExceptionModal.jsx (4+4+4+4); RutaEnCursoCard.jsx resultó ser código muerto (el changelog v2.2.341 dice "eliminar RutaEnCursoCard" pero el archivo nunca se borró) y ShiftExceptionModal.jsx tampoco tiene botón trigger en EmployeeDetailView.jsx — mismo patrón que EmployeeScheduleView.jsx del PR17; ambos migrados igual y verificados por sustitución 1:1. Verificado en vivo: /pedidos?tab=rutas→"Ver mapa" (Ruta #4 real), /ventas-perdidas (productos reales buscados por clientes). PR20 (v2.17.28) `promotions.js`(+6fn)/`stockParams.js`(+3fn)/`laboratorios.js`(+2fn)/`employeeSelfService.js`(+3fn)/`system.js`(+2fn)/`permissions.js`(+2fn) + `auth.js` nuevo (1fn) — 21 sitios TabPromos+TabBonificaciones+TabSinVenta+TabLaboratorios+EmployeeProfileView+AuthContext+FormTurnos.jsx (3×7); AuthContext.jsx es el flujo de login crítico, preservado exacto; FormTurnos.jsx resultó inalcanzable (modal "manageShifts" sin trigger); verificado en vivo con 5 logins frescos + /promociones, /promociones?tab=bonificaciones, /productos?tab=sinventa, /laboratorios (355 reales), /profile (datos reales de Edwin Nuñez). PR21 (v2.17.29) `pushSubscriptions.js`+`audit.js` nuevos (2fn c/u) + `stockParams.js`(+6fn)/`pedidos.js`(+2fn)/`employees.js`(+3fn)/`requests.js`(+1fn) — 20 sitios ExpandedPanel+ItemSections+ApoioScanModal+ProductosView+MinMaxView+EmployeeDetailView+auditSlice+usePushSubscription+NuevoConteoModal+EmployeeFormModal (2×10); ProductosView.jsx y NuevoConteoModal.jsx no necesitaron ninguna función nueva (100% reuso); verificado en vivo: /auditview (1000 registros reales), /minmax→expandir fila (stock/compra/venta reales), /conteo-inventario→Nuevo Conteo, /dashboard→detalle+editar empleado real (Dolores Tejada Hernández); ApoioScanModal.jsx no ejercitable en vivo (requiere scan de carné), verificado 1:1. PR22 (v2.17.30) — CIERRE: último bloque de 19 archivos de 1 sitio cada uno (TabExpenses, PayrollView, NoAccessView, AccessDeniedView, pedidoPrint.js, useTimeClockEngine.js, PracticanteModal, AppLayout, FormWfmAnalytics, FormEditPayrollEntry, TabStaff, SyncHealthBanner, SidebarSyncStatus, TabHistorial, ConfigPanel, TabMinMaxRequests, EmployeeDocumentsView, WidgetSrsInventory, WidgetInventorySearch). 12 funciones nuevas + extensión de `fetchOvertimeBankRows` (agrega `subtype`, superset-reuse) repartidas en `data/payroll.js`, `data/branches.js`, `data/permissions.js`, `data/pedidos.js`, `data/practicantes.js`, `data/ventasPerdidas.js`(×2), `data/schedules.js`(×2), `data/inventory.js`(×2), `data/promotions.js`, `data/stockParams.js`, `data/minmaxRequests.js`. Reuso exacto: `insertApprovalRequestSilent` (useTimeClockEngine, mismo shape que WidgetAnnulmentRequest) y `fetchOwnApprovalRequests` (EmployeeDocumentsView, select superset ya usado por EmployeeRequestsView); `fetchRoleName` y `fetchInventorySyncLogRecent` son funciones nuevas compartidas por 2 archivos cada una (NoAccessView/AccessDeniedView; SyncHealthBanner/SidebarSyncStatus). Verificado en vivo: /promociones?tab=historial (promoción cerrada real "OMEGA 3 1000MG"), /branches/2→tab Gastos (renderiza sin error, estado vacío correcto), /minmax→panel Configuración abre con los 15 campos poblados desde `fetchStockConfigFull` (NO se guardó — escribir `stock_config` de producción requiere consentimiento explícito del usuario en el momento), /payroll y /overview cargan sin errores (SidebarSyncStatus + badge de ventas perdidas en AppLayout se ejercitan en cada carga de página visitada, sin errores nuevos en ninguna). Escaneo Python de todo `src/` confirma 0 sitios `supabase.from()` reales restantes fuera de `src/data/*.js` (quedan los 11 de `systemSlice.js`/`fetchBoot`, deliberadamente fuera de alcance — Bloque 6.B). Build + lint + 15 tests unitarios verdes. **CIERRA Bloque 6.A: 173 sitios corregidos migrados en 22 PRs, 0 restantes (excluyendo los 11 de `fetchBoot` reservados para 6.B).** |
| 6.B | Partir `fetchBoot` monolítico | Un `bootStatus` global bloquea todos los datos sensibles → race condition conocida + sobre-fetch | 🟢 **CERRADO (2026-07-15, v2.17.31).** `status` por slice, coexistiendo: `fetchBoot()` en `systemSlice.js` dividido en grupo liviano (holidays/branches/roles/shifts/announcements) y grupo empleados (rosters/employees_safe/events/documents/branch_assign), ambos arrancando en paralelo — mismo tiempo total de red, cero serialización nueva. `bootStatus` mantiene su significado exacto (sigue esperando a que TODO termine); `employeesStatus` (nuevo) resuelve en cuanto el grupo de empleados específicamente termina. Migrados los únicos 2 puntos de entrada al modal "Editar Empleado" en todo el repo: `StaffManagementView.jsx` (3 usos) y `EmployeeDetailView.jsx` (`handleEditProfile` — este NO tenía ninguna protección hasta ahora, mismo race condition alcanzable sin gate). Verificado en vivo con Playwright: login fresco → "Edición rápida" en `/dashboard` y "Editar" en `/dashboard/empleado/:id` abren el modal con DUI real poblado por ambos caminos. Sin cambios de esquema de BD. Build+lint+15 tests verdes. **Sobre-fetch investigado y cerrado sin cambios (2026-07-15)**: `employees_safe`/`employee_events`/`employee_documents` ya se escalaban a "mi sucursal"/"solo yo" para self-service desde antes de esta sesión. Lo único sin scope es `employee_rosters` (semana actual, hay FK a `employees` para escalarlo por sucursal) y `shifts` — pero con ~47 empleados activos totales, el rosters de una semana es como máximo 47 filas: el ahorro real de escalarlo es de unas ~40 filas descartadas en memoria, no un problema de performance. `shifts` es una tabla de referencia chica (~10-20 filas) de uso legítimo compañía-entera (Monitor/Auditoría/Kiosco/Admin) — escalarla arriesga esos consumidores por un ahorro casi nulo. Más "status por slice" (branches/roles/shifts propios) no tiene ningún consumidor real hoy. Decisión informada del usuario: no vale el riesgo/complejidad para el tamaño actual de la empresa — revisar si la plantilla crece significativamente |
| 6.C | Dividir `TabMinMax.jsx`/`TabPedidos.jsx` (~3,900 líneas c/u) | Mini-apps en un archivo | ✅ **CERRADO por completo 2026-07-16 (segunda mitad — hook de estado/fetch).** Primera mitad (extracción mecánica JSX→sub-componentes) cerrada 2026-07-15 (v2.17.2→v2.17.8, 7 PRs): `TabMinMax.jsx` 3947→2584 líneas (-34%, 10 sub-componentes); `TabPedidos.jsx` 3943→2037 líneas (-48%, 10 sub-componentes: `EmpChip`/`StagePill`/`SucPill`/animaciones de stage, `PauseModal`/`AnularModal`/`ApoioScanModal`, `ItemSection`+`ItemSections`, `LifecycleTimeline`+`PauseBadge`, `DifSection`, `PostCompletionSection`, `ReceptionActions`, `FilterPill`). **Hallazgo de esa mitad, no corregido:** `StageAnim`/`EmpChip`/`StagePill` (TabPedidos) no tienen ningún caller — código muerto real, extraído igual, decisión de borrarlo queda aparte. Segunda mitad (2026-07-16, v2.17.49→v2.17.50, 2 PRs): extrae el hook de estado/fetch de cada componente principal — `useMinMaxData.js` (50 useState/useRef + 9 effects + 24 handlers + 6 useMemo, `TabMinMax.jsx` 2565→1666 líneas) y `usePedidosData.js` (43 useState/useRef + 9 effects + 28 handlers + 7 useMemo, `TabPedidos.jsx` 1971→918 líneas), ambos en `src/views/{productos,pedidos}/tab{minmax,pedidos}/`. Extracción mecánica: mismos nombres, misma lógica; el componente queda reducido a `hook call → destructure → mismo JSX de siempre`. **Única desviación no textual** (PR1, TabMinMax): 2 fetchers de tooltip de Bodega duplicados inline en el JSX se consolidaron en `openBodegaTooltip`/`closeBodegaTooltip`; `_openBodegaEdit` se movió tal cual como `openBodegaEdit(row, field, isBodega)`. `STAT_CFGS`/`VISIBLE_STAT_KEYS` (antes locales a TabMinMax.jsx) se movieron a `tabminmax/constants.js` por ser necesitados tanto por el hook como por el JSX. TabPedidos (PR2) no tuvo ninguna desviación — 100% mecánico, incluidos 3 handlers async que ya vivían embebidos en su JSX desde antes (onConfirmed de RecepcionModal, botones Iniciar/Completar ruta), dejados intactos. Ambos PRs verificados con build+lint+15 tests + Playwright en vivo contra datos reales de producción (`/minmax` con La Popular: cost cards, matriz ABC×XYZ, fila expandida; `/pedidos?tab=pedidos` con cards reales, timeline de lifecycle, fotos de apoyo) sin ejercitar ninguna escritura real (ni editar MIN/MAX ni acciones de lifecycle de pedidos) |
...
| 7A.2 | `auto-copy-weekly-roster`: bugs `target_type:'ALL'` + `status='ACTIVE'` (encadenados, corregir JUNTOS) | ✅ Aplicado y desplegado 2026-07-15 (v2.17.34). Causa raíz confirmada: `status='ACTIVE'` (×2) nunca matcheaba (`employees.status` real es `'ACTIVO'`), por eso `recipientIds` quedaba vacío y `target_type` caía siempre en `'ALL'`. Corregidos ambos literales, edge function redesplegada. Impacto histórico = 0 (nunca se disparó); bug quedaba latente hacia adelante |
| 7A.3 | `RecepcionModal:439 handleTodoOk` ("marcar todo OK") sin caller — posible botón faltante | ✅ Confirmado y aplicado 2026-07-15 (v2.17.35). No era duplicado de `handleConfirmarTodo` (esa confirma TODAS las cajas accesibles desde el picker; `handleTodoOk` es el atajo para la caja/pedido ya abierto en la pantalla de items). Botón "Todo OK" agregado en el footer, junto a "Confirmar Caja X" |
...
| 7A.5 | `TabPedidos.jsx` `ReceptionActions`: sin bloque "Confirmado llegada" (existe el equivalente `empChip(erpEmp)` para el paso "Sistema de Ventas" pero no para "Llegada") | ✅ Aplicado 2026-07-15 (v2.17.36). Tarjeta "Llegada confirmada" agregada en `ReceptionActions.jsx`, mismo patrón visual que el bloque de `erpOk` (empChip + acento emerald), label de tipo reutilizando el vocabulario de `LLEGADA_TIPO_INFO` de `PostCompletionSection.jsx` |
| 7A.6 | `TabMinMax.jsx`: `hideFiltered` — acción bulk "ocultar todo lo filtrado" completa (RPC + audit log `MINMAX_HIDE_FILTERED`) sin botón en UI | ✅ Aplicado 2026-07-15 (v2.17.37). Botón en la toolbar de drafts junto a "Descartar"/"Publicar", con `ConfirmModal` (mismo patrón que "Descartar": isDestructive, isProcessing, mensaje que aclara que es reversible desde el filtro de ocultos) |
| 7A.7 | `TabMinMax.jsx`: celda "Despacho" calcula MIN/MAX ya redondeado por la regla de despacho (`dispMin`/`dispMax`/`applyRule`) pero el JSX solo muestra el nombre de la regla, nunca el resultado numérico | ✅ Aplicado 2026-07-15 (v2.17.38). Línea nueva bajo el badge de presentación, solo cuando `hasRule && hasPres`, display puramente aditivo sin matemática nueva. **Hallazgo nuevo durante la verificación**: nunca se pudo ver en vivo porque `get_stock_analysis` lee `dr.solo_cajas/multiplo/blister/multiplo_unidades` de `dispatch_rules` — columnas vacías en las 845 filas de la tabla (0/0/0/0). La regla real vive en `dispatch_rules.dispatch_multiplo` (393 filas con valor >1), columna que la RPC nunca lee — `hasRule` es siempre `false` hoy en toda la vista. Desalineamiento previo RPC↔esquema, no introducido por este PR. Fuera de alcance (gap de UI, no de pipeline) — **queda como ítem nuevo para una sesión futura**: corregir `get_stock_analysis` para leer las columnas `dispatch_*` reales |
...
| 7B.7 | Vista de "objetos huérfanos" para Sistema | Medio (1-2sem) | Medio — mantenimiento preventivo | ✅ **Aplicado 2026-07-16 (v2.17.43).** `OrphanObjectsView.jsx` en menú Sistema (moduleKey `orphan_objects`, permiso ya otorgado a role_id=13 igual que `sync_health`) — tablero sobre `orphan_objects_registry` (tabla nueva), sembrada con 8 casos reales re-verificados con grep antes de sembrar (no copiados ciego de notas viejas — 2 correcciones encontradas: `EmpChip`/`StagePill` SÍ están importados en `TabPedidos.jsx`, solo sin uso en el JSX, no "sin ningún caller" como decía la nota original). No es detección automática — es un registro manual con cambio de estado (candidato/confirmado/falso positivo/resuelto). Verificado en vivo: 8 filas con notas y selector de estado editable |
| 7B.6 | Historial de precios en catálogo | Medio (1sem) | Medio | ✅ **Aplicado 2026-07-16 (v2.17.44).** Sección nueva "Historial de precios" en `ExpandedProductRow` (`TabCatalogo.jsx`) sobre `product_precios_history` — ya tenía RLS abierta a `authenticated`, no hizo falta RPC. Dedupe en la UI de snapshots idénticos consecutivos (la tabla acumula 1 fila por corrida de sync aunque no cambie el precio, write-churn preexistente fuera de alcance). Coexiste con "Cambios en el producto" (changelog campo-a-campo), no lo reemplaza. Verificado en vivo con producto real (26 filas de historial) |
| 7B.2 | Tracker de corto vence (reglas de Bodega ya documentadas) | Medio (2-3sem) | Alto — reduce mermas | ✅ **Aplicado 2026-07-16 (v2.17.46) — alcance de solo lectura, ver detalle.** Decisión del usuario: `proveedores.vineta` (columna nueva) cruzado con `product_precios.vineta` para desambiguar labs con varios proveedores. Nuevo RPC `get_product_vencimiento_policy` (match por viñeta → fallback proveedor único → null). `TabPoliticaVencimiento.jsx`: input "Viñeta" nuevo. `ExpandedPanel.jsx` (TabMinMax): proveedor resuelto + punto rojo COFARSAL a nivel de producto (regla b) + fecha límite de envío a bodega calculada con la regla (g) real (vence diciembre, política 2 meses → límite última semana de SEPTIEMBRE, no octubre — el envío tarda ~1 mes en llegar, regla h) + aviso ND "reportar a jefe 6-7 meses antes" (regla f). Verificado con datos reales de prod en transacciones ROLLBACK: caso único y caso ambiguo (lab con 5 proveedores) ambos resuelven correctamente. **Fuera de alcance, decisión de producto aparte**: reglas (d) descuento a responsable y (e) escalación a jefe de Bodega — son un workflow de aprobación/atribución nuevo, no una vista de solo lectura |
| 7B.8 | Modo offline del kiosco | Alto (3-4sem) | Alto para sucursales con mala conexión | ✅ **Fases A+B aplicadas 2026-07-16 (v2.17.45).** Fase A: `validateKioskToken`/`useKioskDevice.js` ahora distinguen error de red de revocación real (antes ambos mostraban "KIOSCO NO AUTORIZADO" por igual) — ventana de gracia 15min vía `lastVerifiedAt`. Fase B: `finalizePunch` ya no es fire-and-forget (podía pintar "éxito" sin que el insert llegara a la BD) — ahora espera el resultado real y encola en `localStorage` (`src/utils/attendanceQueue.js`) si falla, reintentando solo (evento `online` + poll 30s) vía el mismo `registerAttendance`. `OfflineBanner` montado en `TimeClockView.jsx` (antes solo en `AppLayout.jsx`). **Fase C descartada, no diferida**: `/kiosk` está gateado por `!isMobileOrApp()` en `App.jsx` — nunca corre en la app nativa Capacitor ni en tablets/celulares, solo en navegador de escritorio normal, así que `@capacitor/network` no aplicaría a esta pantalla. Fase D (cache de datos) queda diferida, sin problema real que resuelva hoy. Verificado en vivo con Playwright (`context.setOffline`) + 15 tests unitarios verdes |
