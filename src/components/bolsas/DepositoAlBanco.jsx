...
export default function DepositoAlBanco({ abierto, bolsas, personas, onClose, onHecho }) {
    ...
    const cerrar = useCallback(async () => {
        ...
        onHecho?.(data);
        onClose?.();
    }, [bolsas, nMonto, nAporte, aporteNota, recibidoPor, nota, showToast, onHecho, onClose]);

    /* La lista de a quién se le entrega el remanente sale del maestro de
     * personal, no de un texto escrito acá — un rótulo escrito a mano se
     * desincroniza del registro en cuanto alguien cambia de nombre. */
    ...
    return (
        <LiquidModal open={!!abierto} onClose={guardando ? undefined : onClose}
            ...
            <LiquidModal.Body className="space-y-4">
                ...
                {/* Sólo si hay remanente. Quien cierra queda como quien lo
                    entregó —es lo único que el portal puede afirmar—; a quién se
                    lo dio es un dato, y por eso no se le pide que confirme. */}
                {remanente >= 0.01 && !noAlcanza && (
                    <div className="space-y-1.5">
                        <label className="text-caption font-bold text-content-2">
                            A quién se le entrega el remanente
                            <span className="font-normal text-content-3"> (opcional)</span>
                        </label>
                        <LiquidSelect
                            value={recibidoPor} onChange={setRecibidoPor}
                            options={gente} placeholder="Elige a quién…"
                            icon={Banknote} clearable
                        />
                        <p className="text-caption text-content-3">
