 ...
 *
 * Exports:
 *   DataTable  — contenedor con toolbar, thead y tbody tematizados
...
function useTokens() {
  ...
  return {
    ...
    theadBg: isCompat
      ...
      : 'bg-[#0052CC]/5 backdrop-blur-xl',
    theadBorderRow: isCompat
      ? 'border-b border-slate-200'
      : isAurora
      ? 'border-b border-white/[0.08]'
      : 'border-b border-[#0052CC]/10',
    // th text
    thText: isAurora ? 'text-white/50' : 'text-slate-500',
    thHover: isCompat
      ? 'hover:bg-slate-100 hover:text-slate-700'
      : isAurora
      ? 'hover:bg-white/[0.07] hover:text-white/80'
      : 'hover:bg-white/25 hover:text-slate-700',
    // toolbar separator (above thead, inside the card)
    ...
    rowHover: isCompat
      ...
      : isAurora
      ? 'hover:bg-white/[0.05]'
      : 'hover:bg-[#0052CC]/[0.035]',
    // cell
    cellText: isAurora ? 'text-white/80' : 'text-slate-700',
    // skeleton
    ...
    emptyIcon: isAurora ? 'text-white/20' : 'text-slate-300',
  };
}
...
export function DataTable({
  ...
  return (
    <TableCtx.Provider value={tk}>
      ...
      {/* ── Toolbar (conteo, filtros de fila, acciones bulk) ─────────────── */}
      {toolbar && (
        <div className={`px-4 md:px-8 py-3.5 flex items-center justify-between gap-3 shrink-0 ${tk.toolbarBorder}`}>
          {toolbar}
        </div>
      )}
