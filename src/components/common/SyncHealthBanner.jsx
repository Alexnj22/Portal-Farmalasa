...
export default function SyncHealthBanner() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      {/* Summary strip */}
      <div className={`flex items-center gap-2 px-4 py-2.5 border-b border-divider shrink-0 ${
        hasErrors || anyStale ? 'bg-danger/10' : allGood ? 'bg-success/10' : ''
      }`}>
        {hasErrors || anyStale
      ...
      {/* Branch list */}
      <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] divide-y divide-divider p-1.5 space-y-0.5">
        {branches.length === 0
          ? [1,2,3,4,5,6,7].map(i => (
