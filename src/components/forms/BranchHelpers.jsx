...
export const Switch = ({ on, onToggle }) => (
    <button
        ...
        className={`relative inline-flex items-center flex-shrink-0 w-12 h-7 rounded-full transition-all duration-300 ease-[cubic-bezier(0.23,1,0.32,1)] shadow-[inset_0_2px_4px_rgba(0,0,0,0.1)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand/50 active:scale-[0.97] transform-gpu ${
            on ? "bg-brand border border-brand-hover" : "bg-surface-card-hover/80 border border-slate-300"
        }`}
        aria-pressed={on}
    >
        <span
    ...
    return (
        <div className="relative group flex items-center w-full">
            ...
            <input
                ...
                className={`w-full py-3.5 bg-surface-card border border-border-card rounded-[1.25rem] text-[16px] font-bold text-content-2 placeholder-slate-400 outline-none transition-all duration-300 shadow-[inset_0_2px_10px_rgba(255,255,255,0.5)] focus:bg-white focus:border-brand/30 focus:shadow-[0_0_0_4px_rgba(0,82,204,0.15)] ${Icon ? 'pl-14 pr-4' : 'px-4'} ${className}`}
