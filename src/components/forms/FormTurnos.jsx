...
const FormTurnos = ({ branches }) => {
    ...
    const TurnoCard = useCallback(({ shift }) => {
        ...
        return (
            <div data-surface="card" data-tono={isCurrentlyEditing ? 'brand' : undefined}
                            className={`transition-all duration-[var(--dur-slow)] relative overflow-hidden group ${isBeingDeleted ? 'bg-danger/10' : ''}`}>
