 ...
 * días: módulo 12, campana 8, mi caja 3, Inicio 3—, así que ponerlo en una sola
 * dejaría fuera el 75% de las confirmaciones, y una quinta pantalla nueva se
 * olvidaría de pedirlo.
 *
 * Por eso `resolver` **no devuelve hasta que la entrega se resolvió**: quien
 * llama sigue haciendo su trabajo de después (imprimir el papel, cerrar el
 * turno) exactamente igual que antes, sin saber que hubo un diálogo en el
 * medio. Un `false` temprano habría cortado esa cadena en silencio.
 *
 * Quien lo usa tiene que pintar `dialogoDeEntrega` en su árbol. Sin eso el
...
export default function useResolverCorte({ nombreSala = {}, origen = 'modulo' } = {}) {
     ...
     *
     * `esperando` guarda el `resolve` de la promesa que `resolver` devolvió:
     * quien confirmó sigue detenido en su `await` hasta que la entrega se
     * firma, se saltea o se cierra el diálogo. Sin eso, el llamador seguiría
     * de largo e imprimiría el papel de un corte que todavía no se escribió.
     */
    const esperando = useRef(null);
    const [pidiendo, setPidiendo] = useState(null);

    const terminar = useCallback((ok) => {
        setPidiendo(null);
        esperando.current?.(ok);
    ...
    const resolver = useCallback(async (corte, estado, opts = {}) => {
        ...
        // cambió la caja de manos, así que no hay a quién entregársela.
        if (estado !== 'CONFIRMADO') return escribir(corte, estado, opts);

        /* El ÚLTIMO corte del día no pregunta nada: no hay a quién entregarle
         * («si cierra a las 7 y confirma un corte a las 7 ya es el último, no
         * debe pedir ahí nada» — usuario, 3-sep). Lo contesta el servidor con
         * la MISMA función con la que después marca el corte; calcularlo acá
         * abriría la puerta a que la pantalla no pida la firma y el servidor
         * anote que nadie recibió.
         *
    ...
    const dialogoDeEntrega = pidiendo ? (
        <EntregaDeCaja
            corte={pidiendo.corte}
            sala={nombreSala[pidiendo.corte.branch_id] || ''}
            ocupado={ocupadoId === pidiendo.corte.id}
            onEntregar={async ({ persona, vale }) => {
                const ok = await escribir(pidiendo.corte, pidiendo.estado, {
                    ...pidiendo.opts, recibidoPor: persona?.id, vale,
                });
                // Si falló, el diálogo SIGUE abierto: el vale no se gastó —la
                // transacción se revirtió entera— y el aviso ya dice qué pasó,
                // así que se puede firmar de nuevo o salir por «no hay quien
                // reciba». Cerrarlo obligaría a rehacer el conteo entero.
                if (ok) terminar(true);
            }}
            onSinEntrega={async (motivo) => {
                const ok = await escribir(pidiendo.corte, pidiendo.estado, {
                    ...pidiendo.opts, sinEntregaMotivo: motivo,
                });
                if (ok) terminar(true);
            }}
