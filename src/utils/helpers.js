...
// ✅ NUEVO: Normalizador para búsquedas (sin tildes, minúsculas)
...
export const getEffectiveBranchId = (emp) => {
    const t = toLocalISO(new Date()); // FIX: Usa hora local
    const s = emp.history?.find(h => h.type === 'SUPPORT' && h.date <= t && h.endDate >= t);
    return s ? parseInt(s.targetBranchId) : emp.branchId;
};
...
export const getTodayScheduleConfig = (employee, shifts, specificDateObj = new Date()) => {
    // FIX: Usar toLocalISO para asegurar que la fecha string coincida con el día JS
    const dateStr = toLocalISO(specificDateObj); 
    const exception = (employee.exceptions || []).find(ex => ex.date === dateStr);
    ...
    if (!dayConfig || dayConfig.isOffDay || !dayConfig.shiftId) {
        return { isOffDay: true, shift: null };
    }

    const shift = shifts.find(s => s.id.toString() === dayConfig.shiftId.toString());
    
    return {
        isOffDay: false,
        shift: shift,
        lunchTime: dayConfig.lunchTime,
        lactationTime: dayConfig.lactationTime
    };
};

export const getTodayAttendanceStatus = (emp, shifts) => { 
    const todayStr = toLocalISO(new Date()); // FIX: Usa hora local
