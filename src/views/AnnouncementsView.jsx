...
import {
  ...
  Archive, Eye, AlertCircle, PartyPopper, ChevronLeft, ChevronRight, Loader2, Clock, Flame, Edit3, Save
} from 'lucide-react';
...
const AnnouncementCard = memo(({ ann, onArchive, onDelete, onViewDetail, onEdit, isEditingThis }) => {
  const renderBadge = () => {
    switch (ann.badgeType) {
      case 'GLOBAL':
        return <span className="flex items-center gap-1.5 text-[#007AFF] bg-[#007AFF]/10 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-widest border border-[#007AFF]/20"><Globe size={12} strokeWidth={2} /> {ann.badgeText}</span>;
      case 'BRANCH':
        return <span className="flex items-center gap-1.5 text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-widest border border-emerald-200/50"><Building2 size={12} strokeWidth={2} /> {ann.badgeText}</span>;
      case 'ROLE':
        return <span className="flex items-center gap-1.5 text-purple-600 bg-purple-50 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-widest border border-purple-200/50"><Users size={12} strokeWidth={2} /> {ann.badgeText}</span>;
      case 'EMPLOYEE':
        return <span className="flex items-center gap-1.5 text-orange-600 bg-orange-50 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-widest border border-orange-200/50"><User size={12} strokeWidth={2} /> {ann.badgeText}</span>;
      default: return null;
    }
  };
  ...
  return (
    <div
      className={`p-6 rounded-[2.5rem] border flex flex-col gap-4 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] group relative transform-gpu ${
        // CLEAN UI: Mantenemos shake y borde ámbar nítido sin glow neón
        isEditingThis
    ...
    >
      <div className={`absolute top-5 right-5 flex items-center gap-2 transition-opacity duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${isEditingThis ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
        {!ann.isCompleted && (
          <>
            {/* 🚨 SOLO MUESTRA EDITAR SI TIENE 0 LECTURAS */}
            {ann.readIds.length === 0 && (
              <button
                onClick={() => onEdit(ann)}
                className={`p-2.5 rounded-full transition-all duration-300 active:scale-95 shadow-sm border ${isEditingThis
                  ? 'bg-amber-100 text-amber-600 border-amber-300 hover:bg-amber-500 hover:text-white'
                  : 'bg-white/80 text-amber-500 border-amber-100 hover:bg-amber-50 hover:text-amber-600 hover:border-amber-200 hover:-translate-y-0.5 hover:shadow-md'
                  }`}
                title="Editar aviso (Solo antes de ser leído)"
              >
        ...
        {/* 🚨 SOLO MUESTRA ELIMINAR SI TIENE 0 LECTURAS */}
        {ann.readIds.length === 0 && (
          <button
            ...
            title="Eliminar aviso (Solo antes de ser leído)"
          >
      ...
      <div className="mt-2 space-y-2">
        <div className={`flex justify-between items-end text-[10px] font-bold uppercase tracking-widest ${ann.priority === 'URGENT' ? 'text-red-400' : 'text-slate-500'}`}>
          <span>Progreso de Lectura</span>
          <span className={ann.priority === 'URGENT' && ann.readPercentage < 100 ? 'text-red-500' : ann.readPercentage === 100 ? 'text-emerald-500' : 'text-[#007AFF]'}>
            {ann.readPercentage}%
          </span>
        </div>
        <div className={`w-full rounded-full h-2.5 overflow-hidden border ${ann.priority === 'URGENT' ? 'bg-red-50/50 border-red-200/50' : 'bg-white/50 border-white/60'}`}>
          <div
            className={`h-full rounded-full transition-all duration-1000 ease-[cubic-bezier(0.23,1,0.32,1)] shadow-sm ${ann.priority === 'URGENT' && ann.readPercentage < 100 ? 'bg-red-500' : ann.readPercentage === 100 ? 'bg-emerald-500' : 'bg-[#007AFF]'}`}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pt-4 border-t border-white/60 gap-4">
        ...
        >
          ...
          Ver Detalle ({ann.readIds.length}/{ann.totalExpected})
        </button>
...
const AnnouncementsView = ({ openModal }) => {
  ...
  // 🚨 REQUERIMIENTO 2: Referencia al input para darle focus automático
  ...
  // 🚨 REQUERIMIENTO 1: Cerrar buscador y cancelar edición al presionar ESCAPE
  useEffect(() => {
  ...
  const handlePublish = async (e) => {
    ...
    if (!title.trim() || !message.trim()) {
      setError('¡Ey! El aviso no tiene título o mensaje. Llena los campos.');
      return;
    }
    ...
    try {
      if (editingAnnId) {
        ...
        const getReadableTarget = (type, val) => {
          if (type === 'GLOBAL') return 'Toda la Empresa';
          if (type === 'ROLE') return val;
          if (type === 'BRANCH') return branchNameById.get(String(val)) || val;
          if (type === 'EMPLOYEE') {
            const ids = Array.isArray(val) ? val : [val];
            return ids.map(id => employeesById.get(String(id))?.name || 'Empleado Desconocido');
          }
          return val;
        };
        ...
        const auditDetails = {
          previous: {
            title: originalAnn.title,
            message: originalAnn.message,
            priority: originalAnn.priority,
            targetType: originalAnn.targetType,
            targetValue: getReadableTarget(originalAnn.targetType, originalAnn.targetValue)
          },
          new: {
            title: updatePayload.title,
            message: updatePayload.message,
            priority: updatePayload.priority,
            targetType: updatePayload.targetType,
            targetValue: getReadableTarget(updatePayload.targetType, updatePayload.targetValue)
          }
        };

        await updateAnnouncement(editingAnnId, updatePayload, auditDetails);

        useToastStore.getState().showToast(
          'Aviso Actualizado',
          'Las pantallas reflejarán los cambios inmediatamente.',
          'success'
        );
      } else {
        ...
        useToastStore.getState().showToast(
          '¡Boom! Enviado',
          'El aviso ya está en las pantallas de tu equipo. 🚀',
          'success'
        );
      }
      ...
      setError(`Hubo un error al ${editingAnnId ? 'actualizar' : 'publicar'} el aviso. Intenta de nuevo.`);
      console.error(err);
    } finally {
  ...
  // 🚨 FUNCIONES DE ARCHIVADO CON CONFIRMACIÓN
  const handleArchiveCallback = useCallback((id) => {
    setError('');
    setArchiveDialog({ isOpen: true, annId: id });
  }, []);

  const executeArchive = async () => {
    if (!archiveDialog.annId) return;
    setIsSubmitting(true); // 🚨 Activa la animación del modal
    try {
      await archiveAnnouncement(archiveDialog.annId);
      setArchiveDialog({ isOpen: false, annId: null });
      // 🚨 Dispara el Toast de éxito
      useToastStore.getState().showToast('Aviso Archivado', 'El aviso ya no será visible en los kioscos.', 'success');
    } catch (err) {
      setError('No se pudo archivar el aviso.');
      setArchiveDialog({ isOpen: false, annId: null });
    } finally {
      setIsSubmitting(false); // 🚨 Apaga la animación
    }
  };

  const handleDeleteCallback = useCallback((ann) => {
    setError('');
    // 🚨 PROTECCIÓN DE ELIMINADO RESTAURADA
    if (ann.readIds?.length > 0) {
      setAlertDialog({
        isOpen: true,
        title: 'Operación Bloqueada',
        message: 'Alguien ya leyó este aviso. Por seguridad no puedes eliminarlo, archívalo.'
      });
      return;
    }
  ...
  const processedAnnouncements = useMemo(() => {
    return (announcements || []).map((ann) => {
      ...
      return {
        ...
        badgeText,
        badgeType: ann.targetType
      };
    });
  }, [announcements, employees, getTargetAudience, branchNameById]);

  // 🚨 REQUERIMIENTO 3 y 4: Buscador ultra-potenciado (Sin restricciones de tipo)
  const currentList = useMemo(() => {
    const baseList = listTab === 'ACTIVE'
      ? processedAnnouncements.filter((a) => !a.isCompleted)
      : processedAnnouncements.filter((a) => a.isCompleted);
    ...
    return baseList.filter(a => {
      // 1. Búsqueda básica: Título, mensaje o etiqueta del badge
      const matchesBasic =
        a.title.toLowerCase().includes(query) ||
        a.message.toLowerCase().includes(query) ||
        (a.badgeText && a.badgeText.toLowerCase().includes(query));

      if (matchesBasic) return true;

      // 2. Búsqueda profunda UNIVERSAL: Revisa si la persona escrita en el buscador 
      // pertenece a la audiencia de este aviso (Aplica para GLOBAL, SUCURSAL, ROL y PERSONAL).
      if (Array.isArray(a.audience)) {
        return a.audience.some(emp => emp.name && emp.name.toLowerCase().includes(query));
      }
  ...
  // ==========================================================
  // 🎨 PÍLDORA DE FILTROS (MÁSTER LIQUIDGLASS)
  // ==========================================================
  const renderFiltersContent = () => (
    ...
    >
      <div className={`flex items-center h-full shrink-0 transform-gpu overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] origin-left ${isSearchMode ? "max-w-[800px] opacity-100 px-4 md:px-5 gap-3" : "max-w-0 opacity-0 pointer-events-none px-0 gap-0 m-0 border-transparent"}`}>
        ...
        />
        {announcementSearch && (
          <button onClick={() => setAnnouncementSearch('')} className="p-1 text-slate-400 hover:text-red-500 transition-all hover:scale-110 hover:-translate-y-0.5 active:scale-95 transform-gpu shrink-0">
            <X size={16} strokeWidth={2.5} />
          </button>
        )}
        <button onClick={() => setIsSearchMode(false)} className="w-10 h-10 md:w-11 md:h-11 rounded-full bg-transparent hover:bg-white text-slate-500 flex items-center justify-center shrink-0 transition-all duration-300 hover:shadow-md hover:text-[#007AFF] hover:-translate-y-0.5 ml-2">
      ...
      <div className={`flex items-center h-full shrink-0 transform-gpu overflow-visible transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] origin-right ${isSearchMode ? "max-w-0 opacity-0 pointer-events-none pl-0 pr-0 gap-0 m-0" : "max-w-[800px] opacity-100 pl-2 pr-2 md:pr-3 gap-2 md:gap-3"}`}>
        <div className="flex items-center min-w-0 gap-1 md:gap-2">
          <button
            ...
            className={`px-4 md:px-6 h-9 md:h-10 rounded-full text-[10px] md:text-[11px] font-black uppercase tracking-widest transition-all duration-300 transform-gpu whitespace-nowrap border shrink-0 ${listTab === 'ACTIVE'
          ...
          <button
            ...
            className={`px-4 md:px-6 h-9 md:h-10 rounded-full text-[10px] md:text-[11px] font-black uppercase tracking-widest transition-all duration-300 transform-gpu whitespace-nowrap border shrink-0 ${listTab === 'ARCHIVED'
              ? 'bg-white text-slate-800 border-white shadow-md scale-[1.02]'
              : 'bg-transparent text-slate-500 border-transparent hover:bg-white hover:text-slate-800 hover:-translate-y-0.5 hover:shadow-md hover:border-white/90'
              }`}
          >
        ...
        {/* 🚨 REQUERIMIENTO 2: Autofocus en input */}
        <button
          onClick={() => {
  ...
  return (
    <>
      <style>{`
          /* ANIMACIÓN DE SHAKE PARA LA TARJETA EN EDICIÓN CORREGIDA */
      ...
      <ConfirmModal
        ...
        onClose={() => setConfirmDialog({ isOpen: false, annId: null })}
        onConfirm={executeDelete}
        title="¿Eliminar este aviso?"
        message="Esta acción borrará el aviso para siempre. No podrás recuperarlo ni ver las estadísticas."
      ...
      <ConfirmModal
        ...
        onClose={() => setArchiveDialog({ isOpen: false, annId: null })}
        onConfirm={executeArchive}
        title="¿Archivar Aviso?"
        message="El aviso se moverá a la pestaña de Archivo y dejará de mostrarse en los Kioscos. ¿Continuar?"
        confirmText="Sí, Archivar"
        isProcessing={isSubmitting} 
        isDestructive={false}       
      />

      {/* Modal de Alerta Estricta */}
      <AlertModal
      ...
      >
        ...
        <div className="flex flex-col lg:flex-row items-start gap-6 md:gap-8 px-2 md:px-0 w-full h-full lg:h-[calc(100vh-230px)]">
          ...
          {/* ✅ COLUMNA IZQUIERDA (Píldora de Redacción/Edición) */}
          <div className="w-full lg:w-[400px] xl:w-[450px] shrink-0 h-auto group/panel transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] z-10 transform-gpu">
            <div className={`bg-white/40 backdrop-blur-[30px] backdrop-saturate-[180%] border p-6 md:p-8 rounded-[2.5rem] transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover/panel:-translate-y-[2px] relative overflow-hidden ${editingAnnId
              ...
              <form onSubmit={handlePublish} className="space-y-5 relative z-10">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      ...
                      className={`flex items-center justify-center gap-2 py-3 rounded-xl border font-bold text-xs transition-all duration-300 ${priority === 'NORMAL' ? 'bg-white/80 border-[#007AFF]/30 text-[#007AFF] shadow-[0_2px_10px_rgba(0,122,255,0.2)]' : 'bg-white/40 border-white/60 text-slate-500 hover:bg-white/80 hover:shadow-sm hover:-translate-y-0.5'}`}
                    >
                      <Megaphone size={14} /> Normal
                    </button>
                    <button
                      type="button"
                      onClick={() => setPriority('URGENT')}
                      className={`flex items-center justify-center gap-2 py-3 rounded-xl border font-bold text-xs transition-all duration-300 ${priority === 'URGENT' ? 'bg-red-50 border-red-300 text-red-600 shadow-[0_2px_10px_rgba(239,68,68,0.2)]' : 'bg-white/40 border-white/60 text-slate-500 hover:bg-white/80 hover:shadow-sm hover:-translate-y-0.5'}`}
                    >
                      <Flame size={14} className={priority === 'URGENT' ? 'animate-pulse' : ''} /> Urgente
                    </button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className={`w-full py-3.5 px-4 bg-white/50 border border-white/60 focus:bg-white focus:border-[#007AFF]/30 focus:shadow-[0_0_0_4px_rgba(0,122,255,0.15)] rounded-2xl text-[13px] outline-none font-medium text-slate-700 resize-none h-28 transition-all duration-300 placeholder-slate-400 placeholder:font-normal placeholder:tracking-normal leading-relaxed custom-scrollbar ${error && !message.trim() ? 'border-amber-300 focus:ring-amber-500/20' : ''}`}
                    value={message}
                ...
                <div className="pt-3 border-t border-white/50">
                    ...
                    {targetTypes.map((type) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex-1 h-9 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all duration-300 transform-gpu whitespace-nowrap border ${isActive
                            ? 'bg-white text-[#007AFF] border-white shadow-sm scale-[1.02]'
                            : 'bg-transparent text-slate-500 border-transparent hover:bg-white hover:text-slate-800 hover:-translate-y-0.5 hover:shadow-sm hover:border-white/90'
                            }`}
                        >
                          {type.label}
                        </button>
                  ...
                  {targetType === 'BRANCH' && (
                    <select
                      className={`w-full py-3.5 px-4 bg-white/50 border border-white/60 focus:bg-white focus:border-[#007AFF]/30 focus:shadow-[0_0_0_4px_rgba(0,122,255,0.15)] rounded-2xl text-[13px] outline-none font-bold text-slate-700 transition-all duration-300 appearance-none cursor-pointer ${error && !targetValue ? 'border-amber-300' : ''}`}
                      value={targetValue}
                  ...
                  {targetType === 'EMPLOYEE' && (
                    <div className="space-y-3">
                      {selectedEmployees.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 p-3 bg-white/60 rounded-[1rem] border border-white/80 shadow-sm transition-all duration-300">
                          {selectedEmployees.map((id) => {
                            const emp = employeesById.get(String(id));
                            return (
                              <div
                                key={id}
                                className="flex items-center gap-1.5 bg-[#007AFF]/10 text-[#007AFF] px-2.5 py-1.5 rounded-lg text-[11px] font-bold border border-[#007AFF]/20 transition-all hover:scale-105"
                              >
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          className={`w-full pl-11 pr-4 py-3.5 bg-white/50 border border-white/60 focus:bg-white focus:border-[#007AFF]/30 focus:shadow-[0_0_0_4px_rgba(0,122,255,0.15)] rounded-2xl text-[13px] outline-none font-bold text-slate-700 transition-all duration-300 placeholder-slate-400 placeholder:font-normal placeholder:tracking-normal ${error && selectedEmployees.length === 0 ? 'border-amber-300' : ''}`}
                          value={empSearch}
                        ...
                        {empSearch.trim() && (
                          <div className="absolute z-20 w-full mt-2 bg-white/90 backdrop-blur-xl border border-white/90 rounded-[1.25rem] shadow-[0_12px_40px_rgba(0,0,0,0.12)] max-h-60 overflow-y-auto p-1 animate-in fade-in zoom-in-95 duration-200">
                            {filteredEmployeeSearch.length ? (
                              filteredEmployeeSearch.map((emp) => (
                                <button
                                  ...
                                  className="w-full p-3 hover:bg-[#007AFF]/10 text-left flex items-center justify-between rounded-xl mx-0.5 transition-colors"
                                >
                                  <p className="text-[13px] font-bold text-slate-700">{emp.name}</p>
                                  <Plus size={14} className="text-[#007AFF]" />
                                </button>
                              ))
                            ) : (
                              <div className="p-3 text-[12px] text-slate-400 font-bold text-center">Sin resultados.</div>
                            )}
                          </div>
                        )}
                      </div>
                ...
                >
                  {isSubmitting ? (
                    ...
                    <><Send size={16} strokeWidth={2.5} /> Publicar Aviso</>
                  )}
                </button>
              </form>
            </div>
          ...
          {/* ✅ COLUMNA DERECHA (Lista de Avisos con Padding Reparado) */}
          <div
            className="flex-1 flex flex-col min-w-0 w-full h-[100dvh] overflow-y-auto overscroll-contain pb-32 scrollbar-hide -mt-[140px] md:-mt-[190px] pt-[140px] md:pt-[190px] pointer-events-auto"
          >
            <div className="space-y-5 flex-1 pt-4 px-3 md:px-4">
              {paginatedList.length === 0 ? (
                // ESTADO VACÍO GLOWING (Liquidglass)
                <div className="flex flex-col items-center justify-center h-full min-h-[400px] animate-in fade-in zoom-in-95 duration-700 ease-[cubic-bezier(0.23,1,0.32,1)]">
                  <div className="relative group flex flex-col items-center text-center">
                    ...
                    <div className={`absolute top-2 w-28 h-28 rounded-full blur-[40px] opacity-30 transition-colors duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] ${announcementSearch ? 'bg-[#007AFF]' :
                      listTab === 'ACTIVE' ? 'bg-emerald-500' :
                        'bg-slate-400'
                      }`}></div>
                    ...
                    <div className={`relative z-10 w-24 h-24 rounded-[2rem] flex items-center justify-center mb-6 bg-white/60 backdrop-blur-xl border border-white/80 shadow-[0_12px_40px_rgba(0,0,0,0.08)] transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover:-translate-y-2 group-hover:shadow-[0_16px_50px_rgba(0,0,0,0.12)] ${announcementSearch ? 'text-[#007AFF]' :
                      listTab === 'ACTIVE' ? 'text-emerald-500' :
                        'text-slate-400'
                      }`}>
                      {announcementSearch ? (
                    ...
                    <h3 className="font-bold text-[22px] text-slate-800 tracking-tight mb-2 transition-colors duration-300">
                      {announcementSearch ? 'Sin resultados' : listTab === 'ACTIVE' ? 'Todo está al día' : 'Archivo vacío'}
                    </h3>
                    <p className="font-medium text-[14px] text-slate-500 max-w-[280px] leading-relaxed">
                      {announcementSearch
                        ? 'No encontramos avisos con esa búsqueda. Intenta probar con otras palabras.'
                        : listTab === 'ACTIVE'
                          ? 'Bandeja limpia. No hay avisos activos pendientes por el momento.'
                          : 'Aquí aparecerán los avisos que ya cumplieron su ciclo.'}
                    </p>
                ...
                paginatedList.map((ann) => (
                  <AnnouncementCard
                    key={ann.id}
                    ann={ann}
                    onArchive={handleArchiveCallback}
                    onDelete={handleDeleteCallback}
                    onViewDetail={handleViewDetailCallback}
                    onEdit={() => editingAnnId === ann.id ? handleCancelEdit() : handleEditClick(ann)}
                    isEditingThis={editingAnnId === ann.id}
                  />
                ))
              )}
            </div>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between pt-6 mt-2 border-t border-slate-200/50 shrink-0 px-3 md:px-4">
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest bg-white/60 backdrop-blur-sm shadow-sm px-3 py-1.5 rounded-lg border border-white/80">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="w-10 h-10 flex items-center justify-center bg-white/70 border border-white/90 rounded-xl shadow-sm text-[#007AFF] hover:bg-white hover:shadow-md hover:-translate-y-0.5 disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:translate-y-0 disabled:hover:shadow-sm transition-all duration-300 active:scale-95 backdrop-blur-md"
                  >
