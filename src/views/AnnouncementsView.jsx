  ...
  return (
    <div
      data-surface="card"
                    ...
                    className={`p-6 flex flex-col gap-4 transition-all duration-[var(--dur-lento)] ease-[var(--ease-spring)] group relative transform-gpu ${
    ...
    >
      ...
      <div className="mt-2 space-y-2">
        ...
        <div data-surface={ann.priority === 'URGENT' && !isScheduled ? undefined : 'card'} className={`w-full rounded-full h-2.5 overflow-hidden border ${ann.priority === 'URGENT' && !isScheduled ? 'bg-danger/10 border-danger/30' : ''}`}>
          <div
            className={`h-full rounded-full transition-all duration-[var(--dur-lento)] ease-[var(--ease-spring)] shadow-sm ${ann.priority === 'URGENT' && ann.readPercentage < 100 && !isScheduled ? 'bg-danger' : ann.readPercentage === 100 ? 'bg-success' : isScheduled ? 'bg-chart-3' : 'bg-brand'}`}
              ...
              {paginatedList.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full min-h-[400px] animate-in fade-in zoom-in-95 duration-[var(--dur-lento)] ease-[var(--ease-spring)]">
                  <div className="relative group flex flex-col items-center text-center">
                    ...
                    <div className={`relative z-base w-24 h-24 rounded-modal flex items-center justify-center mb-6 bg-surface-card backdrop-blur-xl border border-border-card shadow-[var(--shadow-elevation-md)] transition-all duration-[var(--dur-lento)] ease-[var(--ease-spring)] group-hover:-translate-y-2 group-hover:shadow-[var(--shadow-elevation-lg)] ${announcementSearch ? 'text-brand-text' : listTab === 'ACTIVE' ? 'text-success' : listTab === 'SCHEDULED' ? 'text-chart-3-text' : 'text-content-3'}`}>
