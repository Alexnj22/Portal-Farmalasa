                ...
                <div className="space-y-4 animate-in fade-in duration-300">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
                        {Array.from({ length: 5 }).map((_, i) => (
                            <div key={i} className={`animate-pulse bg-white/60 backdrop-blur-xl border border-white/60 rounded-[1.75rem] p-4 h-24 ${i === 4 ? 'col-span-2 lg:col-span-1' : ''}`} />
                        ))}
                    </div>
                    {/* Events pills skeleton */}
                    <div className="flex gap-2">
                        {Array.from({ length: 3 }).map((_, i) => (
                            <div key={i} className="animate-pulse bg-slate-200/80 rounded-full h-7 w-28" />
                        ))}
                    </div>
                    {/* Schedule skeleton */}
                    <div className="animate-pulse bg-white/60 backdrop-blur-xl border border-white/60 rounded-[2rem] p-4 space-y-3">
                        <div className="flex items-center justify-between mb-2">
                            <div className="bg-slate-200/80 rounded-full h-4 w-32" />
