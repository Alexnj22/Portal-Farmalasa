...
// Debe coincidir con el estimado de filas por página del PDF (pedidoPrint.js)
const ROWS_PER_PAGE = 30;

function countPrintable(items) {
    return items.filter(r => !r.sin_stock && (r.cantidad_asignada ?? 0) > 0).length;
}
...
const TIPOS = [
    {
        ...
        icon: PackageCheck,
        color: 'emerald',
        label: 'Todo llegó completo',
        sub: 'Todas las cajas están presentes y en buen estado',
    },
    {
        id: 'falta_caja',
        icon: PackageX,
        color: 'amber',
        label: 'Falta una caja',
        sub: 'Una o más cajas no llegaron con el pedido',
    },
    {
        id: 'caja_danada',
        icon: AlertTriangle,
        color: 'rose',
        label: 'Una caja llegó dañada',
        sub: 'El empaque está dañado — se revisarán los productos al contar',
    },
];

export default function LlegadaModal({ open, onClose, onConfirm, items = [], pedidoNumero }) {
    ...
    const totalBoxes = useMemo(() => Math.max(1, Math.ceil(countPrintable(items) / ROWS_PER_PAGE)), [items]);
            ...
            {screen === 2 && (
                <div className="px-5 py-4 space-y-4">
                    <p className="text-[11px] text-slate-500">
                        {tipo === 'falta_caja'
                            ? '¿Cuál(es) caja(s) no llegaron?'
                            : '¿Cuál(es) caja(s) llegaron dañadas?'}
                        <span className="ml-1 text-slate-400">— el pedido tiene {totalBoxes} caja{totalBoxes > 1 ? 's' : ''}</span>
                    </p>

                    <div className="flex flex-wrap gap-2">
                        {Array.from({ length: totalBoxes }, (_, i) => i + 1).map(n => {
                            const sel = cajasAfect.includes(n);
                            return (
