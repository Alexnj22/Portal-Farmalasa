        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <Badge variant={badge.variante} size="sm" uppercase={false}>{badge.label}</Badge>
              {ruta.salida_at && (
                ...
                return (
                  <div key={stop.id} data-surface={done ? undefined : 'card'} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border transition-colors ${done ? 'bg-success/10 border-success/30' : ''}`}>
                    ...
                    {isConductor && !isBranch && !done && ruta.status === 'en_ruta' && (
                      <Button tone="success" disabled={busy} onClick={() => handleEntregarStop(stop)}>{busy ? <Loader2 size={10} className="animate-spin" /> : <CheckCircle2 size={10} />}
                        Entregué</Button>
                    )}
