        ...
        <div className="col-span-1 lg:col-span-2 bg-white/40 backdrop-blur-3xl backdrop-saturate-[180%] border border-white/80 rounded-[2rem] p-4 shadow-[inset_0_2px_15px_rgba(255,255,255,0.7),0_10px_40px_rgba(0,0,0,0.05)] flex flex-col justify-between hover:shadow-[0_15px_50px_rgba(0,0,0,0.08)] transition-all duration-300 group/chart relative overflow-visible min-h-[150px] z-10">
            ...
            <div className="flex items-center justify-between mb-4 pr-10 relative z-10">
                ...
                <div className="flex items-center bg-white/60 p-0.5 rounded-full border border-white shadow-[inset_0_1px_4px_rgba(0,0,0,0.03)] h-7">
                    ...
                    <button onClick={() => setChartView('HOURS')} className={`px-3 h-full text-[8.5px] font-black uppercase tracking-widest rounded-full transition-all duration-200 active:scale-[0.97] ${chartView === 'HOURS' ? 'bg-white text-[#0052CC] shadow-sm scale-[1.02]' : 'text-slate-400 hover:text-slate-600 hover:bg-white/50'}`}>Horas</button>
                    <button onClick={() => setChartView('DAYS')} className={`px-3 h-full text-[8.5px] font-black uppercase tracking-widest rounded-full transition-all duration-200 active:scale-[0.97] ${chartView === 'DAYS' ? 'bg-white text-[#0052CC] shadow-sm scale-[1.02]' : 'text-slate-400 hover:text-slate-600 hover:bg-white/50'}`}>Días</button>
                </div>
            </div>
                        ...
                        currentChartData.map((item, i) => (
                            ...
                            >
                                ...
                                <div className="absolute mb-1 bottom-full left-1/2 -translate-x-1/2 bg-slate-900/90 backdrop-blur-md text-white px-2.5 py-1.5 rounded-xl shadow-xl opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none w-max z-[100] translate-y-2 group-hover:-translate-y-1 flex flex-col items-center border border-white/10">
                                    ...
                                    {chartView === 'DAYS' && (
                                        <p className="text-[7px] text-[#0052CC] font-black uppercase tracking-widest mt-1 bg-blue-500/10 px-1.5 py-0.5 rounded-full">Clic para ver horas</p>
                                    )}
                                </div>
            ...
            <div className="flex flex-wrap items-center gap-3 mt-4 justify-center md:justify-start relative z-10">
                ...
                <div className="flex items-center gap-1 text-[8px] font-bold text-slate-500 uppercase tracking-widest"><div className="w-2 h-2 rounded-full bg-[#0052CC]"></div> Normal</div>
                <div className="flex items-center gap-1 text-[8px] font-bold text-slate-500 uppercase tracking-widest"><div className="w-2 h-2 rounded-full bg-[#F79009]"></div> Pico</div>
