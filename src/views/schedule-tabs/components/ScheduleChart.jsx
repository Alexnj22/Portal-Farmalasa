...
const ScheduleChart = ({
    ...
    return (
        // 🚨 Reduje el padding de p-5 a p-4 y el min-h-[220px] a min-h-[180px]
        <div className="col-span-1 lg:col-span-2 bg-white/40 backdrop-blur-3xl backdrop-saturate-[180%] border border-white/80 rounded-[2rem] p-4 shadow-[inset_0_2px_15px_rgba(255,255,255,0.7),0_10px_40px_rgba(0,0,0,0.05)] flex flex-col justify-between hover:shadow-[0_15px_50px_rgba(0,0,0,0.08)] transition-all duration-500 group/chart relative overflow-visible min-h-[180px]">
            
            <div className="absolute top-3 right-3 opacity-0 group-hover/chart:opacity-100 transition-opacity duration-300 z-20">
                <button onClick={() => openModal && openModal("viewWfmAnalytics")} className="w-7 h-7 rounded-full bg-white/90 backdrop-blur-md text-[#007AFF] border border-blue-100 shadow-md flex items-center justify-center hover:bg-blue-50 hover:scale-105 active:scale-95 transition-all" title="Expandir Análisis">
                    <Maximize2 size={12} strokeWidth={2.5} />
                </button>
            </div>

            {/* 🚨 Reduje el margen inferior mb-8 a mb-4 */}
            <div className="flex items-center justify-between mb-4 pr-10 relative z-10">
                ...
                <div className="flex items-center bg-white/60 p-0.5 rounded-full border border-white shadow-[inset_0_1px_4px_rgba(0,0,0,0.03)] h-7">
                    {typeof chartView === 'number' && (
                        <button onClick={() => setChartView('DAYS')} className="px-2.5 h-full text-[8.5px] font-black uppercase tracking-widest rounded-full transition-all duration-300 text-slate-500 hover:text-slate-800 flex items-center gap-1 hover:bg-white/50">
                            <ChevronLeft size={10} strokeWidth={3} /> Volver a Días
                        </button>
                    )}
                    <button onClick={() => setChartView('HOURS')} className={`px-3 h-full text-[8.5px] font-black uppercase tracking-widest rounded-full transition-all duration-300 ${chartView === 'HOURS' ? 'bg-white text-[#007AFF] shadow-sm scale-[1.02]' : 'text-slate-400 hover:text-slate-600 hover:bg-white/50'}`}>Horas</button>
                    <button onClick={() => setChartView('DAYS')} className={`px-3 h-full text-[8.5px] font-black uppercase tracking-widest rounded-full transition-all duration-300 ${chartView === 'DAYS' ? 'bg-white text-[#007AFF] shadow-sm scale-[1.02]' : 'text-slate-400 hover:text-slate-600 hover:bg-white/50'}`}>Días</button>
                </div>
            </div>

            <div className="flex flex-col w-full border-b border-slate-200/60 pb-1.5 relative z-10 flex-1">
                {/* 🚨 Reduje la altura de las barras de h-[110px] a h-[80px] */}
                <div className="flex items-end gap-1.5 h-[80px] w-full relative">
                    ...
                    {isLoadingSales ? (
                        ...
                        currentChartData.map((item, i) => (
                            <div
                                ...
                                className={`flex-1 flex flex-col justify-end items-center group relative z-10 h-full ${chartView === 'DAYS' ? 'cursor-pointer' : ''}`}
                            >
                                <div className="absolute mb-1 bottom-full left-1/2 -translate-x-1/2 bg-slate-900/90 backdrop-blur-md text-white px-2.5 py-1.5 rounded-xl shadow-xl opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none w-max z-50 translate-y-2 group-hover:-translate-y-1 flex flex-col items-center border border-white/10">
                                ...
                                <div
                                    className={`w-full transition-all duration-500 ease-out group-hover:opacity-80 group-hover:shadow-md origin-bottom shadow-sm ${chartView === 'DAYS' ? 'rounded-t-[6px] group-hover:scale-y-[1.05]' : 'rounded-t-[4px] group-hover:-translate-y-[2px]'}`}
                                    style={{ height: item.height, backgroundColor: item.color }}
                                ></div>
                                
                                <span className="text-[7px] md:text-[7.5px] font-bold text-slate-400 mt-1 absolute -bottom-4 opacity-80 group-hover:opacity-100 group-hover:-translate-y-0.5 group-hover:text-cyan-500 transition-all whitespace-nowrap">
                                    {item.label}
                                </span>
                            </div>
                        ))
            ...
            {/* 🚨 Reduje el margen superior mt-8 a mt-4 */}
