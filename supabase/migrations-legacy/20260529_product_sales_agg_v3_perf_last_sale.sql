-- get_product_sales_agg v3: optimiza last_sale para evitar timeout
-- - last_sale_hist usa product_sales_monthly_agg (tabla agregada, rapida) en lugar de sales_invoice_items
-- - last_sale_live solo escanea el mes actual de sales_invoice_items (dataset pequeno)
-- - zero_sale_cands usa CROSS JOIN branch_esid (esid precomputado, 0 filas si branch es NULL)
...
-- Resolve branch_id → erp_sucursal_id once; 0 rows when p_branch_id is NULL or unknown
branch_esid AS (
...
-- Zero-sale candidates: CROSS JOIN branch_esid returns 0 rows when p_branch_id IS NULL
zero_sale_cands AS (
...
-- Last sale from the aggregated monthly table (fast — covers all history before current month)
last_sale_hist AS (
  SELECT
    a.erp_product_id                                                                  AS prod_id,
    a.branch_id,
    MAX((to_date(a.year_month || '-01', 'YYYY-MM-DD')
         + INTERVAL '1 month' - INTERVAL '1 day')::date)                              AS last_date
  FROM public.product_sales_monthly_agg a
  WHERE a.erp_product_id IN (SELECT erp_product_id FROM all_cands)
  GROUP BY a.erp_product_id, a.branch_id
),
-- Last sale from current month's live invoices only (small dataset)
last_sale_live AS (
