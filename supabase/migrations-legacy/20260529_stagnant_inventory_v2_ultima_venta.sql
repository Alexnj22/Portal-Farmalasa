-- get_stagnant_inventory v2: todas las sucursales, min/max con 0 ventas, ultima_venta
...
LANGUAGE sql STABLE AS $$
  ...
  minmax_no_sales AS (
    SELECT mm.erp_sucursal_id AS suc_id, mm.erp_product_id AS prod_id,
      COALESCE(ic.total_units, 0)::bigint AS total_units, ic.min_venc
    FROM erp_minmax mm
    LEFT JOIN inv_cur ic ON ic.suc_id = mm.erp_sucursal_id AND ic.prod_id = mm.erp_product_id
    WHERE p_erp_sucursal_id IS NOT NULL
      AND mm.erp_sucursal_id = p_erp_sucursal_id
      AND NOT EXISTS (SELECT 1 FROM sales_6m s WHERE s.suc_id = mm.erp_sucursal_id AND s.prod_id = mm.erp_product_id)
      AND NOT EXISTS (SELECT 1 FROM stagnant_src ss WHERE ss.suc_id = mm.erp_sucursal_id AND ss.prod_id = mm.erp_product_id)
  ),
  candidates AS (
    SELECT prod_id,
    ...
    FROM (
      SELECT suc_id, prod_id, total_units, min_venc FROM stagnant_src
      UNION ALL
      SELECT suc_id, prod_id, total_units, min_venc FROM minmax_no_sales
    ) u
    GROUP BY prod_id
  )
  SELECT
...
-- get_products_sold_no_minmax: soporte null (todas las sucursales)
CREATE OR REPLACE FUNCTION public.get_products_sold_no_minmax(
  p_erp_sucursal_id integer DEFAULT NULL
)
...
LANGUAGE sql STABLE AS $$
  ...
  WHERE inv.fecha  >= CURRENT_DATE - INTERVAL '6 months'
    ...
    AND ii.cantidad > 0
    AND NOT EXISTS (
      SELECT 1 FROM erp_minmax mm
      WHERE mm.erp_product_id = ii.erp_product_id
        AND (p_erp_sucursal_id IS NULL OR mm.erp_sucursal_id = p_erp_sucursal_id)
    )
