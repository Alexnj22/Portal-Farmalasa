-- calculate_stock_params v2: usa erp_sucursal_map en lugar de branch_map hardcodeada
-- Antes: WITH branch_map(bid, esid) AS (VALUES (4,1),(25,2),...)
-- Ahora: WITH branch_map AS (SELECT branch_id, erp_sucursal_id FROM erp_sucursal_map)
-- Agregar una sucursal nueva = INSERT en erp_sucursal_map; cero cambios en esta función.
...
BEGIN
  ...
  IF p_erp_sucursal_id IS NULL OR p_erp_sucursal_id != 6 THEN
    ...
    with_min AS (
      SELECT *,
        GREATEST(FLOOR(velocity * effective_lead_days)::int, 0) AS computed_min
      FROM classified
    )
