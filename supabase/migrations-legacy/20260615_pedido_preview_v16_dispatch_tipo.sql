-- get_pedido_preview v16: expone dispatch_tipo y dispatch_factor en el output.
-- El RPC ya calculaba correctamente la cantidad redondeada al múltiplo de despacho,
-- pero devolvía presentacion_tipo/factor de la presentación ERP (no la de despacho).
-- Ahora el frontend puede convertir qty y lotes a packs de despacho para mostrar en PDF.
...
AS $function$
  ...
  con_reglas AS (
    SELECT d.*,
      ...
      dpf.dp_factor         AS dispatch_factor,
      CASE
        WHEN d.asignado_raw <= 0 OR d.bodega_disponible <= 0 THEN 0

        -- v15/v16: regla por presentación explícita (prioridad sobre legado)
