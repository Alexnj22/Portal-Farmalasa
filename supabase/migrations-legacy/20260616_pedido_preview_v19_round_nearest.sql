-- get_pedido_preview v19 / v19b:
-- (1) Redondeo al múltiplo de despacho más cercano usando umbral de 40% — el código
--     anterior (v15-v18) siempre redondeaba hacia ARRIBA (CEIL) sin importar el residuo;
--     el umbral de 40% solo decidía "0 o no", nunca "hacia arriba o hacia abajo" entre
--     múltiplos intermedios. Ahora: si lo que falta para completar la siguiente unidad de
--     despacho (blister/caja/etc.) es <40% de esa unidad, se redondea hacia abajo; si es
--     ≥40%, hacia arriba. Esto subsume el check de "0" de v18 (es el caso múltiplo=0).
--     Ej. blister=10: need=30→3 blister (exacto); need=25→3 blister (residuo 5=50%≥40%);
--     need=22→2 blister (residuo 2=20%<40%).
-- (2) Cap por disponibilidad de bodega: antes, si el redondeado excedía lo disponible
--     para una sucursal, se abandonaba el redondeo POR COMPLETO y se mandaba crudo en
--     unidades ERP sueltas (rompe el empaque de blister/caja). Ahora busca el múltiplo de
--     despacho completo más grande que sí cabe en lo disponible, y solo si ni 1 unidad
--     completa cabe, manda lo disponible tal cual en presentación ERP base.
-- (3) Auto-detección de presentación para productos SIN dispatch_rule: en bodega los
--     productos se almacenan en su empaque físico (blister, caja…) y no se fraccionan.
--     Antes solo los productos con dispatch_rules veían redondeo; ahora todos los
--     productos con una presentación física (factor > 1) se redondean automáticamente
--     al empaque más pequeño disponible usando el mismo umbral del 40%.
--     Ej: need=66 tabletas, BLISTER X 10 disponible → 7 blisters (residuo 6 ≥ 40%×10).
--     La regla explícita (dispatch_rules) siempre tiene prioridad sobre la auto-detección.

CREATE OR REPLACE FUNCTION get_pedido_preview(
...
AS $function$
  ...
  stock_sucursal AS (
    ...
    WHERE em.erp_sucursal_id = ANY(p_sucursal_ids)
      AND CASE
            WHEN p_use_portal_minmax
              THEN COALESCE(psp.manual_max, psp.max_units, 0) > 0
            ELSE em.max_qty > 0
          END
    GROUP BY
      em.erp_sucursal_id, em.erp_product_id, em.erp_presentacion_id,
  ...
  -- v19b: para productos SIN dispatch_rule, auto-detecta el empaque físico más pequeño
  -- disponible (factor > 1). En bodega no se fraccionan blisters ni cajas, así que incluso
  -- sin regla explícita la cantidad debe redondearse al empaque entero más cercano.
  -- ORDER BY factor ASC → BLISTER X 10 tiene prioridad sobre CAJA X 30 (más granular).
  auto_pres_factor AS (
    SELECT DISTINCT ON (pp.product_id)
      pp.product_id,
...
GRANT EXECUTE ON FUNCTION get_pedido_preview(integer[], integer[], boolean) TO authenticated;
