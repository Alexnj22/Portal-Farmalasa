-- calculate_stock_params v4
-- Cambio: elimina "primera asignación" del auto-apply.
-- Antes: si publicado era 0/0 y el nuevo > 0 → auto-publicaba.
-- Ahora: solo auto-aplica si YA había un valor publicado y el cambio es ≤ 40%.
--        Las primeras asignaciones siempre quedan como borrador (draft_status='pending').
--
-- Corrección puntual: Salud 5 (erp_sucursal_id=7) fue auto-publicada por primera
-- asignación. Se revierte a borrador conservando los mismos valores min/max.

-- ── 1. Revertir Salud 5 de publicado a borrador ───────────────────────────────
--   draft_min/draft_max ← valor publicado actual (si ya había borrador, se respeta)
--   min_units/max_units ← NULL  (ya no está publicado)
--   published_at ← NULL         (sale de la vista de pedidos)
UPDATE product_stock_params
SET
  draft_min           = COALESCE(draft_min, min_units),
  draft_max           = COALESCE(draft_max, max_units),
  draft_status        = 'pending',
  draft_calculated_at = COALESCE(draft_calculated_at, calculated_at, NOW()),
  min_units           = NULL,
  max_units           = NULL,
  published_at        = NULL,
  published_by        = NULL,
  updated_at          = NOW()
WHERE erp_sucursal_id = 7
  AND (min_units IS NOT NULL OR max_units IS NOT NULL)
  AND published_at IS NOT NULL;

-- ── 2. Función actualizada (v4) ───────────────────────────────────────────────
...
BEGIN
  ...
  IF p_erp_sucursal_id IS NULL OR p_erp_sucursal_id != 6 THEN
    ...
    -- ── Auto-apply: solo cambios estables ≤ 40% sobre valores ya publicados ─────
    -- v4: eliminada la condición "primera asignación" (publicado era 0/0 → nuevo > 0).
    -- La primera vez que se calcula un producto SIEMPRE queda como borrador
    -- para que el usuario lo revise y publique manualmente.
    WITH auto_apply AS (
      ...
      WHERE psp.draft_status = 'pending'
        ...
        -- Solo auto-aplica cuando YA existe un valor publicado y el cambio es ≤ 40%
