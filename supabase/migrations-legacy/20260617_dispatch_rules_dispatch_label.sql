-- dispatch_label: etiqueta personalizada para el PDF cuando el producto
-- no tiene presentación CAJA en el catálogo pero se despacha en cajas virtuales.
--
-- Ejemplo: Electrolit solo tiene UNIDAD (factor=1) en el catálogo pero
-- se despacha en múltiplos de 12 que físicamente son cajas. Al poner
-- dispatch_label='CAJA' + dispatch_multiplo=12, el PDF muestra "1 CAJA"
-- en vez de "12 UNIDADES".
--
-- Implementación: cuando dispatch_label IS NOT NULL, se pliega dp_multiplo
-- dentro de dp_factor (dp_factor = pp.factor × dispatch_multiplo, dp_multiplo = 1).
-- Esto hace que dispatch_factor retorne 12 en lugar de 1, y toDispatch()
-- en el frontend convierte: toDispatch(12, 1, 12) = round(12×1/12) = 1 CAJA.

ALTER TABLE dispatch_rules ADD COLUMN IF NOT EXISTS dispatch_label text;

-- get_pedido_preview v21: dispatch_label override en dispatch_pres_factor
...
AS $function$
  ...
  -- v21: dispatch_label override — cuando está definido, dp_factor absorbe dp_multiplo
  -- (dp_factor = pp.factor × dispatch_multiplo, dp_multiplo = 1) para que dispatch_factor
  -- retorne el factor combinado y toDispatch() convierta correctamente en el frontend.
  ...
  -- v20: ORDER BY factor DESC → elige la presentación MAYOR disponible (CAJA > BLISTER).
  auto_pres_factor AS (
    SELECT DISTINCT ON (pp.product_id)
      pp.product_id,
