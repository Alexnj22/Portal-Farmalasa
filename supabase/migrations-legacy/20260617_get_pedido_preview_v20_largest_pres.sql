-- get_pedido_preview v20: fix auto-detección de presentación de despacho
--
-- Bug en v19b: auto_pres_factor usaba ORDER BY factor ASC (presentación más pequeña primero).
-- Esto hacía que AMITRAL GX se despachara en BLISTER (×10) en vez de CAJA (×100 o similar),
-- y que productos similares mostraran la presentación más granular en lugar de la mayor.
-- Fix: ORDER BY factor DESC → se elige la presentación mayor disponible (CAJA > BLISTER > und).
-- Esto es consistente con el CSV de MinMax que también elige la presentación mayor.
-- La regla dispatch_rules explícita siempre tiene prioridad sobre la auto-detección (sin cambio).
...
AS $function$
  ...
  -- v19b usaba ASC (más pequeña primero), lo que causaba AMITRAL GX → 10 BLISTER en vez de 1 CAJA.
  auto_pres_factor AS (
    SELECT DISTINCT ON (pp.product_id)
      pp.product_id,
