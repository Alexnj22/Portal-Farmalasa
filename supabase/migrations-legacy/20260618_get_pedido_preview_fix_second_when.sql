-- ============================================================
-- get_pedido_preview v4 — fix segundo WHEN en con_reglas
--
-- Bug: el segundo WHEN de con_reglas no tenía la guarda
-- "asignado_uncapped > 0", entonces cuando la fracción era < 40%
-- (asignado_uncapped=0) y max_asignable era grande (bodega con
-- sobrante), el segundo WHEN disparaba y enviaba
-- FLOOR(max_asignable / unit_base) * unit_base unidades — mucho
-- más que reponer.
--
-- Ejemplo: VIROGRIP LIMON X 24, sucursal necesita 7 unidades,
-- bodega tiene 168. max_asignable=117, FLOOR(117/24)=4 cajas →
-- enviaba 96 cuando solo necesitaba 7.
--
-- Fix: agregar "cu.asignado_uncapped > 0" al segundo WHEN en
-- los tres CASE de con_reglas (asignado_final, dispatch_tipo,
-- dispatch_factor). Ahora cuando asignado_uncapped=0, el ELSE
-- envía asignado_raw como despacho parcial (correcto).
-- ============================================================
...
AS $function$
  ...
  con_reglas AS (
    SELECT
      ...
      -- Ambos WHEN requieren asignado_uncapped > 0.
      -- Si asignado_uncapped = 0 (fracción < 40% del dispatch_unit), el ELSE
      -- envía asignado_raw como despacho parcial en presentacion_tipo base.
      CASE
