-- ============================================================
-- get_pedido_preview v5 — regla de despacho siempre se respeta
--
-- Corrección de lógica en con_reglas:
--
-- Comportamiento correcto:
--   1. asignado_uncapped > 0 y <= max_asignable → dispatch en units completas (normal)
--   2. asignado_uncapped > 0 y > max_asignable  → floor(max_asignable/unit) units (cap)
--   3. asignado_raw > 0, fracción < 40%, bodega < 1 dispatch_unit → parcial huérfano
--      (bodega no puede formar ni una caja para nadie, mejor enviar que dejar huérfanos)
--   4. fracción < 40% y bodega tiene cajas completas disponibles → asignado_final = 0
--      La regla se respeta. La sucursal acumula necesidad hasta el siguiente pedido.
--
-- revision_minmax cambia de "asignado_final=0" a "asignado_raw=0":
--   Solo se activa cuando la distribución proporcional dio 0 a esta sucursal
--   (bodega gastó todo en sucursales con mayor peso de demanda).
--   NO se activa cuando la regla bloquea la fracción correctamente.
-- ============================================================

CREATE OR REPLACE FUNCTION public.get_pedido_preview(
    p_sucursal_ids integer[],
    p_target_ids   integer[] DEFAULT NULL::integer[],
...
AS $function$
  ...
  stock_sucursal AS (
    SELECT
      ...
      CASE WHEN p_use_portal_minmax THEN ROUND(COALESCE(psp.manual_min,psp.min_units,0)::numeric/NULLIF(COALESCE(pp.factor,1)::numeric,0))::integer ELSE em.min_qty END AS min_qty,
      CASE WHEN p_use_portal_minmax THEN ROUND(COALESCE(psp.manual_max,psp.max_units,0)::numeric/NULLIF(COALESCE(pp.factor,1)::numeric,0))::integer ELSE em.max_qty END AS max_qty,
    ...
    LEFT JOIN product_stock_params psp ON psp.erp_product_id = em.erp_product_id AND psp.erp_sucursal_id = em.erp_sucursal_id
    ...
    WHERE em.erp_sucursal_id = ANY(p_sucursal_ids)
      AND CASE WHEN p_use_portal_minmax THEN COALESCE(psp.manual_max,psp.max_units,0) > 0 ELSE em.max_qty > 0 END
    GROUP BY em.erp_sucursal_id, em.erp_product_id, em.erp_presentacion_id, em.min_qty, em.max_qty, pr.tipo, pp.factor, psp.min_units, psp.max_units, psp.manual_min, psp.manual_max
  ),
  ...
  pres_units_needed AS (
    ...
    FROM necesidades GROUP BY erp_product_id, erp_presentacion_id, factor
  ),
  pres_units_total AS (SELECT erp_product_id, SUM(units_needed) AS units_total FROM pres_units_needed GROUP BY erp_product_id),

  pending_committed AS (
    SELECT pi.erp_product_id, SUM(pi.cantidad_asignada::numeric * COALESCE(pp.factor, 1)) AS committed_units
    FROM pedido_items pi JOIN pedidos pd ON pd.id = pi.pedido_id
    LEFT JOIN product_precios pp ON pp.product_id = pi.erp_product_id AND pp.id_presentacion = pi.erp_presentacion_id
    WHERE pi.status = 'pendiente' AND pd.status NOT IN ('anulado', 'completado')
  ...
  bodega_raw AS (
    ...
    LEFT JOIN inv_dedup inv ON inv.erp_sucursal_id = (SELECT erp_sucursal_id FROM bodega_suc) AND inv.erp_product_id = pu.erp_product_id
  ...
  bodega AS (
    SELECT br.erp_product_id, br.erp_presentacion_id,
      ROUND(br.bodega_units_total * COALESCE(pu.units_needed / NULLIF(pt.units_total, 0), 1.0) / NULLIF(br.factor, 0), 2) AS bodega_pk
    FROM bodega_raw br
  ...
  ventas_suc AS (
    ...
    FROM product_sales_monthly_agg s JOIN suc_map sm ON sm.branch_id = s.branch_id
  ...
  distribucion_floor AS (
    SELECT n.erp_sucursal_id, n.erp_product_id, n.erp_presentacion_id,
      ...
      CASE
        ...
        ELSE LEAST(n.reponer, FLOOR(t.bodega_disponible * n.peso_suc / t.total_pesos))::integer
      ...
      CASE
        ...
        ELSE LEAST(n.reponer::numeric, t.bodega_disponible * n.peso_suc / t.total_pesos)
      END AS quota_real
    FROM necesidades_ponderadas n
  ...
  raw_totals AS (
    ...
    FROM distribucion GROUP BY erp_product_id, erp_presentacion_id
  ),
  ...
  dispatch_pres_factor AS (
    SELECT DISTINCT ON (dr.erp_product_id) dr.erp_product_id,
      CASE WHEN dr.dispatch_label IS NOT NULL THEN pp.factor::numeric * COALESCE(dr.dispatch_multiplo,1)::numeric ELSE pp.factor::numeric END AS dp_factor,
      CASE WHEN dr.dispatch_label IS NOT NULL THEN 1::numeric ELSE COALESCE(dr.dispatch_multiplo,1)::numeric END AS dp_multiplo,
  ...
  auto_pres_factor AS (
    ...
    WHERE pp.factor > 1 ORDER BY pp.product_id, pp.factor DESC
  ),
  ...
  con_reglas_unit AS (
    SELECT d.*,
      ...
      dpf.dp_multiplo, cfm.caja_factor,
      CASE
        ...
        WHEN COALESCE(dr.solo_cajas,false) = true
             ...
             AND d.presentacion_tipo != 'CAJA' AND cfm.caja_factor IS NOT NULL THEN cfm.caja_factor
  ...
  -- Redondea a la unidad de despacho (caja/blister/etc).
  -- Fracción >= 40% → redondeo hacia arriba. < 40% → 0 (no alcanza para una unidad).
  ...
  con_reglas AS (
    SELECT
      ...
      cu.tiene_regla, cu.regla_multiplo, cu.regla_blister, cu.regla_solo_cajas, cu.regla_multiplo_unidades,
      cu.asignado_raw,  -- necesario para revision_minmax en el SELECT final
      CASE
        WHEN cu.asignado_uncapped > 0 AND cu.asignado_uncapped <= cu.max_asignable THEN cu.dp_tipo
        WHEN cu.asignado_uncapped > 0 AND cu.unit_base IS NOT NULL
             AND FLOOR(cu.max_asignable * cu.factor / cu.unit_base) >= 1 THEN cu.dp_tipo
        ELSE cu.presentacion_tipo
      END AS dispatch_tipo,
      CASE
        WHEN cu.asignado_uncapped > 0 AND cu.asignado_uncapped <= cu.max_asignable THEN cu.dp_factor
        WHEN cu.asignado_uncapped > 0 AND cu.unit_base IS NOT NULL
             AND FLOOR(cu.max_asignable * cu.factor / cu.unit_base) >= 1 THEN cu.dp_factor
        ELSE cu.factor
      END AS dispatch_factor,
      CASE
        -- 1. Normal: cabe en dispatch units completas
        WHEN cu.asignado_uncapped > 0 AND cu.asignado_uncapped <= cu.max_asignable
          THEN cu.asignado_uncapped
        -- 2. Capped: asignado_uncapped excede el cap → bajar al floor del cap
        WHEN cu.asignado_uncapped > 0 AND cu.unit_base IS NOT NULL
             AND FLOOR(cu.max_asignable * cu.factor / cu.unit_base) >= 1
          THEN (FLOOR(cu.max_asignable * cu.factor / cu.unit_base) * cu.unit_base / NULLIF(cu.factor, 0))::integer
        -- 3. Huérfano: bodega tiene menos de 1 dispatch_unit en total → nadie puede
        --    recibir una caja completa, mejor enviar lo disponible que dejar stock huérfano
        WHEN cu.asignado_raw > 0 AND cu.unit_base IS NOT NULL
             AND cu.bodega_disponible < (cu.unit_base / NULLIF(cu.factor, 0))
          THEN cu.asignado_raw
        -- 4. Regla no cumplida: fracción < 40% y bodega tiene cajas completas.
        --    La regla se respeta. La sucursal acumula necesidad hasta el próximo pedido.
        ELSE 0
  ...
  bodega_lotes_raw AS (
    SELECT inv.erp_product_id, inv.lote, inv.fecha_vencimiento, SUM(inv.unidades) AS total_units
    FROM inv_dedup inv
    INNER JOIN (SELECT DISTINCT erp_product_id FROM con_reglas) cr ON cr.erp_product_id = inv.erp_product_id
    WHERE inv.erp_sucursal_id = (SELECT erp_sucursal_id FROM bodega_suc)
      AND inv.is_vencidos = false AND inv.unidades > 0
    GROUP BY inv.erp_product_id, inv.lote, inv.fecha_vencimiento HAVING SUM(inv.unidades) > 0
  ),

  bodega_lotes_pres AS (
    SELECT bl.erp_product_id, pu.erp_presentacion_id, bl.lote, bl.fecha_vencimiento,
      GREATEST(0, FLOOR(bl.total_units::numeric * COALESCE(pu.units_needed / NULLIF(pt.units_total, 0), 1.0) / NULLIF(pu.factor, 0)))::integer AS lote_packs
  ...
  bodega_lotes_fefo AS (
    ...
    FROM bodega_lotes_pres bl WHERE bl.lote_packs > 0
  ),
  ...
  lote_intersect AS (
    SELECT so.erp_sucursal_id, so.erp_product_id, so.erp_presentacion_id, lf.lote, lf.fecha_vencimiento,
      GREATEST(0, LEAST(so.suc_cum_end, lf.lote_cum_end) - GREATEST(so.suc_cum_start, lf.lote_cum_start))::integer AS take_packs
    FROM suc_order so
    JOIN bodega_lotes_fefo lf ON lf.erp_product_id = so.erp_product_id AND lf.erp_presentacion_id = so.erp_presentacion_id
  ),

  lotes_por_sucursal AS (
    SELECT erp_sucursal_id, erp_product_id, erp_presentacion_id,
      jsonb_agg(jsonb_build_object('lote', lote, 'fecha_vencimiento', fecha_vencimiento, 'packs', take_packs) ORDER BY fecha_vencimiento ASC NULLS LAST, lote ASC) AS lotes_seq
    FROM lote_intersect WHERE take_packs > 0
    GROUP BY erp_sucursal_id, erp_product_id, erp_presentacion_id
  )

  SELECT
    cr.erp_sucursal_id, cr.erp_product_id, cr.erp_presentacion_id,
    p.nombre::text AS product_name, lab.nombre::text AS laboratorio,
    ...
    -- revision_minmax usa asignado_raw (no asignado_final) para no confundir
    -- "regla bloquea fracción correctamente" con "bodega no alcanzó ni 1 pack proporcional"
    (cr.bodega_disponible > 0 AND cr.asignado_raw = 0 AND cr.reponer > 0) AS revision_minmax,
    ...
    cr.regla_multiplo, cr.regla_blister, cr.regla_solo_cajas, cr.regla_multiplo_unidades,
    COALESCE(p.es_antibiotico, false) AS es_antibiotico,
    cr.ventas_6m,
    lps.lotes_seq AS lotes_bodega,
    cr.dispatch_tipo, cr.dispatch_factor
  FROM con_reglas cr
