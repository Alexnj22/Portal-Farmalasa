-- Fix: CSV bodega mostraba códigos ERP ("1x10") como nombre de presentación.
-- Causa: cuando bodega no tiene inventario de un producto, inv_summary queda vacío
-- y el fallback era catalog_pres que usa product_precios.descripcion ("1x10", "1x1").
-- Ejemplo: AMLODIPINA ECOMED tiene presentacion='CAJA' en inventario de sucursales,
-- pero bodega sin stock → caía a catalog_pres → CSV mostraba "1x10" en vez de "CAJA".
--
-- Fix: nuevo CTE inv_other_pres escanea inventario de TODAS las sucursales para
-- obtener los nombres reales (CAJA, BLISTER, FRASCO, etc.) de las presentaciones
-- con factor>1. Se usa como fallback antes de catalog_pres.
--
-- Prioridad final en presentations[]:
--   1. inv_base_pres  (base/pequeña de cualquier sucursal)
--   2. inv_summary    (factor>1 de la sucursal actual, si tiene inventario)
--   3. inv_other_pres (factor>1 de cualquier sucursal — nombres reales)
--   4. catalog_pres   (último recurso: product_precios.descripcion)
...
LANGUAGE sql STABLE AS $$
  ...
  SELECT
    ...
    END || (CASE
      ...
      WHEN iop.presentations IS NOT NULL
      THEN iop.presentations
