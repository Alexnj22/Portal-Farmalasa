-- Agrega has_pending_branches al RPC get_stock_analysis.
-- Campo boolean: true cuando alguna sucursal (no bodega) tiene draft_status='pending'
-- para ese producto. Útil en la vista bodega para mostrar el badge de "suc. pendientes"
-- incluso cuando bodega ya está en live (el trigger fix hizo que bodega ya no quede en draft).
...
LANGUAGE sql STABLE AS $$
  ...
  -- Sucursales con borrador pendiente por producto (excluye bodega)
  pending_branches AS (
  ...
  params AS (
    SELECT
      ...
      -- Modelo aditivo: effective = sum_publicado + delta_manual
