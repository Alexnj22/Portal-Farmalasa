-- get_pedido_preview v24: usa v_product_factor para el factor real de inventario.
-- inv_suc e inv_bodega ahora hacen LEFT JOIN a v_product_factor por (product_id, presentacion).
-- Fallback: split('x') del detalle si no hay match en catálogo.
...
AS $function$
  ...
  -- Stock de sucursales: factor desde v_product_factor (autoritativo); fallback a split('x') del detalle
  inv_suc AS (
    SELECT DISTINCT ON (
  ...
  -- Stock de bodega: mismo lookup de factor
  inv_bodega AS (
    SELECT DISTINCT ON (
