-- Fix: stock_sucursal usa el factor de la dispatch rule cuando existe (no el más alto).
-- Para productos SIN regla, usa el menor factor > 1 (blister antes que caja).
-- Esto corrige:
--   · NOVOMIT suc1: ROUND(181/250)=1 → factor=250 (incorrecto). Ahora factor=10 (BLISTER).
--   · AERO OM suc2/4: ROUND(150/250)=1 → factor=250 (incorrecto). Ahora factor=10 (BLISTER).
--   · auto_pres_factor (fallback sin regla): cambia DESC→ASC para elegir el menor pack > 1.
-- Las 3 funciones (preview, stats, sin_bodega) reciben el mismo fix en su CTE necesidades.

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. get_pedido_preview  (v25)
-- ─────────────────────────────────────────────────────────────────────────────
...
AS $function$
  ...
  inv_bodega AS (
    SELECT DISTINCT ON (
      i.erp_product_id, i.lote, i.fecha_vencimiento, i.is_vencidos,
      TRIM(LOWER(COALESCE(i.presentacion,''))), LOWER(COALESCE(i.detalle,''))
    )
      i.erp_product_id, i.lote, i.fecha_vencimiento, i.is_vencidos, i.cantidad, i.detalle,
      i.cantidad::numeric * COALESCE(
        vf.factor,
        NULLIF(split_part(LOWER(COALESCE(i.detalle,'')), 'x', 2), '')::numeric,
        1
  ...
  -- Factor preferido: el de la presentación de despacho cuando existe dispatch rule.
  -- Sin regla: queda NULL y stock_sucursal elige el menor factor > 1.
  pref_factor AS (
  ...
  stock_sucursal AS (
    ...
    LEFT JOIN inv_agg ia ON ia.erp_sucursal_id = psp.erp_sucursal_id AND ia.erp_product_id = psp.erp_product_id
    ...
    ORDER BY
      psp.erp_sucursal_id,
      psp.erp_product_id,
      -- 1º: factor exacto de la dispatch rule (cuando existe)
      (pp.factor = COALESCE(pf.pref, -1)) DESC,
      -- 2º: sin regla → preferir factor > 1 sobre UNIDAD
      CASE WHEN pf.pref IS NULL THEN (pp.factor > 1)::int ELSE 0 END DESC,
      -- 3º: el menor factor disponible
      pp.factor ASC,
      pp.id_presentacion
  ),
  ...
  bodega_raw AS (
    ...
    LEFT JOIN inv_bodega inv ON inv.erp_product_id = pu.erp_product_id
  ...
  dispatch_pres_factor AS (
    SELECT DISTINCT ON (dr.erp_product_id)
      dr.erp_product_id, pp.factor::numeric AS dp_factor,
      COALESCE(dr.dispatch_multiplo, 1)::numeric AS dp_multiplo, pres.tipo AS dp_tipo
    FROM dispatch_rules dr
    ...
    SELECT d.*,
      ...
      d.asignado_raw::numeric
        + GREATEST(0, d.bodega_disponible - rt.total_raw_assigned)
          * d.asignado_raw::numeric / NULLIF(rt.total_raw_assigned, 0) AS max_asignable,
      ...
      CASE
        ...
        ELSE GREATEST(0,
          (FLOOR(cu.asignado_raw::numeric * cu.factor / cu.unit_base)
           + CASE WHEN (cu.asignado_raw::numeric * cu.factor)
                       - FLOOR(cu.asignado_raw::numeric * cu.factor / cu.unit_base) * cu.unit_base
                       >= 0.40 * cu.unit_base THEN 1 ELSE 0 END
  ...
  con_reglas AS (
    SELECT
      ...
      CASE WHEN cu.asignado_uncapped <= cu.max_asignable THEN cu.dp_tipo
           WHEN cu.unit_base IS NOT NULL AND FLOOR(cu.max_asignable * cu.factor / cu.unit_base) >= 1 THEN cu.dp_tipo
           ELSE cu.presentacion_tipo END AS dispatch_tipo,
      CASE WHEN cu.asignado_uncapped <= cu.max_asignable THEN cu.dp_factor
           WHEN cu.unit_base IS NOT NULL AND FLOOR(cu.max_asignable * cu.factor / cu.unit_base) >= 1 THEN cu.dp_factor
           ELSE cu.factor END AS dispatch_factor,
      CASE WHEN cu.asignado_uncapped <= cu.max_asignable THEN cu.asignado_uncapped
           ...
           ELSE cu.asignado_raw END AS asignado_final
  ...
  bodega_lotes_raw AS (
    ...
    WHERE inv.is_vencidos = false AND inv.unidades > 0
  ...
  bodega_lotes_fefo AS (
    SELECT bl.*,
      COALESCE(SUM(bl.lote_packs) OVER (PARTITION BY bl.erp_product_id, bl.erp_presentacion_id
        ORDER BY bl.fecha_vencimiento ASC NULLS LAST, bl.lote ASC
        ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING), 0)::integer AS lote_cum_start,
      SUM(bl.lote_packs) OVER (PARTITION BY bl.erp_product_id, bl.erp_presentacion_id
        ORDER BY bl.fecha_vencimiento ASC NULLS LAST, bl.lote ASC
        ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)::integer AS lote_cum_end
  ...
  suc_order AS (
    SELECT cr.erp_sucursal_id, cr.erp_product_id, cr.erp_presentacion_id, cr.asignado_final,
      COALESCE(SUM(cr.asignado_final) OVER (PARTITION BY cr.erp_product_id, cr.erp_presentacion_id
        ORDER BY COALESCE(esm.orden_despacho,999) ASC, cr.erp_sucursal_id ASC
        ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING), 0)::integer AS suc_cum_start,
      SUM(cr.asignado_final) OVER (PARTITION BY cr.erp_product_id, cr.erp_presentacion_id
        ORDER BY COALESCE(esm.orden_despacho,999) ASC, cr.erp_sucursal_id ASC
        ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)::integer AS suc_cum_end
    FROM con_reglas cr
  ...
  lotes_por_sucursal AS (
    SELECT erp_sucursal_id, erp_product_id, erp_presentacion_id,
      jsonb_agg(jsonb_build_object('lote', lote, 'fecha_vencimiento', fecha_vencimiento, 'packs', take_packs)
        ORDER BY fecha_vencimiento ASC NULLS LAST, lote ASC) AS lotes_seq
  ...
  LEFT JOIN lotes_por_sucursal lps ON lps.erp_sucursal_id = cr.erp_sucursal_id
