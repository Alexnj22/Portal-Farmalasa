-- cantidad_problema en pedido_items + receive_pedido_sucursal v4 (v2.2.259)
ALTER TABLE pedido_items
    ADD COLUMN IF NOT EXISTS cantidad_problema INTEGER DEFAULT NULL;

DROP FUNCTION IF EXISTS receive_pedido_sucursal(uuid, integer, jsonb, uuid);
DROP FUNCTION IF EXISTS receive_pedido_sucursal(uuid, integer, jsonb, uuid, uuid[]);
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pedido_items WHERE pedido_id = p_pedido_id AND status = 'pendiente') THEN
