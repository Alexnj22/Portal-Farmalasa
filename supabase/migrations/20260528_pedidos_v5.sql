-- ============================================================
-- Pedidos v5: correcciones al algoritmo de distribución
--
-- Fix 1 — Peso dominado por ventas (rotación de inventario)
--          Antes: peso = reponer × (1 + share_ventas)
--                 → prioriza quien más necesita; ventas solo como desempate
--          Ahora: peso = ventas_6m
--                 → prioriza quien más vende; sin historial → fallback a reponer
--
-- Fix 2 — Método del mayor residuo (Largest Remainder Method)
--          FLOOR proporcional dejaba unidades sin asignar por acumulación
--          de decimales truncados. Ahora: el sobrante se distribuye una
--          unidad a la vez a las sucursales con mayor fracción decimal,
--          respetando siempre el cap individual de reponer.
-- ============================================================
...
AS $$
  ...
  -- Fix 1: peso = ventas_6m (rotación). Sin historial → fallback a reponer.
  ...
  -- Fix 2a: asignación FLOOR + quota_real para calcular fracción decimal.
  distribucion_floor AS (
    SELECT
      ...
      -- valor real antes de FLOOR, para calcular la fracción sobrante
      CASE
  ...
  -- Fix 2b: calcular sobrante y ranking por fracción decimal.
  distribucion_lr AS (
    SELECT
      df.*,
      GREATEST(0,
  ...
  -- Fix 2c: dar +1 a las sucursales top-ranked hasta agotar el sobrante.
  distribucion AS (
    SELECT
      lr.erp_sucursal_id,
      lr.erp_product_id,
      ...
      CASE
        -- sin escasez: mantener asignación directa
        WHEN lr.bodega_disponible <= 0 OR lr.bodega_disponible >= lr.total_reponer
          THEN lr.asignado_floor
        -- escasez: dar +1 si tiene fracción alta, no está en su cap y hay sobrante
