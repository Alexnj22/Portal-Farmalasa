-- get_product_sales_agg v4: last_sale_hist y last_sale_live sin filtro IN
-- Full scan + hash aggregate sobre product_sales_monthly_agg (más rápido que semi-join IN)
-- Mismo patrón que get_stagnant_inventory usa con last_sale_all
...
-- Full scan sin filtro IN — hash aggregate simple, más rápido que semi-join
last_sale_hist AS (
  SELECT
...
-- Solo mes actual — pequeño por definición
last_sale_live AS (
