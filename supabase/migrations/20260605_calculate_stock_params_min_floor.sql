...
-- El único par válido con MIN=0 es (0,1) — producto de rotación tan baja que solo
-- se mantiene 1 unidad como techo. Cualquier otro caso (MIN=0, MAX=3) no tiene
-- sentido operativo: ¿cuándo pedís? cuando llegás a 0, ya es tarde.
--
-- Regla: computed_min = GREATEST(FLOOR(v × lead), 1) si CEIL(v × cycle) > 1
--                     = GREATEST(FLOOR(v × lead), 0) si CEIL(v × cycle) = 1  ← el 0/1
--
-- Equivale a: GREATEST(FLOOR(v × lead), CASE WHEN CEIL(v × cycle) > 1 THEN 1 ELSE 0 END)
...
BEGIN
  ...
  IF p_erp_sucursal_id IS NULL OR p_erp_sucursal_id != 6 THEN
    ...
    with_min AS (
      SELECT *,
        -- MIN floor: if MAX would be >1, MIN must be at least 1.
        -- Only (0, 1) is valid with MIN=0 — very slow mover, no point ordering before stockout.
        GREATEST(
