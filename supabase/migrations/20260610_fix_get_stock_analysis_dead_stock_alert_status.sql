-- Restaura alert_status = 'dead_stock' en el branch 2 de get_stock_analysis.
-- La migración 20260610_minmax_calc_columns.sql lo cambió a 'no_data' accidentalmente,
-- rompiendo el tab "Sin movimiento" (UI cuenta alert_status === 'dead_stock' → mostraba 0).
-- Branch 3 (sin historial) conserva 'no_data' — son conceptos distintos en la UI.
...
AS $function$
  ...
  FROM (
    ...
    -- alert_status = 'dead_stock' (el tab "Sin movimiento" en la UI lo cuenta así)
    SELECT
      inv2.erp_product_id,
    ...
    -- alert_status = 'no_data' (distinto de dead_stock; UI usa noHistory = alert_status==='no_data')
    SELECT
      p3.id,
      p3.nombre,
      'D'::text,
