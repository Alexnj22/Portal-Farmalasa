-- get_stock_analysis v6: agrega last_sale_date al resultado via CTE MATERIALIZED
-- Reemplaza la llamada separada get_last_sale_dates (que fallaba para algunos productos
-- por discrepancias en el join branch_id cuando la función corre en paralelo).
-- La fecha de última venta ahora viene directa del mismo scan de ventas, garantizando
-- que cualquier producto con units_sold_6m > 0 tenga su fecha.
--
-- También agrega índices en sales_invoice_items / sales_invoices que aceleran tanto
-- este CTE como get_product_last_sales (panel expandido).

-- Índices de performance
CREATE INDEX IF NOT EXISTS idx_sii_erp_product_id
  ON sales_invoice_items(erp_product_id)
  WHERE erp_product_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_si_branch_estado
  ON sales_invoices(branch_id, estado);

-- Nueva versión de get_stock_analysis (DROP + CREATE porque cambia la firma)
...
AS $function$
  ...
  -- Última venta por producto en esta sucursal (MATERIALIZED para calcular una sola vez)
