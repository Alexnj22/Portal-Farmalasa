-- ============================================================
-- Pedidos FASE 1 — Correcciones SQL
--
-- 1.1  inv_dedup CTE: DISTINCT ON elimina filas duplicadas que el
--      ERP inserta por el bug conocido (~5 585 grupos / 7 390 filas extra).
--      Afecta stock_sucursal, bodega_raw y bodega_lotes_raw.
--
-- 1.2  max_qty ya unificado: todas las funciones usan erp_minmax.max_qty
--      (los valores que el usuario configura en el módulo MinMax).
--      Sin cambio de criterio; se documenta explícitamente.
--
-- 1.3  pending_committed CTE: descuenta de bodega las unidades ya
--      asignadas en pedidos pendientes (status='pendiente') que aún
--      no han sido despachados, evitando sobre-asignación entre pedidos.
-- ============================================================


-- ── 1. get_pedido_preview v10 ────────────────────────────────
...
AS $$
  ...
  -- 1.1 Dedup: una fila por (sucursal, product, lote, vencimiento, is_vencidos)
  inv_dedup AS (
    SELECT DISTINCT ON (erp_sucursal_id, erp_product_id, lote, fecha_vencimiento, is_vencidos)
      erp_sucursal_id, erp_product_id, lote, fecha_vencimiento, is_vencidos, cantidad
    FROM inventory
    ORDER BY erp_sucursal_id, erp_product_id, lote, fecha_vencimiento, is_vencidos
  ),
  ...
  -- 1.3 Unidades ya comprometidas en pedidos activos no despachados
  ...
  -- FEFO secuencial — igual que v9, ahora sobre inv_dedup
  bodega_lotes_raw AS (
...
-- ── 2. get_pedido_sucursal_stats v9 ─────────────────────────
...
-- Bodega neto: stock real menos unidades ya comprometidas en pedidos activos
bodega_net AS (
  SELECT inv.erp_product_id,
    SUM(inv.cantidad) - COALESCE(MAX(pc.committed_units), 0) AS net_units
  FROM inv_dedup inv
...
-- ── 4. get_pedido_sin_bodega_count v9 ───────────────────────
...
WHERE NOT EXISTS (
  SELECT 1 FROM bodega_net bn
  WHERE bn.erp_product_id = np.erp_product_id AND bn.net_units > 0
);
$$;
