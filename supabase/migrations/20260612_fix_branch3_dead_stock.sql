-- get_stock_analysis v7: Branch 3 vuelve a retornar dead_stock + is_dead_stock=true
-- Regresión introducida en v6 (last_sale_date): Branch 3 retornaba out_of_stock/ok + false,
-- lo que hacía que los productos "sin historial" no aparecieran en el chip "Sin movimiento"
-- ni se identificaran correctamente en el renderizado.
-- Restaura el comportamiento de v2.2.19.
...
AS $function$
  ...
  -- Última venta por producto en esta sucursal
  ...
  FROM (
    ...
    -- Retorna dead_stock + is_dead_stock=true para que aparezcan bajo "Sin movimiento"
    -- y sean identificables/editables en el frontend (restaura comportamiento v2.2.19)
    SELECT
      p3.id,
      p3.nombre,
      'D'::text,
