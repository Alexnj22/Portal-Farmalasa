-- get_stock_analysis v8: Branch 4 — productos solo en catálogo (sin params ni inventario en sucursal)
-- + fix Branch 1: AND pr.daily_velocity IS NOT NULL (evita solapamiento con Branch 3)
-- + fix Branch 3: elimina NOT EXISTS auto-referencial que siempre excluía todo
-- + nuevo: is_catalog_only boolean en RETURNS TABLE
--
-- Branch 4 retorna is_catalog_only=true, alert_status='no_data':
--   - Ocultos por defecto (frontend los filtra si !searchTerm && filterAlert !== 'no_data')
--   - Aparecen al filtrar "Sin historial" o al buscar por nombre
--   - Al guardar MIN/MAX se crea fila en product_stock_params; en siguiente carga pasan a Branch 1/3
...
AS $function$
  ...
  last_sale AS MATERIALIZED (
    ...
    WHERE bm_s.erp_sucursal_id   = p_erp_sucursal_id
  ...
  FROM (
    -- Branch 1: Productos con fila en product_stock_params y con velocidad calculada
    SELECT
    ...
    -- Branch 3: Sin historial — fila en params pero sin velocidad calculada
    -- is_dead_stock=true → aparece bajo "Sin movimiento"
    SELECT
      p3.id,
      p3.nombre,
      'D'::text,
    ...
    -- Branch 4: Catalog-only — productos sin params NI inventario en esta sucursal
    -- alert_status='no_data' + is_catalog_only=true:
    --   ocultos por defecto; visibles al buscar o al filtrar "Sin historial"
    SELECT
      p4.id,
      p4.nombre,
      'D'::text,
