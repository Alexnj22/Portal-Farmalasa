-- ================================================================
-- Pedidos v2.2.55 — Mejoras integrales
-- A1: Enforce solo_cajas in SQL + data fix (multiplo rules had solo_cajas=true by bug)
-- B1: p_target_ids — global-distribution mode (distribute all bodega, show subset)
-- B2: multiplo_unidades rule (units multiple)
-- C1: laboratorio column in RETURNS TABLE
-- ================================================================

-- B2: add column (idempotent)
ALTER TABLE dispatch_rules ADD COLUMN IF NOT EXISTS multiplo_unidades smallint;

-- A1: data fix — existing multiplo rules had solo_cajas=true by frontend bug; clear them
UPDATE dispatch_rules SET solo_cajas = false WHERE multiplo IS NOT NULL AND solo_cajas = true;

-- ── get_pedido_preview v12 ────────────────────────────────────

DROP FUNCTION IF EXISTS get_pedido_preview(integer[]);
DROP FUNCTION IF EXISTS get_pedido_preview(integer[], integer[]);
...
AS $function$
  ...
  -- inv_dedup: normalise cantidad → unidades using detalle factor (e.g. "1X10" → ×10)
  -- Dedup key includes presentacion+detalle to keep distinct presentations of same product.
  inv_dedup AS (
    SELECT DISTINCT ON (
  ...
  -- A1: factor de la presentación CAJA por producto (para solo_cajas enforcement)
  caja_factor_map AS (
  ...
  con_reglas AS (
    SELECT d.*,
      ...
      CASE
        ...
        -- solo_cajas: round packs down to whole cajas (only when presentation is not CAJA)
        WHEN COALESCE(dr.solo_cajas, false) = true
          ...
          THEN GREATEST(0, FLOOR(
            ...
            / NULLIF(COALESCE(cfm.caja_factor, d.factor), 0)
          ) * COALESCE(cfm.caja_factor, d.factor) / NULLIF(d.factor, 0))::integer

        -- multiplo de cajas
        WHEN dr.multiplo IS NOT NULL
          THEN (FLOOR(d.asignado_raw::numeric / dr.multiplo) * dr.multiplo)::integer

        -- multiplo de blisters
        WHEN dr.blister IS NOT NULL
          THEN (FLOOR(d.asignado_raw::numeric / dr.blister) * dr.blister)::integer

        -- multiplo de unidades: asegurar que (packs × factor) sea múltiplo de N
        WHEN dr.multiplo_unidades IS NOT NULL
          THEN FLOOR(FLOOR(d.asignado_raw::numeric * d.factor / dr.multiplo_unidades)
...
GRANT EXECUTE ON FUNCTION get_pedido_preview(integer[], integer[]) TO authenticated;
