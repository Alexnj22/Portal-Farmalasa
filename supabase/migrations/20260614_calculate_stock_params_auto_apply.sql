-- calculate_stock_params v3
--   1. Excluye Bodega del recálculo global (p_erp_sucursal_id IS NULL = solo sucursales).
--      Bodega se auto-mantiene vía trigger + publish_stock_params.
--   2. Auto-aplica borradores con cambio ≤ 40% vs publicado,
--      y primeras asignaciones (publicado 0/0 → nuevo > 0).
--      Los cambios > 40% o que llevan a 0/0 quedan como borrador para revisión.
--   3. Retorna auto_applied + drafted en el JSON de resultado.
--
-- Limpieza de datos existentes:
--   - Filas con min_units=0 y max_units=0 se convierten a NULL (se muestran como --)
--   - Borradores 0/0 pendientes también se limpian

-- ── 1. Limpia 0/0 publicados existentes → NULL ───────────────────────────────
...
WHERE erp_sucursal_id != 6
  AND COALESCE(min_units, 0) = 0
  AND COALESCE(max_units, 0) = 0
  AND is_hidden IS NOT TRUE
  AND (
    draft_status IS DISTINCT FROM 'pending'
    OR (COALESCE(draft_min, 0) = 0 AND COALESCE(draft_max, 0) = 0)
  );

-- ── 2. Limpia borradores 0/0 pendientes ──────────────────────────────────────
UPDATE product_stock_params
SET
  draft_min           = NULL,
  draft_max           = NULL,
...
BEGIN
  ...
  -- Cuando se llama sin parámetro (cron mensual), Bodega queda excluida:
  -- sus valores los mantiene el trigger trg_bodega_draft_sync y publish_stock_params.
  IF p_erp_sucursal_id IS NULL OR p_erp_sucursal_id != 6 THEN
    ...
    -- ── Auto-apply: publica los borradores que califican ──────────────────────
    -- Califica si:
    --   a) publicado era 0/0 y el nuevo > 0  (primera asignación)
    --   b) publicado > 0 y cambio ≤ 40% en MIN y en MAX
    -- No califica si:
    --   - el nuevo valor sería 0/0  (pasa a borrador para confirmar)
    --   - cambio > 40% en alguno   (requiere revisión manual)
    WITH auto_apply AS (
      ...
      WHERE psp.draft_status = 'pending'
        ...
        AND (
          -- a) Primera asignación: publicado era 0/0, nuevo > 0
          (
            COALESCE(psp.min_units, 0) = 0
            AND COALESCE(psp.max_units, 0) = 0
            AND psp.draft_min > 0
          )
          OR
          -- b) Cambio estable ≤ 40% en MIN y en MAX
          (
            COALESCE(psp.min_units, 0) > 0
    ...
    SELECT
      erp_product_id, erp_sucursal_id, NOW(),
      min_units, max_units, daily_velocity, velocity_30d,
      abc_class, demand_variability, cv, calculated_at
    FROM auto_apply;
  ...
  -- ── Bodega: solo si se pide explícitamente (p_erp_sucursal_id = 6) ──────────
  -- Con NULL (cron mensual) se excluye: se auto-mantiene como Σ sucursales.
