-- get_pedido_preview v13: agrega p_use_portal_minmax boolean (DEFAULT false)
-- Cuando true, usa product_stock_params (Portal) en vez de erp_minmax para MIN/MAX.
-- Convierte min_units/max_units → packs dividiendo por el factor de presentación.
-- Si el producto no tiene entrada en product_stock_params, se filtra (max=0).
...
AS $function$
  ...
  -- Stock por sucursal con MIN/MAX seleccionable (ERP vs Portal)
  stock_sucursal AS (
    SELECT
      em.erp_sucursal_id, em.erp_product_id, em.erp_presentacion_id,
      -- MIN: ERP directo o Portal (units ÷ factor → packs)
      CASE
