-- get_pedido_preview v14: solo_cajas usa CEIL (cajas completas, redondear arriba)
-- v13 usaba FLOOR → enviaba menos cajas de lo necesario.
-- CEIL garantiza que se envíen suficientes cajas para cubrir la necesidad.
-- Cuando caja_factor IS NULL (sin presentación CAJA en product_precios), se
-- devuelve asignado_raw sin cambio (igual que antes — data issue, no code issue).
  ...
  con_reglas AS (
    SELECT d.*,
      ...
      CASE
        ...
        -- solo_cajas: convertir a cajas completas redondeando ARRIBA (CEIL).
        -- Solo aplica cuando la presentación actual NO ES caja (si ya es CAJA,
        -- asignado_raw ya está en cajas — no se necesita conversión).
        -- Si caja_factor IS NULL (producto sin presentación CAJA en product_precios),
        -- se devuelve asignado_raw sin cambio.
