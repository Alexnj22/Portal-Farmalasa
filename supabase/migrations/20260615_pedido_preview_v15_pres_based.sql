-- get_pedido_preview v15: regla de despacho por presentación seleccionada
-- Añade CTE dispatch_pres_factor que lee dispatch_id_presentacion + dispatch_multiplo
-- de dispatch_rules. Si esos campos están rellenos, usan la fórmula universal:
--   CEIL(asignado_raw * d.factor / (dp_factor * dp_multiplo)) * dp_factor * dp_multiplo / d.factor
-- Las reglas legacy (solo_cajas, multiplo, blister, multiplo_unidades) siguen
-- como fallback para reglas ya existentes que aún no se hayan migrado.
...
AS $function$
  ...
  -- v15: factor + multiplo de la presentación explícita elegida en la regla de despacho.
  -- Si el producto tiene duplicados en product_precios para esa presentación, toma el mayor factor.
  ...
  con_reglas AS (
    SELECT d.*,
      ...
      CASE
        ...
        -- v15: regla por presentación explícita (prioridad sobre legado)
