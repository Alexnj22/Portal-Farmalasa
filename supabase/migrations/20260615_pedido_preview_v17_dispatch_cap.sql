...
AS $function$
  ...
  -- Suma del asignado_raw por (producto, presentacion) para calcular cuánto sobrante hay.
  raw_totals AS (
  ...
  -- Legacy: factor de la presentación CAJA/BOLSA del producto (para regla solo_cajas)
  caja_factor_map AS (
  ...
  -- v15/v16: factor, multiplo y tipo de la presentación explícita elegida en la regla de despacho.
  ...
  -- v17: calcula cantidad sin cap para luego comparar contra max_asignable.
  con_reglas_uncapped AS (
    SELECT d.*,
      rt.total_raw_assigned,
      -- Máximo que esta sucursal puede tomar: su parte raw + su fracción proporcional del sobrante de bodega.
      ...
      CASE
        ...
        WHEN dpf.dp_factor IS NOT NULL
          THEN GREATEST(0, CEIL(
            d.asignado_raw::numeric * d.factor
            / NULLIF(dpf.dp_factor * dpf.dp_multiplo, 0)
          ) * dpf.dp_factor * dpf.dp_multiplo
          / NULLIF(d.factor, 0))::integer

        -- Legacy: solo_cajas (redondea arriba a cajas completas)
        WHEN COALESCE(dr.solo_cajas, false) = true
             AND dr.multiplo IS NULL AND dr.blister IS NULL AND dr.multiplo_unidades IS NULL
             AND d.presentacion_tipo != 'CAJA'
             AND cfm.caja_factor IS NOT NULL
          THEN GREATEST(0, CEIL(
            d.asignado_raw::numeric * d.factor
            / NULLIF(cfm.caja_factor, 0)
          ) * cfm.caja_factor / NULLIF(d.factor, 0))::integer

        -- Legacy: múltiplo de packs
        WHEN dr.multiplo IS NOT NULL
          THEN (CEIL(d.asignado_raw::numeric / dr.multiplo) * dr.multiplo)::integer

        -- Legacy: múltiplo de packs (blíster/sobre)
        WHEN dr.blister IS NOT NULL
          THEN (CEIL(d.asignado_raw::numeric / dr.blister) * dr.blister)::integer

        -- Legacy: múltiplo de unidades
        WHEN dr.multiplo_unidades IS NOT NULL
          THEN CEIL(CEIL(d.asignado_raw::numeric * d.factor / dr.multiplo_unidades)
               * dr.multiplo_unidades / NULLIF(d.factor, 0))::integer
  ...
  -- v17: aplica cap. Si asignado_uncapped > max_asignable: enviar asignado_raw
  -- usando presentación ERP (sin redondeo de despacho).
  con_reglas AS (
    SELECT
      ...
      -- Si el redondeo excede el disponible y había regla de pres explícita: volver a ERP pres.
      CASE
        WHEN cu.dp_factor IS NOT NULL AND cu.asignado_uncapped > cu.max_asignable
          THEN cu.presentacion_tipo
        ELSE cu.dp_tipo
      ...
      CASE
        WHEN cu.asignado_uncapped > cu.max_asignable
          THEN cu.asignado_raw     -- despachar el disponible real, sin redondear
        ELSE cu.asignado_uncapped
