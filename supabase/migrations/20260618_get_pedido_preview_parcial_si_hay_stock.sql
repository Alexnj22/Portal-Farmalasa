-- ============================================================
-- get_pedido_preview v3 — despacho parcial cuando hay stock
--
-- Bug: cuando bodega asignaba packs a una sucursal pero la fracción
-- no alcanzaba el 40% del dispatch_unit, el sistema bloqueaba
-- el despacho (asignado_final=0) aunque había inventario disponible.
--
-- Fix: en con_reglas se cambia el primer WHEN de
--   "asignado_uncapped <= max_asignable"
-- a
--   "asignado_uncapped > 0 AND asignado_uncapped <= max_asignable"
--
-- Ahora cuando la regla del 40% da asignado_uncapped=0, el ELSE
-- fallback envía asignado_raw como despacho parcial (con presentacion_tipo
-- en lugar de dispatch_tipo). revision_minmax queda solo para los casos
-- donde asignado_raw=0 (bodega genuinamente no pudo asignar nada).
-- ============================================================
...
AS $function$
  ...
  dispatch_pres_factor AS (
    SELECT DISTINCT ON (dr.erp_product_id)
      dr.erp_product_id,
      CASE
        WHEN dr.dispatch_label IS NOT NULL
          THEN pp.factor::numeric * COALESCE(dr.dispatch_multiplo, 1)::numeric
        ELSE pp.factor::numeric
      END AS dp_factor,
      CASE
        WHEN dr.dispatch_label IS NOT NULL THEN 1::numeric
        ELSE COALESCE(dr.dispatch_multiplo, 1)::numeric
      END AS dp_multiplo,
  ...
  -- Redondea asignado_raw a la unidad de despacho más cercana (↑ si fracción ≥ 40%).
  -- Si el resultado es 0 (fracción insuficiente), con_reglas lo convierte en parcial
  -- enviando asignado_raw directamente en vez de bloquear con cero.
  ...
  con_reglas AS (
    SELECT
      ...
      -- Usa dispatch_tipo/factor de la regla solo cuando el despacho es en unidades completas.
      -- Cuando asignado_uncapped=0 (fracción < 40%), cae al ELSE y usa presentacion_tipo/factor
      -- para marcar que es un despacho parcial fuera de la unidad mínima de despacho.
      CASE
      ...
      CASE
        ...
        ELSE cu.asignado_raw  -- despacho parcial: se envía lo que bodega asignó
