-- get_pedido_preview v23: split inv_dedup → inv_suc + inv_bodega + SET statement_timeout
-- Problema: inv_dedup leía TODA inventory (todas suc + bodega) con DISTINCT ON/ORDER BY
-- enorme → "canceling statement due to statement timeout" con 5+ sucursales.
-- Fix:
--   inv_suc  : DISTINCT ON filtrado a p_sucursal_ids (alimenta inv_agg)
--   inv_bodega: DISTINCT ON filtrado a bodega (alimenta bodega_raw + bodega_lotes_raw)
-- Además: SET statement_timeout = '120s' en la función como red de seguridad.
-- Stats y sin_bodega también reciben SET statement_timeout = '60s' via ALTER FUNCTION.
...
AS $function$
  ...
  -- Solo sucursales objetivo — evita escanear toda la tabla de inventario
  inv_suc AS (
    SELECT DISTINCT ON (
      erp_sucursal_id, erp_product_id, lote, fecha_vencimiento, is_vencidos,
      TRIM(LOWER(COALESCE(presentacion,''))), LOWER(COALESCE(detalle,''))
    )
      erp_sucursal_id, erp_product_id, is_vencidos,
      cantidad::numeric
        * COALESCE(NULLIF(split_part(LOWER(COALESCE(detalle,'')), 'x', 2), '')::numeric, 1)
        AS unidades
    FROM inventory
    WHERE erp_sucursal_id = ANY(p_sucursal_ids)
    ORDER BY
      erp_sucursal_id, erp_product_id, lote, fecha_vencimiento, is_vencidos,
      TRIM(LOWER(COALESCE(presentacion,''))), LOWER(COALESCE(detalle,''))
  ),

  -- Solo bodega — para asignación FEFO y disponibilidad
  inv_bodega AS (
    SELECT DISTINCT ON (
      erp_product_id, lote, fecha_vencimiento, is_vencidos,
      TRIM(LOWER(COALESCE(presentacion,''))), LOWER(COALESCE(detalle,''))
    )
      erp_product_id, lote, fecha_vencimiento, is_vencidos, cantidad, detalle,
      cantidad::numeric
        * COALESCE(NULLIF(split_part(LOWER(COALESCE(detalle,'')), 'x', 2), '')::numeric, 1)
        AS unidades
    FROM inventory
...
ALTER FUNCTION public.get_pedido_sucursal_stats(integer[]) SET statement_timeout = '60s';
ALTER FUNCTION public.get_pedido_sin_bodega(integer[])     SET statement_timeout = '60s';
