-- 7A.7 gap: get_stock_analysis leía dr.solo_cajas/dr.multiplo/dr.blister/dr.multiplo_unidades
-- (columnas viejas del modelo de dispatch_rules, las 845 filas tienen estas 4 en NULL/false/0)
-- en vez de dr.dispatch_id_presentacion/dispatch_multiplo/dispatch_label (el modelo real,
-- ya usado correctamente por get_pedido_preview y por el que escribe TabReglas.jsx).
-- Resultado: hasRule siempre false en TabMinMax, la celda Despacho nunca mostraba el número.
--
-- Fix: mismo patrón CTE `dispatch_pres_factor` que ya usa get_pedido_preview
-- (20260701_fix_pedido_preview_double_rounding_dispatch.sql) — JOIN a product_precios vía
-- dispatch_id_presentacion para obtener el factor real de la presentación de despacho,
-- combinado con dispatch_multiplo. Verificado: 845/845 filas con dispatch_id_presentacion
-- tienen match en product_precios (0 huérfanos), por eso el JOIN puede ser INNER dentro del CTE.
--
-- Cambio de contrato: dispatch_solo_cajas/dispatch_blister/dispatch_multiplo_unidades (siempre
-- NULL/false hoy, sin consumidor real) se reemplazan por dispatch_pres_factor (numeric) y
-- dispatch_tipo (text, label o tipo de presentación) — dispatch_multiplo se mantiene con el
-- mismo nombre pero ahora sourced de la columna nueva. El frontend (TabMinMax.jsx) se actualiza
-- en el mismo commit para consumir estos 3 campos.

SET lock_timeout = '5s';

DROP FUNCTION IF EXISTS public.get_stock_analysis(integer);

CREATE FUNCTION public.get_stock_analysis(p_erp_sucursal_id integer)
...
AS $function$
  ...
  -- Ventas en vivo desde sales_invoices; factor pre-calculado por trigger
  -- (antes: regexp_match por fila).
  live_sales AS (
    SELECT
      ii.erp_product_id,
  ...
  catalog_pres AS (
    ...
    FROM product_precios WHERE factor > 1
    GROUP BY product_id
  ),
...
REVOKE ALL ON FUNCTION public.get_stock_analysis(integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_stock_analysis(integer) TO anon, authenticated, service_role;
