...
-- Las líneas tienen que decir si fueron recontadas y cuánto decía el primer
-- conteo — sin eso el supervisor no puede ver si su recuento cambió algo, que
-- es justamente la métrica de calidad del conteo original.
-- Cambia el tipo de retorno, así que va DROP + CREATE: al ir en una sola
-- migración es atómico, no hay ventana sin función.

DROP FUNCTION IF EXISTS public.get_conteo_items_search(uuid, text, text, integer, integer, integer);

CREATE FUNCTION public.get_conteo_items_search(p_conteo_id uuid, p_search text DEFAULT NULL::text, p_filtro text DEFAULT 'TODOS'::text, p_limit integer DEFAULT 50, p_offset integer DEFAULT 0, p_erp_product_id integer DEFAULT NULL::integer)
...
-- El supervisor recuenta la plata primero: con el filtro de diferencias, los
-- productos salen ordenados por el valor absoluto de su desvío, no alfabéticos.
...
BEGIN
  ...
  per_product AS (
    SELECT
      ...
      COALESCE(SUM(ABS(COALESCE(w.diferencia,0)) * COALESCE(w.costo_unitario,0)),0) AS valor_dif_abs
    FROM with_live w
  ...
  ORDER BY
    CASE WHEN p_filtro = 'DIFERENCIA' THEN pp.valor_dif_abs END DESC NULLS LAST,
    pp.product_nombre
  LIMIT p_limit OFFSET p_offset;
END;
$function$;
...
-- El reporte del ajuste y el de resultados también muestran el recuento.
