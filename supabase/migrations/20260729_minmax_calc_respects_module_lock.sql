-- F0 (cierre) — calculate_stock_params respeta el candado de mantenimiento.
--
-- El candado de F0 (20260729_module_locks_maintenance_gate) vive dentro de
-- auth_can_edit_any(), que se evalua en RLS y en RPCs llamadas por personas.
-- service_role saltea RLS por completo, asi que el candado NO detiene crons.
-- Eso es intencional y esta documentado... con UNA excepcion que el plan
-- declaro y quedo sin aplicar: calculate_stock_params es la unica de las 23
-- RPCs que exime a service_role explicitamente, porque la llama el cron
-- auto-calculate-minmax el dia 1 de cada mes. Si alguien tiene MIN·MAX en
-- mantenimiento ese dia, el recalculo mensual reescribiria borradores y
-- min/max de todo el catalogo por debajo de quien esta trabajando.
--
-- Se agrega el chequeo de candado ANTES de todo, y aplica tambien a
-- service_role.
--
-- Array ARRAY['minmax','pedidos']: es exactamente el que usan psp_insert /
-- psp_update sobre product_stock_params. Si un humano no puede escribir la
-- tabla porque 'pedidos' esta bloqueado, el cron tampoco deberia. Sobre-bloqueo
-- deliberado, igual que en auth_module_locked (F0.2).
--
-- Para personas la respuesta es una EXCEPTION con el motivo real; hasta hoy
-- caian en el PERMISSION_DENIED generico ("se requiere permiso de edicion"),
-- que es mentira cuando el permiso existe y lo que hay es un candado.
-- Para el cron es { ok:false, skipped:true, reason:'module_locked' }, que es el
-- contrato que auto-calculate-minmax ya sabe leer y registrar en
-- minmax_sync_log como sucursal SALTADA (v2.206).
--
-- NOTA DE HISTORIAL: el cuerpo de la funcion que va abajo es el que estaba
-- vigente en ese momento; la migracion
-- 20260729_minmax_f2_historial_datadays_sparse (F2, el mismo dia) lo reemplaza
-- para agregarle data_days y el reset de sparse_data. Se deja fiel a proposito
-- para que reproducir las migraciones locales de cero de el mismo resultado.
...
BEGIN
  -- Candado de mantenimiento: aplica INCLUSO a service_role (ver cabecera).
