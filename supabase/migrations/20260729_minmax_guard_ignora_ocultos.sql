...
-- ─── El recálculo mensual llevaba desde junio sin correr ────────────────────
-- calculate_stock_params se salta la sucursal ENTERA si encuentra un solo
-- draft_status='pending'. Esa guarda no excluía los ocultos, y ahí estaba el
-- nudo:
--
--   1. Ocultar un producto escribía draft_min=0, draft_max=0, draft_status='pending'
--      (useMinMaxData.hideFiltered) — la propuesta de dejarlo en cero.
--   2. Esa propuesta NO se puede ver ni publicar desde la UI: la tabla excluye
--      los ocultos incondicionalmente, y el contador de borradores los saltea a
--      propósito para que el badge no se desincronice del filtro
--      (useMinMaxData.js:298 lo documenta).
--   3. calculate_stock_params tampoco los toca: su upsert lleva
--      `WHERE is_hidden IS NOT TRUE`.
--
-- Resultado: un pendiente invisible e inlimpiable que bloquea el recálculo de
-- toda su sucursal para siempre. Medido: 32 filas así en las 6 sucursales
-- (11 en La Popular), TODAS ocultas, cero visibles — servicios y no-inventario
-- (APLICACION DE INYECCION, SERVICIO A DOMICILIO, COMISIONES, DIETAS COFARSAL).
-- Los updated_at coinciden exactamente con los MINMAX_HIDE del 17-jul 18:09.
--
-- Acá va la línea que faltaba en la guarda. La causa de fondo —que ocultar
-- genere un borrador— se corrige en el frontend y en
-- 20260729_minmax_ocultos_en_menos_barra_menos.sql; esta guarda queda igual como
-- defensa: el borrador de un producto oculto no dice nada sobre si la sucursal
-- se puede recalcular.
