-- El cotejo del Corte Z, ahora con el diagnóstico de la diferencia.
--
-- El panel «Por qué difiere» sabía decir CUÁNTO y no POR QUÉ: mandaba a comparar
-- día por día contra el reporte del origen, a mano. El cuadre diario ya baja al
-- documento y guarda la causa en `ventas_cuadre_hallazgos`; esto la trae a la
-- misma fila del cotejo para que la respuesta esté donde está la pregunta.
--
-- Solo los hallazgos SIN resolver: un día que volvió a cuadrar no tiene por qué
-- seguir explicándose.
SET lock_timeout = '5s';
