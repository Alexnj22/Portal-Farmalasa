...
-- El bloque de Bodega de `publish_stock_params` escribía el par de BORRADOR
-- (draft_min/draft_max = Σ efectiva de sucursales) tal cual salía de la suma,
-- sin pasarlo por la misma escalera que sí aplica al par PUBLICADO. Y la Σ de
-- varias sucursales en 0·1 da 0·2, que `psp_draft_pair_valid` rechaza (con
-- MIN 0 el MAX solo puede ser 0 o 1). Medido antes del arreglo en La Popular:
-- de 1,234 filas de Bodega que iban a quedar con borrador, 109 violaban el
-- check — todas de la forma 0·N con N ≥ 2 — y una sola aborta el lote entero,
-- así que «Publicar todo» no publicaba nada.
--
-- El trigger `sync_bodega_draft_from_branch_stmt` ya lo hacía bien (CTEs
-- `clamped_min`/`clamped`): esta función traía una copia de esa lógica que
-- se quedó a mitad de camino. Se alinean las dos.
--
-- De paso, «¿hay borrador?» pasa a compararse entre pares SANEADOS. Con los
-- crudos, una Σ de 0·2 contra un publicado de 1·2 se leía como cambio cuando
-- las dos, saneadas, son 1·2 — o sea un borrador que no cambiaba nada.
