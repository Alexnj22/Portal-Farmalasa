...
--
-- Es la información que llena la pantalla vacía de Consulta de Inventario. La
-- razón real por la que alguien abre ese widget es que un cliente preguntó por
-- algo que no está; esto se lo adelanta.
--
-- Las tres condiciones son del usuario, y cada una descarta un falso positivo:
--   1. El producto está en el MIN/MAX de la sala. Sin eso saldría medio
--      catálogo: un producto que esa sala nunca vendió no le "falta".
--   2. La sala no tiene ni una unidad. Si tiene poco, es otra conversación.
--   3. La sala que lo tiene queda POR ENCIMA DE SU PROPIO MÍNIMO después de
--      ceder una. No se le saca el único que le queda para tapar el hueco de
--      otra: eso mueve el problema, no lo resuelve.
--
-- La existencia se cuenta en UNIDADES. `inventory` guarda una fila por
-- presentación, así que sumar en crudo mezcla cajas con unidades — un producto
-- con 1 caja de 100 daría 1. Se multiplica por el factor de su presentación.
...
AS $$
    ...
    SELECT a.erp_product_id,
           ...
           jsonb_agg(jsonb_build_object('sala', a.sala, 'unidades', a.unidades)
