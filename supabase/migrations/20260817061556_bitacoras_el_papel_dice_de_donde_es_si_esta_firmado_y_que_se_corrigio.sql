...
-- Tres cosas que al papel le faltaban y en pantalla no se notan.
--
-- 1) La DIRECCION del establecimiento. La bitacora es del local, no de la
--    empresa: un inspector empieza por verificar que la hoja que tiene en la
--    mano es de la sala en la que esta parado.
-- 2) Si el mes esta CERRADO. Sin eso, el borrador de un mes a medio llenar sale
--    con la misma cara que el mes firmado, y en papel esa diferencia ya no se
--    puede recuperar.
-- 3) Cuantas CORRECCIONES tiene cada lectura. El item 3.7 pide control de
--    correcciones y ALCOA pide que el dato sea «original»: un valor corregido
--    impreso igual que uno de primera mano borra justo lo que ese control
--    existe para mostrar.
...
REVOKE EXECUTE ON FUNCTION public.get_bitacora_mes_impreso(bigint, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_bitacora_mes_impreso(bigint, text) TO authenticated, service_role;
