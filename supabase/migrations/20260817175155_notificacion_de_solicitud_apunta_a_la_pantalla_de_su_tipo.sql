-- La pantalla del aviso sale del TIPO de solicitud, no de un valor fijo.
--
-- `v_base` arrancaba en '/requests' —el centro de la SALA— y sólo el cambio de
-- turno de nivel 1 lo corregía. O sea que vacaciones, permiso, incapacidad,
-- anticipo, constancia, horas extra, excepción de turno y cambio de vacaciones
-- mandaban a una pantalla donde esos tipos NI SE LISTAN: la vista filtra por
-- ámbito, así que el enlace `?solicitud=` no encontraba nada, no abría nada y
-- no dejaba error. El aviso llegaba y moría ahí.
--
-- Le pega justo a Talento Humano, que es quien decide todo lo personal. No se
-- había ejercido porque hoy no hay ni una solicitud personal en la tabla — la
-- primera que entre se lleva el enlace muerto.
--
-- El único cambio respecto de la versión viva es ése: la declaración de
-- `v_base`, su asignación derivada de `es_solicitud_operativa()`, y la línea
-- del cambio de turno que queda redundante. El resto del cuerpo es idéntico
-- byte a byte (verificado por md5 contra prod antes de aplicar:
-- fbd031d1f19c122f454939d942c7b0ef).
SET lock_timeout = '5s';
...
BEGIN
    ...
    -- La pantalla, para TODOS los tipos y de una sola vez. Antes esto vivía
    -- como valor por defecto de la declaración, y sólo una rama lo corregía.
    ...
    ELSIF NEW.type = 'SHIFT_CHANGE' AND NEW.current_level = 1 THEN
        ...
        -- Acá vivía `v_base := '/requests-personales'`. Ya no hace falta: la
        -- decide `es_solicitud_operativa` arriba, para TODOS los tipos y no
        -- sólo para éste. (Era `/my-requests` hasta el 2026-08-11.)
