...
-- Las tres funciones que filtran la vista materializada dejan de normalizar
-- fila por fila y leen la columna que la vista ya guarda (ver la migración
-- `la_vista_de_inventario_guarda_la_descripcion_ya_normalizada`).
--
-- El PATH C de `inventory_grouped` NO cambia: ese barre `inventory` en crudo
-- —los vencidos por fecha tienen otra semántica y no salen de la vista— así que
-- ahí no hay columna precalculada que leer.
...
IF p_area_vencidos THEN
    ...
    WHERE m.vencidos_unidades > 0
      ...
      AND (v_pats IS NULL OR m.descripcion_norm LIKE ALL (v_pats));
...
AS $function$
    ...
    WHERE m.soonest_active_venc IS NOT NULL
      ...
      AND (p_search IS NULL OR p_search = ''
           OR m.descripcion_norm LIKE ALL (
