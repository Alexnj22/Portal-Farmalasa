-- Fase 3 de docs/PLAN-MINMAX-AJUSTE-A-MANO-2026-08-20.md — publicar deja de
-- pisar lo que puso una persona.
--
-- Hasta hoy publicar era un barrido: copiaba draft_min/draft_max sobre
-- min_units/max_units sin mirar si esa fila la había tocado alguien. Medido
-- contra producción el 2026-08-20: de 969 pares producto·sala ajustados a mano
-- antes del recálculo del 1-ago y nunca vueltos a tocar, 567 (59%) ya no
-- tienen el valor que se les puso.
--
-- La línea es la del acto deliberado: el BARRIDO (sin p_erp_product_ids) deja
-- quietas las filas con `manual_at`; la publicación DIRIGIDA a productos
-- concretos sí las pisa, porque alguien los eligió uno por uno — el frontend ya
-- distingue los dos casos (`scope: 'selective' | 'all'`).
--
-- Y devuelve `omitidas_por_ajuste_manual`: una publicación que calla lo que no
-- hizo se lee como «publicó todo».
--
-- Probado en staging con los tres casos antes de aplicar acá. Cambia SÓLO el
-- predicado de par_ordenado, el conteo previo y el jsonb de salida; el resto
-- del cuerpo es idéntico al que estaba.
--
-- El cuerpo completo vivo se lee con:
--   SELECT pg_get_functiondef('public.publish_stock_params(integer,integer[],text)'::regprocedure);
...
BEGIN
  ...
  -- Cuántas deja quietas el barrido por tener ajuste de una persona. Se cuenta
  ...
  WITH par_ordenado AS (
    ...
    WHERE psp.draft_status     = 'pending'
      ...
      -- El BARRIDO no pisa lo que puso una persona; la publicación DIRIGIDA a
      -- productos concretos sí, porque alguien los eligió uno por uno.
      AND (p_erp_product_ids IS NOT NULL OR psp.manual_at IS NULL)
  ),
  con_min AS (
