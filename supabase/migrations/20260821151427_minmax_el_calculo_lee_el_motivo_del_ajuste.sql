-- Fase 5 de docs/PLAN-MINMAX-AJUSTE-A-MANO-2026-08-20.md — el cálculo lee el
-- motivo. Es la única fase que puede mover un número.
--
-- Cuatro cambios, y los tres primeros están guardados por
-- `manual_motivo IS NOT NULL`: con cero filas con motivo —que es el estado de
-- hoy— el resultado es idéntico al anterior por construcción.
--
--  1. «ya no rota»  → la venta ANTERIOR a la fecha del ajuste deja de contar.
--                     Medido contra datos reales con la construcción exacta:
--                     el producto con ajuste pasó de 175 días de venta a 30, y
--                     el de al lado sin ajuste quedó en 165 → 165.
--  2. «cliente fijo» → la velocidad sale del ritmo declarado (20 cada 60 días),
--                     no del promedio de 180 días que lo winsoriza como pico.
--  3. «lo buscan»   → el número puesto es PISO. Va sólo al BORRADOR, no a
--                     calc_min/calc_max: esas dos son la propuesta PURA del
--                     cálculo, y es contra ellas que la pantalla decide si un
--                     ajuste está «En conflicto». Contaminarlas dejaría ciego
--                     ese detector.
--  4. El AUTO-APLICAR deja de pisar lo que puso una persona. Era la cuarta
--     puerta y la peor: escribe min_units/max_units sin pasar por
--     publish_stock_params —así que el freno que se le puso allá no lo
--     alcanzaba— y corre sola el día 1 sin que nadie lo decida.
--
-- El LEFT JOIN a `ajustes` en `daily` no es un NOT EXISTS a propósito:
-- `ajustes` tiene pocas filas y entra por hash, mientras que un EXISTS
-- correlacionado fijaría la dirección del join contra las 180,000 facturas del
-- año (CLAUDE.md, medido el 2026-08-17).
--
-- VERIFICACIÓN (2026-08-21): corrida completa sobre La Popular en producción,
-- dentro de una transacción revertida — 1,893 filas en 4,694 ms, 0 borradores
-- mal formados, y draft_min = calc_min en TODAS, que es justo lo que tiene que
-- pasar mientras no haya motivos declarados.
--
-- Lo que NO se pudo medir: la equivalencia contra la versión anterior sobre el
-- catálogo entero. Staging no sirve —devuelve rows: 0, no tiene ventas en la
-- ventana— y correr las dos versiones en producción para compararlas es el
-- trabajo del cron por duplicado. La garantía es estructural: las tres reglas
-- de demanda están guardadas por `manual_motivo IS NOT NULL` y hoy no hay ni
-- una fila con motivo.
