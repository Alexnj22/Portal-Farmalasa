-- get_product_sales_agg — se le quita un filtro que no filtraba, y sus arreglos
-- jsonb salen ordenados.
--
-- 1. `last_sale_live` acotaba con `erp_product_id IN (SELECT ... FROM all_cands)`
--    y no descartaba NI UNA fila: todo producto vendido este mes ya es candidato
--    por construcción. El semi-join costaba 155 ms de los 535 (206 ms → 51 ms
--    medidos sobre el bloque suelto, mismas 6,367 filas) y las filas de más las
--    descarta sola la unión externa del final.
--    Medido idéntico —15,167 filas, 13 columnas, 7 combinaciones de parámetros—
--    contra la versión vigente: mes en curso 535 → 415 ms, mes cerrado
--    447 → 290, con sucursal 471 → 254, tres meses 668 → 481.
--
-- 2. `presentaciones` y `ultima_venta_por_suc` se armaban SIN desempate, así que
--    dos cargas del mismo período devolvían los mismos datos en otro orden: las
--    píldoras de presentación y el globo de «6 suc.» se reacomodaban solos.
--    Es la ÚNICA diferencia que apareció al comparar la versión nueva contra la
--    vigente — y aparece igual comparando la vigente contra sí misma, que es lo
--    que prueba que era desorden y no un cambio de dato.
SET lock_timeout = '5s';
...
RETURNS TABLE(erp_product_id integer, descripcion text, cantidad numeric, neto numeric,
    ...
    oculto_por_first_names text, oculto_por_last_names text, oculto_at timestamptz)
