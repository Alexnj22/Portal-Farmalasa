-- Ventas > Productos: buscar por código de barras.
--
-- Es el caso (b) del razonamiento de `productos_por_codigo`: esta función NO
-- busca sobre `products`, busca sobre el TEXTO DE LA FACTURA
-- (`sales_invoice_items.descripcion`, 548K+ filas, más el agregado mensual).
-- Meterle un join a `products` para leer una columna le cambiaría el plan a la
-- consulta más pesada de Ventas.
--
-- Por eso el código se resuelve a ids en un CTE MATERIALIZED —una vez, sobre
-- 4,400 filas— y los cuatro predicados sólo agregan una pertenencia sobre
-- `erp_product_id`. El quinto, el de los productos que nunca se vendieron, sí
-- lee `products` y ahí va la columna directo.
--
-- ⚠️ Esta migración quedó SUPERADA el mismo día por
-- `20260822181344_ventas_productos_la_busqueda_se_resuelve_sobre_products`:
-- al medirla se vio que el problema no era el código de barras sino QUÉ se
-- buscaba, y toda la búsqueda pasó a resolverse sobre `products`. Se conserva
-- porque el registro de producción la tiene: el archivo es el acta de lo que
-- se aplicó, no de lo que quedó.
SET lock_timeout = '5s';
...
AS $function$
WITH
-- El código de barras resuelto a ids, UNA vez y sobre las ~4,400 filas de
-- `products`. Los buscadores de abajo miran el texto de la FACTURA
-- (`sales_invoice_items.descripcion`, 548K+ filas): meterles un join a
-- `products` para leer una columna les cambiaría el plan. Con esto sólo
-- agregan una pertenencia sobre `erp_product_id`, que ya tiene índice.
-- MATERIALIZED para que sea una vez y no por fila.
cods AS MATERIALIZED (
  SELECT unnest(public.productos_por_codigo(p_search)) AS id
),
branch_esid AS (
  ...
  FROM (
    ...
    WHERE si.fecha BETWEEN b.pl_from AND b.pl_to
      ...
      AND (p_search IS NULL OR p_search = '' OR public.norm_search(sii.descripcion) LIKE ALL (
           ...
           OR sii.erp_product_id IN (SELECT c.id FROM cods c))
    UNION ALL
...
pres_past AS (
  ...
  WHERE p_fini < b.curr_month
    ...
    AND (p_search IS NULL OR p_search = '' OR public.norm_search(a.descripcion) LIKE ALL (
          ARRAY(SELECT '%'||tok||'%' FROM unnest(string_to_array(public.norm_search(p_search),' ')) tok WHERE tok <> ''))
         OR a.erp_product_id IN (SELECT c.id FROM cods c))
...
zero_sale_cands AS (
  ...
  WHERE pr.activo = true
    AND (p_search IS NULL OR p_search = '' OR public.norm_search(pr.nombre) LIKE ALL (
          ARRAY(SELECT '%'||tok||'%' FROM unnest(string_to_array(public.norm_search(p_search),' ')) tok WHERE tok <> ''))
         OR public.norm_search(pr.codigo_barras) LIKE ALL (
          ARRAY(SELECT '%'||tok||'%' FROM unnest(string_to_array(public.norm_search(p_search),' ')) tok WHERE tok <> '')))
