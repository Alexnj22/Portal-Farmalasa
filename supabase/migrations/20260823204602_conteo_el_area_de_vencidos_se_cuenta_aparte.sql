-- Conteo de inventario: el área de vencidos se cuenta APARTE.
--
-- Pedido del usuario el 2026-08-22: «debes separar los productos de bodega de
-- vencidos, debe salir como una tabla aparte o al final de la tabla, no se debe
-- mezclar con la bodega normal».
--
-- Y al medirlo resultó ser más que un problema de orden. En el conteo abierto:
--
--   productos en el área de vencidos          83
--   productos en la bodega normal          2,759
--   productos que están en LAS DOS            42   ← acá estaba el daño
--
-- El agrupador era `GROUP BY erp_product_id` a secas, así que esos 42 productos
-- salían en UNA sola fila y su «Sistema / Físico / Dif.» **sumaba las dos
-- áreas**. No era sólo que se vieran mezclados: el total de esos productos
-- estaba mal, y sin ninguna señal — la fila se pintaba igual de bien.
--
-- Verificado después de aplicar: 2,759 (NORMAL) + 83 (VENCIDOS) = 2,842 contra
-- los 2,800 que devolvía sin separar. La diferencia son exactamente los 42.
--
-- La separación es POR RENGLÓN y no por producto, porque el área de vencidos es
-- otro anaquel: un producto puede estar en los dos y hay que contarlo dos
-- veces, una en cada lugar, con sus propios números.
--
-- `p_area` acepta 'NORMAL', 'VENCIDOS' o NULL. NULL es «las dos juntas», que es
-- exactamente lo que hacían hasta hoy: quien no pase el parámetro no cambia de
-- comportamiento.
--
-- ⚠️ Las tres se ELIMINAN antes de recrearse. Un parámetro nuevo con valor por
-- defecto NO reemplaza la firma vieja: la deja como sobrecarga, y una llamada
-- de tres argumentos seguiría entrando por la puerta equivocada. Es la misma
-- razón por la que `crear_conteo_inventario` se eliminó al ganar
-- `p_fuente_sistema` (20260807214106). Verificado: 0 sobrecargas viejas.
SET lock_timeout = '5s';

DROP FUNCTION IF EXISTS public.get_conteo_products_page(uuid, text, text, integer, integer, integer, text, text);
DROP FUNCTION IF EXISTS public.get_conteo_products_count(uuid, text, text, integer);
...
REVOKE EXECUTE ON FUNCTION public.get_conteo_products_page(uuid, text, text, integer, integer, integer, text, text, text) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.get_conteo_products_page(uuid, text, text, integer, integer, integer, text, text, text) TO authenticated, service_role;
