...
-- contraria.
--
-- ⚠ ESTA VERSIÓN NACIÓ CON UN DEFECTO. Su filtro decía
-- `coalesce(tipo_ficha,'PERSONA')='PERSONA'` y ese valor NO EXISTE. La corrige
-- la migración 20260827190602, que además explica cómo se encontró. Se archiva
-- tal cual porque el registro de producción la tiene.
    ...
    FOR v_emp IN
        SELECT e.id, e.name, e.dui_fecha_vencimiento AS vence
         ...
         WHERE e.status = 'ACTIVO'
           ...
           AND coalesce(e.tipo_ficha, 'PERSONA') = 'PERSONA'
...
COMMENT ON FUNCTION public.avisar_dui_por_vencer(int) IS
  'Avisa a cada persona de su DUI por vencer (una vez por etapa) y a Talento Humano con UN resumen diario. La etapa vive en metadata: el texto cambia, la clave no.';
