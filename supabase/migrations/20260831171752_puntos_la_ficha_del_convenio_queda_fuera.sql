...
-- Cuarta condición: la ficha del cliente tiene que acumular. Va en el `inv` de
-- arriba —junto al monto y al estado— y no al final, porque descarta la factura
-- entera antes de mirarle un solo renglón.
--
-- `coalesce(..., true)`: una venta sin ficha acumula. Es la misma falla segura
-- que el resto de la función — lo que no está clasificado no le puede costar
-- puntos a nadie.
