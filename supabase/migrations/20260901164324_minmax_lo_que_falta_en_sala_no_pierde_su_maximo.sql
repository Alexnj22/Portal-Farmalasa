-- El quiebre le bajaba el numero al producto que faltaba.
--
-- Si un producto no esta en la sala, no vende. El calculo lee esos dias sin
-- venta como demanda que cayo y recorta el maximo; con el maximo mas bajo se
-- pide menos, y el producto vuelve a faltar antes y por mas tiempo. Es un
-- circulo, y el modo de falla es el silencio: no hay error, no falta ninguna
-- fila, y el numero nuevo se ve igual de bien que el viejo.
--
-- Medido contra produccion el 2026-09-01:
--
--   pares con minimo definido y stock en CERO                505
--   de esos, con venta en los ultimos 90 dias                377
--   de esos, sin venta hace 90 dias (descontinuados)         128
--   se les BAJO el maximo el mismo dia estando en cero        80
--     de los cuales se vendian hace poco                      62
--   unidades de maximo quitadas a productos que faltaban     409
--
-- La guarda frena SOLO cuando se dan las tres cosas juntas:
--
--   1. el cambio BAJA el par — subirle el numero a algo que falta esta bien;
--   2. no hay NADA de ese producto en la sala;
--   3. el producto SI se vendia hace poco.
--
-- La tercera es la que evita el defecto opuesto. Sin ella se congelaria tambien
-- lo descontinuado, que tiene stock cero y no vende por la razon correcta: de
-- los 505 en quiebre, 128 no venden hace 90 dias y a esos bajarles el numero es
-- lo que corresponde.
--
-- `product_last_sale` se verifico contra las ventas reales ANTES de confiarle
-- la decision: de 13,599 pares, 13,574 coinciden al dia exacto, 6 faltan, 19
-- estan mas adelantados que la venta real (son las anuladas, que su trigger
-- todavia cuenta) y solo 5 discrepan en el corte de los 90 dias.
--
-- Lo frenado NO se pierde: queda como borrador y aparece en la lista de
-- revision. Son 62 filas mas en esa lista — el precio de no cerrar el circulo
-- solo. Y esto es un PARCHE al efecto, no al calculo: mientras no se guarde
-- historial de existencias, los dias de quiebre siguen contando como demanda
-- cero dentro de la velocidad. El arreglo de fondo es excluir esos dias del
-- denominador, y para eso hay que empezar a registrar el stock diario.
--
-- VERIFICADO corriendo el calculo completo de La Popular en produccion, dentro
-- de una transaccion revertida: 1,885 filas, 159 auto-aplicadas, 316 pares en
-- quiebre con venta reciente, **0 se les bajo el par** y 10 quedaron frenados a
-- revision. Sin la guarda, esos 10 habrian entrado solos.
--
-- El resto del cuerpo es identico a 20260901162649.
