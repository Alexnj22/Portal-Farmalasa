-- Una venta de mayoreo dejaba de contar como si fuera demanda del mostrador.
--
-- El calculo agrupaba las ventas por dia de una vez, y con eso perdia la unica
-- forma de distinguir «veinte personas llevaron una» de «un cliente llevo
-- veinte»: el TAMAÑO de cada venta. Los dos casos se ven identicos una vez
-- sumados, y el segundo no describe lo que hay que tener en vitrina.
--
-- El caso que lo destapo: DICLOFENACO SODICO 50MG X 100 CAP. ROJA SAIMED, en
-- Salud 3, el 26-ago-2026. UNA factura de 26 cajas — 2,600 unidades — que es el
-- 78% de todo lo que ese producto vendio en agosto en las seis salas juntas.
-- La venta normal de mostrador de ese producto son 20 capsulas por un dolar.
-- El calculo proponia 172·243 contra un vigente de 11·16: quince veces.
--
-- Ahora una venta QUINCE veces mas grande que la normal de ese producto en esa
-- sala no entra al calculo de la demanda. La venta existe, se factura y sale en
-- los reportes; lo unico que cambia es que no decide cuanto stock tener.
--
-- POR QUE 15 Y NO 5 NI 10. Se midieron los tres sobre el catalogo entero:
--
--   umbral   renglones excluidos   pares que bajan   costo del MAX
--     5x            5,851               1,313           -3.6%
--    10x            2,543                 687           -1.7%
--    15x              738                 302           -0.8%
--
-- Los tres arreglan Salud 3 exactamente igual (172·243 -> 12·18). La diferencia
-- es que 5x y 10x ADEMAS le bajan el numero a Salud 2 (98·139 -> 73·103) y a La
-- Popular (92·130 -> 77·109), donde vender de a cajas puede ser demanda real y
-- repetida. Se eligio el mas conservador que resuelve el caso — y la auditoria
-- del 1-sep pesa en esa eleccion: medido contra agosto, el maximo ya se queda
-- CORTO en el 20% de los pares, asi que el error caro no es tener de mas.
--
-- POR QUE LA MEDIANA ES POR SALA Y NO POR PRODUCTO. Hay salas que atienden
-- clinicas y ahi vender de a cajas ES lo normal: en el mismo DICLOFENACO, La
-- Popular tiene mediana 20 donde las demas tienen 10. Con la mediana global se
-- le recortarian ventas que para ella son corrientes. Ademas sale gratis: se
-- calcula dentro del mismo barrido que ya esta filtrado por sucursal.
--
-- Medido con la variante final (15x, mediana por producto y sala):
--   703 renglones excluidos de 227,374            (0.3%)
--   291 pares producto·sala bajan de 13,365       (2.2%)
--   0 pares se quedan sin ninguna venta
--   costo de llenar el MAX de las 6 salas: -0.63%
--
-- VERIFICADO corriendo el calculo completo de Salud 3 en produccion, dentro de
-- una transaccion revertida: 2,654 filas, 220 auto-aplicadas, y el DICLOFENACO
-- que proponia 172·243 ahora propone 12·18. GLUDETHON sigue en 11·17, intacto.
--
-- LO QUE ESTA REGLA NO CAZA, y hay que saberlo: la PROMOCION. GLUDETHON AR
-- vendio 214 unidades en agosto en 204 facturas —una unidad por persona, en las
-- seis salas a la vez— y NINGUN umbral lo toca, porque no hay ninguna venta
-- grande que excluir. Su propuesta sigue siendo 11·17 contra 2·4. Eso necesita
-- una lista de campañas declarada a mano, que es otra decision.
--
-- El resto del cuerpo es identico a 20260901164324.
