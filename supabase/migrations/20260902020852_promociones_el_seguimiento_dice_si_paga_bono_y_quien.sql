-- El seguimiento no decía si un producto paga bono ni quién lo cancela.
--
-- Desde que el bono es opcional (20260901220118), un renglón marcado «sólo mide»
-- se veía en Seguimiento con sus columnas de dinero en **$0.00** — exactamente
-- igual que uno que sí paga pero todavía no vendió nada. Son dos situaciones
-- distintas que se leían idénticas: una es «no aplica» y la otra es «todavía
-- no», y confundirlas es cómo alguien termina esperando un pago que nunca se
-- acordó.
--
-- Y faltaba el dato que hace falta para COBRAR: cuando el bono lo paga un
-- proveedor, hay que saber cuál. Estaba en la base desde que se creó la columna
-- y la pantalla no podía verlo.
--
-- El cuerpo es idéntico al de 20260901200526; lo único que cambia son tres
-- campos en la lista de renglones y el LEFT JOIN que trae el nombre del
-- proveedor.
...
COMMENT ON FUNCTION public.get_promocion(bigint) IS
  'El detalle de una promoción. Dice si cada producto paga bono y quién lo cancela: sin eso, «sólo mide» y «todavía no vendió» se leen iguales — los dos en $0.00. `costo_vendedor` cuenta SÓLO lo que tiene a quién pagarse; los fondos sí incluyen lo que no tiene dueño, porque son del área.';
