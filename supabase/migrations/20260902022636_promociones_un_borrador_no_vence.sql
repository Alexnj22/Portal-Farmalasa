-- Un borrador no está corriendo, así que nada suyo debería vencer.
--
-- El paso que cierra por fecha filtraba sólo `r.estado = 'abierto' AND r.fin <
-- v_hoy`, sin mirar el estado de la PROMOCIÓN. Con eso, cargar una promoción
-- vieja —el caso de llenar el histórico: se guarda, se revisa, se activa al día
-- siguiente— dejaba sus renglones CERRADOS antes de que nadie la activara.
--
-- El daño no se ve en los números: `get_promocion` no filtra por estado del
-- renglón, así que el seguimiento sigue mostrando todo bien. Lo que se pierde
-- es la cola: `promociones_registrar_excedentes` sólo abre excedentes de
-- promociones que no están en borrador, y para cuando se activa, el renglón ya
-- está cerrado y `promocion_avance` tampoco lo mira. **La cola queda vacía para
-- siempre y nadie se entera**, porque la pantalla se ve correcta.
--
-- Medido: `QA-BORRADOR-VIEJO`, promoción en borrador con vigencia de julio,
-- salió del ciclo con su renglón en `cerrado / fin_de_vigencia` y 0 excedentes.
--
-- El resto del cuerpo es idéntico a 20260901215738.
...
BEGIN
    ...
    -- El `pm.estado <> 'borrador'` es la corrección: un borrador no corre, así
    -- que su vigencia no vence. Sin esto, cargar una promoción vieja para tener
    -- el histórico la dejaba cerrada antes de activarla, y su cola de
    -- excedentes ya no se abría nunca.
    FOR v_fila IN
...
COMMENT ON FUNCTION public.promociones_ciclo_diario() IS
  'Abre la cola de excedentes, cierra los renglones que terminaron (lote agotado o fin de vigencia) SALVO los de promociones en borrador, finaliza la promoción cuando cerró el último, y avisa al 80% y al 100% del lote de cada sala diciendo dónde sí hay.';
