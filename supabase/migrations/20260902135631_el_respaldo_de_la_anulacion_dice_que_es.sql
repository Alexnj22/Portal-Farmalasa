...
-- Un documento anulado puede tener DOS respaldos —el PDF con el sello y el
-- JSON del evento, con su propio sello de recepción del MH— y hasta hoy
-- `invalidacion_source` traía «el último resuelto» sin decir cuál era. Con dos
-- filas emparejadas, el botón de la lista podía terminar ofreciendo el JSON
-- bajo el rótulo «Ver el PDF que justificó la anulación».
--
-- Sigue devolviendo UNO solo a propósito: es el botón de una fila de tabla, y
-- ahí lo que se quiere es la hoja con el sello. Se agrega `kind` para que el
-- rótulo pueda decir la verdad, y el orden ahora prefiere el PDF en vez de
-- depender de cuál se resolvió último. La lista completa la da
-- `get_purchase_dte_review_source`, que es lo que abre el detalle.
--
-- Los DEFAULT de los parámetros se repiten tal cual: sin ellos Postgres
-- rechaza el REPLACE («cannot remove parameter defaults»), y quitarlos habría
-- roto a cualquiera que llame la función sin argumentos.
...
AS $function$
  ...
  FROM (
    SELECT
      ...
      (
        SELECT json_build_object('id', rq.id, 'kind', rq.kind, 'file_path', rq.file_path, 'filename', rq.filename)
        ...
        ORDER BY (rq.kind = 'orphan_pdf') DESC, rq.resolved_at DESC
        LIMIT 1
