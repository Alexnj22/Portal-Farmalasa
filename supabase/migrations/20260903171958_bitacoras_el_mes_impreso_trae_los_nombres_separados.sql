...
-- El mes impreso trae los nombres SEPARADOS de quien anotó.
--
-- El papel dice «primer nombre + primer apellido» (pedido del usuario,
-- 2026-09-03: «solo nombre y apellido»), y eso NO se puede sacar partiendo
-- `employees.name`, que es una columna generada: «DOLORES CONCEPCION TEJADA
-- HERNANDEZ» cortada a dos palabras da «DOLORES CONCEPCION», que son dos
-- nombres y ningún apellido. Es la misma razón por la que el día ya traía
-- `first_names`/`last_names` para la firma en pantalla.
--
-- `name` se sigue mandando: es el respaldo de `shortEmployeeName` para las
-- fichas viejas que no tienen las dos columnas cargadas.
                    ...
                    SELECT coalesce(json_agg(json_build_object(
                        ...
                        'lecturas', (
                            SELECT coalesce(json_agg(json_build_object(
                                ...
                                'por', e2.name,
                                'por_nombres', e2.first_names, 'por_apellidos', e2.last_names,
                                'tarde', l.tarde,
                        ...
                        'limpiezas', (
                            SELECT coalesce(json_agg(json_build_object(
                                ...
                                'por_nombres', e3.first_names, 'por_apellidos', e3.last_names,
