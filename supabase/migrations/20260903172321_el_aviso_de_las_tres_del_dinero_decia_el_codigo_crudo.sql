...
/* ── Y el aviso decía el código crudo ──────────────────────────────────────
 *
 * La otra mitad de `20260903172223`. `v_etiqueta` no tenía caso para los tres
 * tipos de caja, así que caía a `ELSE NEW.type` y el cuerpo genérico habría
 * dicho «Solicitud de ABONO_APROBACION de Fulano espera tu decisión». Eso es
 * jerga del sistema en una pantalla, que es justo lo que la regla del portal
 * prohíbe: se habla en términos del negocio.
 *
 * Y cada una lleva su cuerpo propio con el MONTO, que es el dato que decide si
 * uno abre la campana ahora o después. Un aviso que no dice de cuánto es obliga
 * a entrar para averiguar lo único que importa. Sale de `monto_actual`/`monto`
 * y no de `total`: esa clave la escriben los tipos de inventario y acá no
 * existe, así que `v_monto` es NULL para las tres.
 */
...
BEGIN
    ...
    ELSIF NEW.type = 'ABONO_APROBACION' THEN
        ...
        v_cuerpo := v_quien || ' cobró ' || v_plata
                 ...
                 -- Se dice que el dinero YA entró: sin eso el aviso se lee como
                 -- un permiso previo, y quien decide creería que frenar es
                 -- gratis. Frenar acá es deshacer un abono aplicado.
