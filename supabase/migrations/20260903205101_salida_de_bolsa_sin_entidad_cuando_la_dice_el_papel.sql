...
-- ── LA ENTIDAD SE EXIGE SÓLO CUANDO LA CONTESTA UNA PERSONA ──────────────────
--
-- `bolsas_tipos_salida.entidad_la_dice_el_papel` existe desde el 2026-09-02
-- (v2.957.0): dice que la entidad NO se pregunta porque sale de la boleta. El
-- formulario lo respeta —`SalidaDeBolsa` no dibuja el campo ni frena por él—,
-- pero esta función se quedó con la regla vieja:
--
--     IF t.etiqueta_entidad IS NOT NULL AND v_entidad IS NULL THEN
--         RAISE EXCEPTION 'Falta el dato: %.', t.etiqueta_entidad;
--
-- Y «POS Promerica» tiene `etiqueta_entidad = 'Remesadora'`, así que toda
-- salida del POS que no fuera una remesa quedaba imposible de registrar por
-- acá. Un retiro por token no tiene remesadora ninguna: el papel no nombra
-- ninguna red y no dice «remesa» en ninguna parte.
--
-- ── Por qué sobrevivió tres días ────────────────────────────────────────────
--
-- Porque los dos caminos de una salida no pasan por el mismo código. Cuando el
-- cajón tiene el efectivo, la salida es un movimiento de caja y la escribe
-- `operar-caja`, que nunca llama acá: ese camino funcionaba. Esta función sólo
-- entra cuando el dinero sale de una BOLSA, o sea justo cuando el cajón no
-- alcanza — y el cajón no alcanza justo después de un corte, que se lleva todo
-- el efectivo a la bolsa del día.
--
-- Medido el 2026-09-03 en Salud 3: el corte de las 13:01 se llevó $331.27 y
-- dejó el cajón en $30.93. A las 13:09 y 13:10 dos intentos de registrar un
-- retiro por token murieron contra «Falta el dato: Remesadora.», mientras la
-- misma operación de las 11:52 —$120.79, con el cajón lleno— se había
-- registrado sin problema.
--
-- Es [[feedback_el_arreglo_de_un_canonico_no_llega_a_su_gemelo]] sobre una
-- regla de negocio: se corrigió en el formulario y la base siguió diciendo lo
-- contrario, sin que nada los enfrentara.
--
-- ── Y la comprobación contra el catálogo también miraba de más ──────────────
--
-- Con `v_entidad` en NULL, `upper(btrim(e.nombre)) = upper(NULL)` no empareja
-- con nada y `v_del_catalogo` queda NULL, así que la guarda siguiente lanzaba
-- «Ese/a remesadora no está en la lista.» — el mismo bloqueo con otro texto.
-- El catálogo valida lo que LLEGÓ; no tiene nada que decir sobre lo que no se
-- preguntó.
...
BEGIN
    ...
    -- La entidad se exige sólo cuando la contesta una PERSONA. Con
    -- `entidad_la_dice_el_papel` sale de la boleta y puede no haber ninguna —un
    -- retiro por token no tiene remesadora—, así que pedirla acá dejaba sin
    -- forma de registrar la salida. Es la MISMA condición que aplica
    -- `SalidaDeBolsa`; escrita en dos sitios porque no hay uno solo que valga
    -- para los dos runtimes, y por eso se mueven juntas.
    IF t.etiqueta_entidad IS NOT NULL
       AND NOT coalesce(t.entidad_la_dice_el_papel, false)
       AND v_entidad IS NULL THEN
    ...
    --
    -- Sólo sobre una entidad que LLEGÓ: sin `v_entidad IS NOT NULL`, una salida
    -- que legítimamente no trae entidad no emparejaba con ninguna fila y caía
    -- en «no está en la lista», que es el mismo bloqueo con otro texto.
    IF v_entidad IS NOT NULL
       AND t.etiqueta_entidad IS NOT NULL
