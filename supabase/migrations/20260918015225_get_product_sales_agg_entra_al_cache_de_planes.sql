...
-- Misma corrección que `get_product_sales_total`, sobre la función que hace el
-- trabajo del listado de Ventas > Productos. Es `LANGUAGE sql` CON `SET
-- search_path`, o sea la trampa 4 de CLAUDE.md: nace genérica y nunca ve los
-- valores de sus argumentos.
--
-- Acá pesa más que en su hermana porque son NUEVE los predicados que quedan sin
-- plegar — cuatro `p_branch_id IS NULL OR …` y cinco `p_search IS NULL OR
-- p_search = '' OR …` — y los dos cambian el plan de verdad:
--
--   · con `p_branch_id` a la vista, `si.branch_id = 4` entra como Index Cond de
--     `idx_si_branch_fecha_full`; como parámetro queda de Filter y hay que leer
--     las otras seis salas para tirarlas.
--   · con `p_search` NULO a la vista, las cinco ramas `IN (SELECT …
--     prods_buscados)` desaparecen del plan; como parámetro el CTE
--     `prods_buscados` se planifica y se evalúa igual, aunque nadie buscó nada.
--     Y el caso mayoritario del listado es SIN búsqueda.
--
-- El cuerpo NO se toca: ni un predicado, ni una cerca `MATERIALIZED`. Lo único
-- que cambia es que ahora entra al caché de planes —`plpgsql` sí entra— y se le
-- pide un plan personalizado por llamada.
--
-- `#variable_conflict use_column` es obligatorio y no es cosmético: los
-- parámetros de salida se llaman igual que columnas que el cuerpo referencia sin
-- calificar (`erp_product_id`, `descripcion`, `cantidad`, `neto`,
-- `presentaciones`). Sin esa línea plpgsql corta con «column reference … is
-- ambiguous» y la función no se crea; con ella, la columna gana, que es lo que
-- el cuerpo venía diciendo cuando era `LANGUAGE sql`.
--
-- Medido antes de aplicar, agosto/2026 · Salud 4 · sin búsqueda:
--   115,331 bloques · 901 MB · 358 ms
-- Las tres huellas de control (una sala, todas las salas, y una búsqueda) se
-- tomaron sobre un mes CERRADO a propósito: el mes en curso se lee en vivo y
-- cambia entre dos mediciones, así que una huella sobre él no prueba nada.

CREATE OR REPLACE FUNCTION public.get_product_sales_agg(
    p_fini date, p_ffin date, p_branch_id integer DEFAULT NULL::integer, p_search text DEFAULT NULL::text
)
RETURNS TABLE(erp_product_id integer, descripcion text, cantidad numeric, neto numeric,
              costo_total numeric, presentaciones jsonb, ultima_venta date, ultima_venta_por_suc jsonb,
              laboratorio_id integer, laboratorio_nombre text, oculto_en_ventas boolean,
              oculto_por_first_names text, oculto_por_last_names text, oculto_at timestamp with time zone)
LANGUAGE plpgsql
STABLE
SET search_path = ''
