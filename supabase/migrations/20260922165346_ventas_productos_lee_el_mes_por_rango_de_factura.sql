-- F5b · Ventas › Productos: los renglones del mes se leen por RANGO de factura.
--
-- Después de F5 (20260922164443) el mes en curso se leía una vez, pero factura
-- por factura: 13,404 descensos al índice `idx_sii_invoice_covering`, ~4 bloques
-- cada uno, 54k bloques. Los `id` de las facturas del mes son casi contiguos
-- (13,433 facturas en su rango contra 13,404 del mes, medido el 2026-09-22), así
-- que se agrega `sii.invoice_id BETWEEN min AND max` de las facturas del mes: el
-- mismo índice se lee en secuencia por sus hojas.
--
-- Es EXACTO por construcción: el join con `facturas_mes` se mantiene, y todo
-- renglón que cumple el join tiene su `invoice_id` dentro de ese rango. El rango
-- sólo decide por dónde se entra. Si los ids dejaran de estar correlacionados
-- con la fecha (un backfill grande), el resultado sigue igual y sólo se lee más.
-- Mes sin facturas: min/max NULL, el BETWEEN no deja pasar nada, igual que antes.
--
-- Comprobado en pg_temp el 2026-09-22 como usuario, dos cuentas, seis casos:
-- 12/12 idénticas contra la función viva. Bloques por llamada:
--   alcance total  mes todas las salas 127,708 → 85,684 · un año 211,647 → 169,623
--                  mes de una sala 105,525 → 63,501 · búsqueda 56,685 → 14,661
--   Salud 1        +710 bloques en cada caso (+1%): no puede aprovechar el rango
--                  porque el RLS ya le recortó las facturas a su sala.
SET lock_timeout = '5s';
