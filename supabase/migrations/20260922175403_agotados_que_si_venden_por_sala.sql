-- Fase 1 · Lo que está agotado y SÍ vende, por sala.
--
-- Medido el 2026-09-22 sobre los 21 días de foto diaria (`inventory_daily`
-- arrancó el 1-sep): de 10,589 pares producto·sala con MIN/MAX, **2,114 (20%)
-- tuvieron al menos un día sin existencia** y **597 estuvieron los 21 días en
-- cero** — de esos, 331 habían vendido en los últimos 90 días, o sea demanda
-- real sin nada que vender (133 de ellos en Salud 5). Nadie los veía: el
-- inventario muestra lo que HAY, y un producto agotado no aparece en ninguna
-- pantalla.
--
-- ⚠️ ESTA VERSIÓN NO FUNCIONA y se corrigió 22 segundos después en
-- `20260922175425`: el LEFT JOIN a `con` traía una expresión inválida
-- (`con_pid_match(NULL)`) que plpgsql NO valida al crear la función —sólo al
-- ejecutarla—, así que habría fallado en la primera llamada. Queda archivada
-- porque se aplicó: el archivo dice lo que la base recibió.
SET lock_timeout = '5s';
...
BEGIN
  ...
  FROM (
    ...
    LEFT JOIN con ON con_pid_match(NULL) IS NULL AND con.pid = psp.erp_product_id
...
REVOKE ALL ON FUNCTION public.get_quiebres_sala(integer, integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_quiebres_sala(integer, integer) TO authenticated, service_role;
