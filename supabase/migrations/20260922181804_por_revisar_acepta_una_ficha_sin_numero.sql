-- E2 · «Por revisar» vuelve a guardar: una ficha sin número del ERP también
-- entra.
--
-- `clientes_por_revisar.erp_id` era NOT NULL con clave `(erp_id, motivo)`, pero
-- el motivo `sin_numero_erp` existe justamente para las fichas que NO tienen
-- número. Cada noche el lote de 25 llevaba una de esas, la tabla la rechazaba y
-- **se perdían las 25**: `a_revisar_no_guardados = 25` en todas las corridas y
-- la última fila guardada es del 24-ago. Un mes sin registrar nada, y la
-- corrida informando «25 a revisar» como si hubieran entrado.
--
-- Dos cambios, y el segundo es el que hace que no vuelva a pasar:
--   1. `erp_id` acepta nulo, y la llave de esas filas pasa a ser la ficha del
--      portal: índice único parcial `(customer_id, motivo) WHERE erp_id IS
--      NULL`. Sin él, `NULL <> NULL` en la clave vieja dejaría crecer una fila
--      repetida por noche.
--   2. El upsert se parte en DOS sentencias —una por llave—, porque una sola
--      no puede tener dos destinos de conflicto. La de sin número toma el
--      `customer_id` del payload (o de `datos`), que es lo único que la
--      identifica.
--
-- Probado con rollback sobre producción: la primera corrida escribe las dos
-- filas (con y sin número) y la segunda escribe 0 — el guard de churn sigue en
-- pie.
SET lock_timeout = '5s';

ALTER TABLE public.clientes_por_revisar ALTER COLUMN erp_id DROP NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS clientes_por_revisar_sin_numero_key
    ON public.clientes_por_revisar (customer_id, motivo)
...
BEGIN
  ...
  DELETE FROM _filas_por_revisar;
