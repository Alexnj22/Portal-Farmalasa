...
-- Las solicitudes pendientes pasan a tarjeta (usuario, 24-sep: «sigamos con las
-- solicitudes»). El aviso lleva `metadata.solicitud` con los datos que la
-- tarjeta dibuja, y el título la sala delante y sin emoji. El texto del cuerpo
-- —lo que se lee en el push— no cambia.
...
BEGIN
    ...
    v_doc := CASE WHEN m->>'correlativo' LIKE '%\_CCF' THEN 'Crédito fiscal'
                  WHEN m->>'correlativo' IS NOT NULL THEN 'Factura' END;
    ...
    SELECT jsonb_agg(jsonb_build_object('nombre', i->>'descripcion',
                                        'cantidad', CASE WHEN i->>'cantidad' ~ '^[0-9]+(\.[0-9]+)?$'
                                                         THEN (i->>'cantidad')::numeric END))
      INTO v_productos
