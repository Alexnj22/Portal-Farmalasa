...
-- Solicitudes pendientes, segunda vuelta (usuario, 24-sep):
--  · Anulación y cambios de factura: fecha, tipo de pago, tipo de documento y
--    cliente; el número deja de mostrarse.
--  · Traslados: de cada producto, cuántas hay en la sala a la que se lo piden y
--    lo que vendió ella en 6 meses y en el último mes.
--  · `ventas_por_mes_de_producto`: una sola respuesta para las ventas de un
--    producto en una sala, que usan este aviso y el de MIN·MAX.

CREATE OR REPLACE FUNCTION public.ventas_por_mes_de_producto(p_erp_product_id integer, p_branch_id bigint)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'extensions'
AS $function$
-- Unidades vendidas de un producto en una sala, mes por mes: los 6 meses
-- CERRADOS. En unidades —`cantidad × factor_unidades`, como
-- `get_minmax_contexto_producto`— y no sumando presentaciones. Una sola
-- respuesta para los avisos que la muestran (MIN·MAX y traslados): escrita dos
-- veces, un día dirían números distintos. `plpgsql` a propósito: el plan
-- depende de los argumentos (regla de planes genéricos en CLAUDE.md).
DECLARE
    v_hoy date := (now() AT TIME ZONE 'America/El_Salvador')::date;
    v     jsonb;
BEGIN
    IF p_erp_product_id IS NULL OR p_branch_id IS NULL THEN RETURN NULL; END IF;
    SELECT jsonb_agg(jsonb_build_object('ym', to_char(g.mes, 'YYYY-MM'),
                                        'unidades', round(coalesce(s.u, 0), 2)) ORDER BY g.mes)
      INTO v
      ...
      LEFT JOIN (
        SELECT date_trunc('month', si.fecha) AS mes,
         ...
         GROUP BY 1) s ON s.mes = g.mes;
    RETURN v;
END;
$function$;

REVOKE ALL ON FUNCTION public.ventas_por_mes_de_producto(integer, bigint) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.ventas_por_mes_de_producto(integer, bigint) TO service_role;
...
BEGIN
    ...
    BEGIN
        SELECT jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
          ...
          INTO v_productos
          FROM (SELECT i FROM jsonb_array_elements(coalesce(m->'items', '[]'::jsonb)) i LIMIT 3) x;
    EXCEPTION WHEN OTHERS THEN
        ...
        SELECT jsonb_agg(jsonb_build_object('nombre', i->>'descripcion'))
          ...
          FROM (SELECT i FROM jsonb_array_elements(coalesce(m->'items', '[]'::jsonb)) i LIMIT 3) x;
    END;
    v_mas := greatest(coalesce(jsonb_array_length(m->'items'), 0) - 3, 0);
