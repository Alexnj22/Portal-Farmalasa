...
-- Cambio sobre 20260924210959: fuera lo que no tiene presentación activa.
...
BEGIN
    ...
    -- El piso de ids de factura del período. Sin él, la búsqueda por producto
    -- recorre TODO el historial de cada uno: medido en Salud 5, 520,921
    -- bloques contra 18,089 con el piso (memoria «el rango de ids entra por
    -- las hojas del índice»: las facturas de un período son casi contiguas).
    rango AS MATERIALIZED (
        SELECT min(id) AS m FROM sales_invoices WHERE fecha >= CURRENT_DATE - 180
    ),
    ventas AS MATERIALIZED (
        SELECT bm.esid, ii.erp_product_id AS pid,
          ...
          JOIN sales_invoice_items ii ON ii.erp_product_id = inv.pid AND ii.invoice_id >= r.m
    ...
    filas AS (
        SELECT p.pid                          AS erp_product_id,
          ...
          JOIN products pr         ON pr.id = p.pid AND pr.activo
                                  -- Sin presentación activa no se puede trasladar, y lo que
                                  -- no la tiene no es mercadería: «COMISIONES POR SERVICIO DE
                                  -- CORRESPONSAL» (el 4239) salía como producto parado.
                                  AND EXISTS (SELECT 1 FROM product_precios pp
                                               WHERE pp.product_id = p.pid AND pp.activo AND pp.factor > 0)
