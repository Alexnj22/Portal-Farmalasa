...
-- El aviso semanal de productos sin venta, al jefe de cada sala.
--
-- La lista la decide `productos_parados_de_sala` —el mismo juez de la pestaña
-- «Stock retenido»—, así que el aviso y la pantalla no pueden contestar
-- distinto. Acá sólo se decide A QUIÉN y QUÉ ES NUEVO.
--
-- «Sólo lo nuevo» (usuario, 2026-09-24): cada producto se avisa UNA vez por
-- sala. `avisado_el` se OBSERVA la primera vez y no se recalcula; si el
-- producto se vende o se envía y después vuelve a quedar parado, sale de la
-- lista (se borra su fila) y vuelve a avisarse como nuevo.
CREATE TABLE public.productos_sin_venta_avisados (
    erp_sucursal_id  smallint    NOT NULL,
    erp_product_id   integer     NOT NULL REFERENCES public.products(id),
    avisado_el       date        NOT NULL DEFAULT CURRENT_DATE,
    created_at       timestamptz NOT NULL DEFAULT now(),
    PRIMARY KEY (erp_sucursal_id, erp_product_id)
);
CREATE INDEX productos_sin_venta_avisados_product_idx
    ON public.productos_sin_venta_avisados (erp_product_id);

ALTER TABLE public.productos_sin_venta_avisados ENABLE ROW LEVEL SECURITY;
CREATE POLICY productos_sin_venta_avisados_select ON public.productos_sin_venta_avisados
    FOR SELECT TO authenticated
    USING ((SELECT public.auth_has_module_permission('gestion_stock', 'can_view')));
REVOKE ALL ON public.productos_sin_venta_avisados FROM anon;
...
BEGIN
    ...
    FOR s IN SELECT * FROM jsonb_array_elements(v_salas) LOOP
        ...
        v_meta := jsonb_build_object('parados', jsonb_build_object(
            ...
            'destinos', (SELECT jsonb_agg(d ORDER BY (d->>'productos')::int DESC)
                           FROM (SELECT jsonb_build_object(
                                        'erp', (e->>'destino')::int,
                                        'sala', v_nombre->>(e->>'destino'),
                                        'productos', count(*),
                                        'costo', round(sum(coalesce((e->>'costo')::numeric, 0)), 2)) d
                                   FROM jsonb_array_elements(v_nuevos) e
                                  GROUP BY (e->>'destino')::int) x),
