-- F4 de docs/PLAN-BUSQUEDA-UNIFICADA-2026-09-25.md: /inventario busca con la
-- regla del portal. Las tres funciones (lista, inversión y próximos a vencer)
-- resuelven la búsqueda UNA vez con `busqueda_productos` y comparan por
-- `erp_product_id`, que la vista materializada ya tiene: no hace falta
-- reconstruirla. La descripción del inventario ES el nombre del producto
-- (medido en 24,226 de 24,226 filas, migración 20260807165453).
...
DECLARE
    ...
    -- los productos que coinciden, resueltos UNA vez —nombre y código de
    -- barras, lo que la tabla muestra—. Antes eran patrones sobre
    -- `descripcion_norm` más una lista aparte de códigos. Es una variable y no
    -- una llamada dentro del WHERE a propósito: así el planificador la ve como
    -- constante y no la evalúa por fila. NULL = sin búsqueda.
    v_ids int[] := CASE WHEN btrim(coalesce(p_search, '')) = '' THEN NULL
                        ELSE ARRAY(SELECT b.id FROM public.busqueda_productos(p_search, false, false, false) b) END;
BEGIN
...
IF p_area_vencidos THEN
    ...
    WHERE m.vencidos_unidades > 0
      ...
      AND (v_ids IS NULL OR m.erp_product_id = ANY(v_ids));
...
ELSE
    ...
    FROM (
        ...
        WHERE i.is_vencidos = false
          ...
          AND (v_ids IS NULL OR i.erp_product_id = ANY(v_ids))
...
DECLARE
  v_result numeric;
  -- La misma búsqueda que la lista (`inventory_grouped`): antes esta suma no
  -- leía el código de barras y la lista sí, así que buscando un código el
  -- total no cuadraba con las filas.
  v_ids int[] := CASE WHEN btrim(coalesce(p_search, '')) = '' THEN NULL
                      ELSE ARRAY(SELECT b.id FROM public.busqueda_productos(p_search, false, false, false) b) END;
BEGIN
...
CREATE OR REPLACE FUNCTION public.inventory_proximos_count(p_erp_id integer DEFAULT NULL::integer, p_lab_id integer DEFAULT NULL::integer, p_categoria text DEFAULT NULL::text, p_search text DEFAULT NULL::text)
 ...
 SET search_path TO 'public'
AS $function$
-- plpgsql y no `LANGUAGE sql` + `SET` (la trampa 4 de CLAUDE.md). La misma
-- búsqueda que la lista: antes no leía el código de barras y el contador de
-- «próximos a vencer» no cuadraba con las filas.
DECLARE
  v_ids int[] := CASE WHEN btrim(coalesce(p_search, '')) = '' THEN NULL
                      ELSE ARRAY(SELECT b.id FROM public.busqueda_productos(p_search, false, false, false) b) END;
BEGIN
