-- F4 de docs/PLAN-BUSQUEDA-UNIFICADA-2026-09-25.md: el conteo físico busca con
-- la regla del portal. Las cuatro funciones (renglones y productos, lista y
-- contador) buscan sobre producto + lote + laboratorio + presentación +
-- código, fila por fila. El `LIKE` sobre `norm_search` se conserva como
-- PREFILTRO —barato y superconjunto de lo que la regla acepta— y la regla
-- exacta (`busqueda_coincide`) decide sólo sobre lo que pasa.
-- Sólo crea y reemplaza funciones: no toca ninguna tabla.
SET lock_timeout = '5s';

-- Los patrones `LIKE` para columnas normalizadas con la regla VIEJA
-- (`norm_search`, que borra la puntuación en vez de separar). Cada palabra se
-- reduce a letras y dígitos —«2.5» → «25», que es como `norm_search` escribe
-- «2.5MG»— y de una mixta («b 12») se toma la parte más larga. Es un
-- superconjunto: la regla exacta decide después. Sirve para las tablas con
-- índice sobre `norm_search(...)` que no conviene reconstruir (ventas).
CREATE OR REPLACE FUNCTION public.busqueda_patrones_legados(p_palabras jsonb)
RETURNS text[]
...
DECLARE
  ...
  i     int;
BEGIN
  IF coalesce(jsonb_array_length(p_palabras), 0) = 0 THEN RETURN NULL; END IF;
  FOR i IN 0 .. jsonb_array_length(p_palabras) - 1 LOOP
    e := p_palabras -> i;
    IF e->>'tipo' = 'mixto' THEN
      SELECT x INTO pieza FROM unnest(string_to_array(e->>'t', ' ')) x ORDER BY length(x) DESC LIMIT 1;
    ELSE
      pieza := e->>'t';
    END IF;
    pats := pats || ('%' || regexp_replace(pieza, '[^a-z0-9]', '', 'g') || '%');
  END LOOP;
  RETURN pats;
END;
$$;

-- ¿Coincide un texto (varios campos ya concatenados con espacio)? La misma
-- regla que `coincide` de src/utils/busqueda.js.
CREATE OR REPLACE FUNCTION public.busqueda_coincide(p_palabras jsonb, p_texto text)
RETURNS boolean
LANGUAGE plpgsql IMMUTABLE PARALLEL SAFE
SET search_path = public, extensions
AS $$
BEGIN
  RETURN public.busqueda_puntaje(p_palabras,
    ARRAY[public.norm_busqueda(p_texto)], ARRAY[public.compactar_busqueda(p_texto)]) > 0;
END;
$$;

REVOKE EXECUTE ON FUNCTION public.busqueda_patrones_legados(jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.busqueda_patrones_legados(jsonb) TO authenticated, service_role;
REVOKE EXECUTE ON FUNCTION public.busqueda_coincide(jsonb, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.busqueda_coincide(jsonb, text) TO authenticated, service_role;

-- El respaldo aproximado del conteo, calculado UNA vez y del mismo modo para la
-- lista y el contador (si no, la paginación diría un número y mostraría otro):
-- NULL si la búsqueda está vacía o si ALGÚN renglón del conteo coincide tal
-- cual; si no, los productos del conteo PARECIDOS a lo escrito (la regla de
-- `busqueda_productos`: nombre, laboratorio y código). La pantalla lo llama
-- también para saber si tiene que avisar «resultados similares».
CREATE OR REPLACE FUNCTION public.conteo_busqueda_aproximada(p_conteo_id uuid, p_search text)
RETURNS integer[]
...
BEGIN
  IF v_pats IS NULL THEN RETURN NULL; END IF;
  IF EXISTS (
    SELECT 1
    ...
    WHERE ci.conteo_id = p_conteo_id
      AND public.norm_search(coalesce(p.nombre,'') || ' ' || coalesce(ci.lote,'') || ' ' ||
            coalesce(l.nombre,'') || ' ' || coalesce(ci.presentacion,'') || ' ' || coalesce(p.codigo_barras,''))
          LIKE ALL (v_pats)
      AND public.busqueda_coincide(v_tok, coalesce(p.nombre,'') || ' ' || coalesce(ci.lote,'') || ' ' ||
            coalesce(l.nombre,'') || ' ' || coalesce(ci.presentacion,'') || ' ' || coalesce(p.codigo_barras,''))
  ) THEN
    RETURN NULL;
  END IF;
  RETURN ARRAY(
    SELECT DISTINCT ci.erp_product_id
    FROM public.conteo_inventario_items ci
    JOIN public.busqueda_productos(p_search, false, true, false) b ON b.id = ci.erp_product_id
    WHERE ci.conteo_id = p_conteo_id AND b.aproximado
  );
END;
$$;

REVOKE EXECUTE ON FUNCTION public.conteo_busqueda_aproximada(uuid, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.conteo_busqueda_aproximada(uuid, text) TO authenticated, service_role;

CREATE OR REPLACE FUNCTION public.get_conteo_items_search(p_conteo_id uuid, p_search text DEFAULT NULL::text, p_filtro text DEFAULT 'TODOS'::text, p_limit integer DEFAULT 50, p_offset integer DEFAULT 0, p_erp_product_id integer DEFAULT NULL::integer, p_erp_product_ids integer[] DEFAULT NULL::integer[], p_area text DEFAULT NULL::text)
...
DECLARE
  ...
  v_filtro text;
  -- La regla del portal (docs/PLAN-BUSQUEDA-UNIFICADA-2026-09-25.md). El
  -- `LIKE` sobre `norm_search` se queda como prefiltro barato —un
  -- superconjunto— y `busqueda_coincide` decide sobre lo que pasa.
  v_tok  jsonb  := public.busqueda_palabras(p_search);
  v_pats text[] := public.busqueda_patrones_legados(public.busqueda_palabras(p_search));
  -- Sin ninguna coincidencia exacta en todo el conteo: los productos PARECIDOS.
  v_aprox int[]  := public.conteo_busqueda_aproximada(p_conteo_id, p_search);
BEGIN
  ...
  filtered AS (
    ...
    WHERE (v_filtro = 'TODOS' OR v_filtro IS NULL
      ...
      AND (v_pats IS NULL OR (v_aprox IS NOT NULL AND b.erp_product_id = ANY(v_aprox)) OR (v_aprox IS NULL AND public.norm_search(
             coalesce(b.p_nombre,'') || ' ' || coalesce(b.lote,'') || ' ' ||
             coalesce(b.l_nombre,'') || ' ' || coalesce(b.presentacion,'') || ' ' ||
             coalesce(b.p_codigo,'')
           ) LIKE ALL (v_pats) AND public.busqueda_coincide(v_tok,
             coalesce(b.p_nombre,'') || ' ' || coalesce(b.lote,'') || ' ' ||
             coalesce(b.l_nombre,'') || ' ' || coalesce(b.presentacion,'') || ' ' ||
             coalesce(b.p_codigo,''))))
...
BEGIN
  ...
  base AS MATERIALIZED (
    SELECT ci.id, ci.conteo_id, ci.erp_product_id, ci.source_inventory_id, ci.presentacion, ci.detalle, ci.lote, ci.fecha_vencimiento, ci.is_vencidos, ci.sistema_cantidad, ci.fisico_cantidad, ci.diferencia, ci.estado_item, ci.nota, ci.contado_por, ci.contado_at, ci.es_agregado_manual, ci.source_sync_key, ci.sistema_inicial, ci.fisico_primer_conteo, ci.recontado_por, ci.recontado_at, ci.grupo_key, p.nombre AS p_nombre, p.es_antibiotico AS p_es_antibiotico, p.foto_url AS p_foto_url,
    ...
    WHERE ci.conteo_id = p_conteo_id
      ...
      -- El área de vencidos es OTRO anaquel, y por eso se cuenta aparte. Sin
      -- este filtro un producto que está en las dos —medidos 42 de 83 en el
      -- conteo abierto— se agrupaba en UNA fila y su «Sistema / Físico / Dif.»
      -- sumaba las dos áreas: no era sólo que se vieran mezclados, era que el
      -- total de esos productos estaba mal.
      -- NULL = las dos, que es como se llamaba hasta ahora.
      AND (p_area IS NULL OR (p_area = 'VENCIDOS') = ci.is_vencidos)
  ),
  live_raw AS MATERIALIZED (
    SELECT i.sync_key, i.erp_product_id AS r_pid, i.is_vencidos AS r_venc, i.cantidad::int AS cantidad,
  ...
  matched AS (
    ...
    WHERE (v_pats IS NULL OR (v_aprox IS NOT NULL AND b.erp_product_id = ANY(v_aprox)) OR (v_aprox IS NULL AND public.norm_search(
...
CREATE OR REPLACE FUNCTION public.get_conteo_products_count(p_conteo_id uuid, p_search text DEFAULT NULL::text, p_filtro text DEFAULT 'TODOS'::text, p_laboratorio_id integer DEFAULT NULL::integer, p_area text DEFAULT NULL::text)
 RETURNS bigint
 LANGUAGE plpgsql
 STABLE
 SET search_path TO 'public', 'extensions'
...
BEGIN
  ...
  matched AS (
    ...
    WHERE (v_pats IS NULL OR (v_aprox IS NOT NULL AND erp_product_id = ANY(v_aprox)) OR (v_aprox IS NULL AND public.norm_search(
...
CREATE OR REPLACE FUNCTION public.get_conteo_items_count(p_conteo_id uuid, p_search text DEFAULT NULL::text, p_filtro text DEFAULT 'TODOS'::text, p_erp_product_id integer DEFAULT NULL::integer)
 ...
 SET plan_cache_mode TO 'force_custom_plan'
AS $function$
-- plpgsql y no `LANGUAGE sql` + `SET` (la trampa 4 de CLAUDE.md): así la
-- búsqueda se resuelve una vez por llamada y no por fila.
DECLARE
  -- La regla del portal (docs/PLAN-BUSQUEDA-UNIFICADA-2026-09-25.md). El
  -- `LIKE` sobre `norm_search` se queda como prefiltro barato —un
  -- superconjunto— y `busqueda_coincide` decide sobre lo que pasa.
...
BEGIN
  ...
  WHERE (cfg.filtro = 'TODOS' OR cfg.filtro IS NULL
    ...
    AND (v_pats IS NULL OR (v_aprox IS NOT NULL AND base.erp_product_id = ANY(v_aprox)) OR (v_aprox IS NULL AND public.norm_search(
