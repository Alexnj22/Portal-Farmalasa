-- El conteo buscaba bien pero lento: cada llamada normalizaba los 3,400
-- renglones del conteo con `norm_search` DOS veces (el respaldo aproximado y
-- la consulta), ~100 ms cada una. Medido: `get_conteo_products_count` pasó de
-- 55 ms sin búsqueda a 347 ms con «acetaminofen».
--
-- El prefiltro ahora compara contra columnas ya normalizadas —`products
-- .busq_todo` (existía) y `laboratorios.nombre_busq` (nueva, 358 filas)— más
-- lote y presentación en minúsculas, sin tildes y sin puntuación (así «2.5 ML»
-- contiene el «25» que busca «2.5»), que son cortos. La regla
-- exacta sigue igual, sobre lo que pasa el prefiltro. Mismos resultados.
SET lock_timeout = '5s';

ALTER TABLE public.laboratorios
  ADD COLUMN IF NOT EXISTS nombre_busq text GENERATED ALWAYS AS (
    public.norm_busqueda(nombre) || ' ' || public.compactar_busqueda(nombre)) STORED;
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE ci.conteo_id = p_conteo_id
      AND (coalesce(p.busq_todo,'') || ' ' || coalesce(l.nombre_busq,'') || ' ' ||
           regexp_replace(public.f_unaccent(lower(coalesce(ci.lote,'') || ' ' || coalesce(ci.presentacion,''))), '[^a-z0-9 ]', '', 'g'))
          LIKE ALL (v_pats)
...
DECLARE
  ...
  v_filtro text;
  -- La regla del portal (docs/PLAN-BUSQUEDA-UNIFICADA-2026-09-25.md). Un
  -- prefiltro `LIKE` sobre columnas YA normalizadas (`products.busq_todo`,
  -- `laboratorios.nombre_busq`) —un superconjunto, sin función por fila— y
  -- `busqueda_coincide` decide sólo sobre lo que pasa.
  v_tok  jsonb  := public.busqueda_palabras(p_search);
  v_pats text[] := public.busqueda_patrones_legados(public.busqueda_palabras(p_search));
...
BEGIN
  ...
  base AS MATERIALIZED (
    SELECT ci.id, ci.conteo_id, ci.erp_product_id, ci.source_inventory_id, ci.presentacion, ci.detalle, ci.lote, ci.fecha_vencimiento, ci.is_vencidos, ci.sistema_cantidad, ci.fisico_cantidad, ci.diferencia, ci.estado_item, ci.nota, ci.contado_por, ci.contado_at, ci.es_agregado_manual, ci.source_sync_key, ci.sistema_inicial, ci.fisico_primer_conteo, ci.recontado_por, ci.recontado_at, ci.grupo_key, (SELECT cc.costo_unitario FROM public.conteo_inventario_costos cc WHERE cc.item_id = ci.id) AS costo_unitario, p.nombre AS p_nombre, p.es_antibiotico AS p_es_antibiotico, p.foto_url AS p_foto_url,
           ...
           p.busq_todo AS p_busq, l.nombre_busq AS l_busq,
  ...
  filtered AS (
    ...
    WHERE (v_filtro = 'TODOS' OR v_filtro IS NULL
      ...
      AND (v_pats IS NULL OR (v_aprox IS NOT NULL AND b.erp_product_id = ANY(v_aprox)) OR (v_aprox IS NULL AND (coalesce(b.p_busq,'') || ' ' || coalesce(b.l_busq,'') || ' ' ||
               regexp_replace(public.f_unaccent(lower(coalesce(b.lote,'') || ' ' || coalesce(b.presentacion,''))), '[^a-z0-9 ]', '', 'g'))
...
BEGIN
  ...
  base AS MATERIALIZED (
    SELECT ci.id, ci.conteo_id, ci.erp_product_id, ci.source_inventory_id, ci.presentacion, ci.detalle, ci.lote, ci.fecha_vencimiento, ci.is_vencidos, ci.sistema_cantidad, ci.fisico_cantidad, ci.diferencia, ci.estado_item, ci.nota, ci.contado_por, ci.contado_at, ci.es_agregado_manual, ci.source_sync_key, ci.sistema_inicial, ci.fisico_primer_conteo, ci.recontado_por, ci.recontado_at, ci.grupo_key, p.nombre AS p_nombre, p.es_antibiotico AS p_es_antibiotico, p.foto_url AS p_foto_url,
           ...
           p.codigo_barras AS p_codigo, p.busq_todo AS p_busq, l.nombre_busq AS l_busq,
  ...
  matched AS (
    ...
    WHERE (v_pats IS NULL OR (v_aprox IS NOT NULL AND b.erp_product_id = ANY(v_aprox)) OR (v_aprox IS NULL AND (coalesce(b.p_busq,'') || ' ' || coalesce(b.l_busq,'') || ' ' ||
               regexp_replace(public.f_unaccent(lower(coalesce(b.lote,'') || ' ' || coalesce(b.presentacion,''))), '[^a-z0-9 ]', '', 'g'))
             LIKE ALL (v_pats) AND public.busqueda_coincide(v_tok,
...
BEGIN
  ...
  base AS MATERIALIZED (
    SELECT ci.erp_product_id, ci.estado_item, ci.diferencia, ci.lote, ci.presentacion,
           ...
           p.codigo_barras, p.busq_todo AS p_busq, l.nombre_busq AS l_busq,
           nt.neto_grupo
    FROM public.conteo_inventario_items ci
    LEFT JOIN public.products p ON p.id = ci.erp_product_id
    LEFT JOIN public.laboratorios l ON l.id = p.laboratorio_id
  ...
  matched AS (
    ...
    WHERE (v_pats IS NULL OR (v_aprox IS NOT NULL AND erp_product_id = ANY(v_aprox)) OR (v_aprox IS NULL AND (coalesce(p_busq,'') || ' ' || coalesce(l_busq,'') || ' ' ||
               regexp_replace(public.f_unaccent(lower(coalesce(lote,'') || ' ' || coalesce(presentacion,''))), '[^a-z0-9 ]', '', 'g'))
             LIKE ALL (v_pats) AND public.busqueda_coincide(v_tok,
...
BEGIN
  ...
  WHERE (cfg.filtro = 'TODOS' OR cfg.filtro IS NULL
    ...
    AND (v_pats IS NULL OR (v_aprox IS NOT NULL AND base.erp_product_id = ANY(v_aprox)) OR (v_aprox IS NULL AND (coalesce(base.p_busq,'') || ' ' || coalesce(base.l_busq,'') || ' ' ||
               regexp_replace(public.f_unaccent(lower(coalesce(base.lote,'') || ' ' || coalesce(base.presentacion,''))), '[^a-z0-9 ]', '', 'g'))
             LIKE ALL (v_pats) AND public.busqueda_coincide(v_tok,
