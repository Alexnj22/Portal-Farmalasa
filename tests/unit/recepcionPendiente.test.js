...
const SALIO = '2026-08-17T14:12:00Z';
const LA_POPULAR_116 = { enviadoAt: '2026-08-17T14:12:00Z', pedidoStatus: 'parcial', pendientes: 139, reenviosHistorial: [] };

describe('hayRecepcionPendiente', () => {
    it('un pedido en curso se recibe', () => {
        expect(hayRecepcionPendiente({ enviadoAt: SALIO, pedidoStatus: 'enviado', pendientes: 174 })).toBe(true);
    });
    ...
    it('con todo contado, «parcial» ya no ofrece recibir — sólo resolver diferencias', () => {
        expect(hayRecepcionPendiente({ enviadoAt: SALIO, pedidoStatus: 'parcial', pendientes: 0 })).toBe(false);
    });
    ...
    it('un pedido que todavía no sale de bodega no se recibe, aunque todo esté pendiente', () => {
        ...
        expect(hayRecepcionPendiente({ enviadoAt: null, pedidoStatus: 'confirmado', pendientes: 174 })).toBe(false);
    });

    it('un pedido terminado no se recibe', () => {
        expect(hayRecepcionPendiente({ enviadoAt: SALIO, pedidoStatus: 'completado', pendientes: 0 })).toBe(false);
        expect(hayRecepcionPendiente({ enviadoAt: SALIO, pedidoStatus: 'anulado', pendientes: 12 })).toBe(false);
    });

    it('un reenvío en camino abre la recepción por su cuenta', () => {
        expect(hayRecepcionPendiente({
            enviadoAt: SALIO, pedidoStatus: 'parcial', pendientes: 0,
    ...
    it('un reenvío que ya llegó no la abre sola', () => {
        expect(hayRecepcionPendiente({
            enviadoAt: SALIO, pedidoStatus: 'completado', pendientes: 0,
    ...
    // El caso real: pedido 137 del 2026-08-24 (`09-240826-2-S2`, Salud 2). El
    // pedido tenía dos salas; sólo Salud 1 se preparó y salió en la ruta #21.
    // `pedidos.status` pasó a «enviado» para el pedido ENTERO, así que Salud 2
    // —sin `iniciado_at`, sin `finalizado_at`, sin parada y con `total_cajas` en
    // NULL— mostraba el botón «Confirmar llegada de cajas».
    it('una sala que no salió no recibe, aunque el PEDIDO esté en ruta', () => {
        expect(hayRecepcionPendiente({
            enviadoAt: null, pedidoStatus: 'enviado', pendientes: 210,
        })).toBe(false);
    });

    it('y la sala del mismo pedido que sí salió recibe normal', () => {
        expect(hayRecepcionPendiente({
            enviadoAt: '2026-08-24T17:51:31Z', pedidoStatus: 'enviado', pendientes: 302,
        })).toBe(true);
    });
});
